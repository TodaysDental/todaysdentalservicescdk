...
// src/services/marketing/publisher.ts
...
// src/services/marketing/ayrshare-client.ts
var AYRSHARE_URL = "https://api.ayrshare.com/api";
var AYRSHARE_HTTP_TIMEOUT_MS = (() => {
  const raw = Number.parseInt(process.env.AYRSHARE_HTTP_TIMEOUT_MS || "12000", 10);
  return Number.isFinite(raw) && raw > 0 ? raw : 12e3;
})();
var AYRSHARE_POST_TIMEOUT_MS = (() => {
  const raw = Number.parseInt(process.env.AYRSHARE_POST_TIMEOUT_MS || "27000", 10);
  return Number.isFinite(raw) && raw > 0 ? raw : 27e3;
})();
var ayrshareHttp = axios_default.create({ timeout: AYRSHARE_HTTP_TIMEOUT_MS });
function extractAyrshareErrorMessage(errorData) {
  if (!errorData)
    return void 0;
  ...
  if (Array.isArray(errorData?.errors)) {
    const msgs = errorData.errors.map((e) => e?.message || e?.error).filter(Boolean).map((m) => String(m));
    if (msgs.length > 0)
...
async function ayrsharePostSingle(apiKey, profileKey, postData, timeoutMs) {
  ...
  try {
    ...
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/post`,
      postData,
      {
        headers: {
    ...
    if (isTimeout) {
      throw new Error(
        `Ayrshare request timed out after ${effectiveTimeout}ms. The post may still have been processed\u2014check History before retrying.`
      );
    }
    const errorMessage = extractAyrshareErrorMessage(errorData) || err.message || "Failed to post";
    throw new Error(errorMessage);
  }
}
async function ayrsharePost(apiKey, profileKey, postData, timeoutMs) {
  ...
  const settled = await Promise.allSettled(
    platforms.map(
      (platform) => ayrsharePostSingle(apiKey, profileKey, {
        ...postData,
        platforms: [platform]
  ...
  return {
    ...
    ...failures.length > 0 ? {
      ...
      partialFailureMessage: `Posted to ${successes.map((s) => s.platform).join(", ")} but failed on ${failures.map((f) => f.platform).join(", ")}: ` + failures.map((f) => f.error).join("; ")
    } : {}
  };
}
async function ayrshareDeletePost(apiKey, profileKey, postId) {
  try {
    const res = await ayrshareHttp.delete(
      `${AYRSHARE_URL}/post`,
      {
        headers: {
          "Authorization": `Bearer ${apiKey}`,
...
async function ayrshareGetHistory(apiKey, profileKey, params) {
  try {
...
async function ayrshareGetAnalytics(apiKey, profileKey, postId) {
  try {
...
async function ayrshareGetSocialStats(apiKey, profileKey, platforms) {
  try {
...
async function ayrshareGetComments(apiKey, profileKey, postId) {
  try {
...
async function ayrshareReplyToComment(apiKey, profileKey, commentId, replyText, platform) {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/comments`,
      {
        commentId,
        comment: replyText,
        platform
...
async function ayrshareSetAutoSchedule(apiKey, profileKey, schedule) {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/auto-schedule/set`,
      schedule,
      {
        headers: {
          "Authorization": `Bearer ${apiKey}`,
...
async function ayrshareGetAutoSchedule(apiKey, profileKey) {
  try {
...
async function ayrshareDeleteAutoSchedule(apiKey, profileKey, title) {
  try {
...
async function ayrshareRegisterWebhook(apiKey, action, url2, secret, profileKey) {
  try {
    const headers = {
      "Authorization": `Bearer ${apiKey}`,
      "Content-Type": "application/json"
    ...
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/hook/webhook`,
      { action, url: url2, secret },
      { headers }
    );
    return res.data;
  } catch (err) {
...
async function ayrshareUnregisterWebhook(apiKey, action, profileKey) {
  try {
    const headers = {
      "Authorization": `Bearer ${apiKey}`,
      "Content-Type": "application/json"
...
async function ayrshareGetWebhooks(apiKey, profileKey) {
  try {
    const headers = {
      "Authorization": `Bearer ${apiKey}`
    };
...
async function ayrshareAutoHashtag(apiKey, text, max = 5, position = "end") {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/hashtags/auto`,
...
async function ayrshareRecommendHashtags(apiKey, keyword, limit = 10) {
  try {
...
async function ayrshareSearchHashtags(apiKey, query, platform = "instagram") {
  try {
...
async function ayrshareCheckBannedHashtags(apiKey, hashtags) {
  try {
...
async function ayrshareValidateMedia(apiKey, mediaUrls, platforms) {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/media/verifyUrl`,
      { mediaUrls, platforms },
      { headers: { "Authorization": `Bearer ${apiKey}` } }
    );
    return res.data;
...
async function ayrshareContentModeration(apiKey, content) {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/validate/moderation`,
      { text: content },
      { headers: { "Authorization": `Bearer ${apiKey}` } }
    );
    return res.data;
...
async function ayrshareResizeImage(apiKey, imageUrl, width, height) {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/media/resize`,
...
async function ayrshareVerifyMediaUrl(apiKey, url2) {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/media/verifyUrl`,
      { url: url2 },
...
async function ayrshareGetLinkAnalytics(apiKey, profileKey, postId) {
  try {
    const params = {};
    if (postId)
      params.id = postId;
...
var TABLE_NAME = process.env.MARKETING_CONFIG_TABLE;
var POSTS_TABLE = process.env.MARKETING_POSTS_TABLE || "MarketingPosts";
var IMAGE_GENERATOR_FUNCTION = process.env.IMAGE_GENERATOR_FUNCTION || "ImageGeneratorFn";
var CLINIC_CONFIG_TABLE2 = process.env.CLINIC_CONFIG_TABLE;
var BATCH_SIZE = 3;
var DELAY_BETWEEN_BATCHES_MS = 2e3;
var MAX_RETRIES = 1;
var RETRY_DELAY_MS = 1e3;
var MAX_SYNC_CLINICS = (() => {
...
var ASYNC_POST_TIMEOUT_MS = 55e3;
...
async function postToClinicWithRetry(apiKey, config, postData, saveHistory, retries = 0) {
  try {
...
function isRetryableError(error) {
  ...
  if (message.includes("timeout"))
    return false;
  return message.includes("rate limit") || message.includes("too many requests") || message.includes("429") || message.includes("network");
}
function resolvePostPlaceholders(postData, clinicData) {
...
async function getClinicConfigMap(clinicIds) {
  if (!CLINIC_CONFIG_TABLE2)
    return {};
  if (!Array.isArray(clinicIds) || clinicIds.length === 0)
    return {};
  try {
    const keys = clinicIds.map((id) => ({ clinicId: String(id) }));
    const response = await ddb.send(new import_lib_dynamodb.BatchGetCommand({
      RequestItems: {
        [CLINIC_CONFIG_TABLE2]: { Keys: keys }
      }
    }));
    const items = response.Responses?.[CLINIC_CONFIG_TABLE2] || [];
    const map = {};
...
var handler = async (event) => {
  ...
  const apiGwEvent = event;
  const origin2 = apiGwEvent.headers?.origin || apiGwEvent.headers?.Origin;
  const corsHeaders = buildCorsHeaders({ allowMethods: ["OPTIONS", "POST", "GET", "DELETE"] }, origin2);
  ...
  try {
    if (apiGwEvent.path.endsWith("/post") && apiGwEvent.httpMethod === "POST") {
      ...
      const requestedClinicIds = targetClinicIds.map((id) => String(id));
      const keys = requestedClinicIds.map((id) => ({ clinicId: id }));
      const dbRes = await ddb.send(new import_lib_dynamodb.BatchGetCommand({
        RequestItems: {
          [TABLE_NAME]: { Keys: keys }
        }
      }));
      const configs = dbRes.Responses?.[TABLE_NAME] || [];
      const configByClinicId = {};
      ...
      const clinicConfigMap = resolvePlaceholders ? await getClinicConfigMap(requestedClinicIds) : {};
      ...
      if (!selfFunctionName) {
        ...
        const allResults2 = [
          ...missingClinicIds.map((clinicId) => ({ clinicId, status: "failed", error: "No marketing profile found for clinic" }))
        ];
        for (const batch of batches) {
          ...
          allResults2.push(...batchResults);
        }
        return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ total: allResults2.length, success: allResults2.filter((r) => r.status === "success").length, failed: allResults2.filter((r) => r.status === "failed").length, results: allResults2 }) };
      }
      ...
      const invokePromises = orderedConfigs.map((config) => {
        const clinicData = clinicConfigMap[String(config?.clinicId)] || config;
        const resolvedPostData = resolvePlaceholders ? resolvePostPlaceholders(postData, clinicData) : postData;
        return lambda.send(new import_client_lambda.InvokeCommand({
    ...
    if (apiGwEvent.path.endsWith("/post/validate") && apiGwEvent.httpMethod === "POST") {
      ...
      if (scheduleDate) {
        ...
        } else if (parsed <= /* @__PURE__ */ new Date()) {
    ...
    if (apiGwEvent.path.endsWith("/post/bulk") && apiGwEvent.httpMethod === "POST") {
      ...
      if (canvasJson && canvasJson.objects && canvasJson.objects.length > 0) {
        try {
          ...
          const invokeRes = await lambda.send(new import_client_lambda.InvokeCommand({
      ...
      if (!selfFunctionName) {
        ...
        const apiKey = await getApiKey();
        const allResults2 = [...missingClinicIds.map((clinicId) => ({ clinicId, status: "failed", error: "No marketing profile found for clinic" }))];
        for (const config of orderedConfigs) {
          try {
            ...
            allResults2.push({ clinicId: config.clinicId, status: "success", id: res.id });
          } catch (e) {
            allResults2.push({ clinicId: config.clinicId, status: "failed", error: e.message });
          }
        }
        return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ total: allResults2.length, success: allResults2.filter((r) => r.status === "success").length, failed: allResults2.filter((r) => r.status === "failed").length, results: allResults2 }) };
      }
    ...
    if (apiGwEvent.path.endsWith("/post") && apiGwEvent.httpMethod === "DELETE") {
      ...
      const dbRes = await ddb.send(new import_lib_dynamodb.GetCommand({
    ...
    if (apiGwEvent.path.endsWith("/history") && apiGwEvent.httpMethod === "GET") {
      ...
      const history = await ayrshareGetHistory(apiKey, dbRes.Item.ayrshareProfileKey, {
        lastRecords: lastRecords ? parseInt(lastRecords) : void 0,
        lastDays: lastDays ? parseInt(lastDays) : void 0
      });
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          recommendations: {
            ...
            maxClinicsPerMinute: Math.floor(6e4 / DELAY_BETWEEN_BATCHES_MS) * BATCH_SIZE,
    ...
    if (apiGwEvent.path.endsWith("/scheduled") && apiGwEvent.httpMethod === "GET") {
      ...
      try {
        let posts = [];
        if (clinicId) {
          const queryRes = await ddb.send(new import_lib_dynamodb.QueryCommand({
            TableName: POSTS_TABLE,
            IndexName: "ByClinic",
          ...
          const scanRes = await ddb.send(new import_lib_dynamodb.ScanCommand({
            TableName: POSTS_TABLE,
            FilterExpression: "attribute_exists(scheduledDate) AND scheduledDate <> :null",
            ExpressionAttributeValues: {
              ":null": null
    ...
    if (apiGwEvent.path.endsWith("/moderate") && apiGwEvent.httpMethod === "POST") {
      ...
      try {
        ...
        const first = Array.isArray(result?.moderation) ? result.moderation[0] : void 0;
        const flagged = Boolean(first?.flagged ?? result?.flagged ?? false);
        const categories = first?.categories ?? result?.categories ?? {};
        const categoryScores = first?.categoryScores ?? result?.categoryScores;
        return {
          statusCode: 200,
          headers: corsHeaders,
          body: JSON.stringify({
    ...
    if (apiGwEvent.path.endsWith("/auto-schedule")) {
      const clinicId = apiGwEvent.httpMethod === "GET" ? apiGwEvent.queryStringParameters?.clinicId : JSON.parse(apiGwEvent.body || "{}").clinicId;
      if (!clinicId) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ error: "clinicId required" }) };
      }
      const dbRes = await ddb.send(new import_lib_dynamodb.GetCommand({ TableName: TABLE_NAME, Key: { clinicId } }));
      if (!dbRes.Item?.ayrshareProfileKey) {
        return { statusCode: 404, headers: corsHeaders, body: JSON.stringify({ error: "Clinic not found" }) };
      }
      const apiKey = await getApiKey();
    ...
    if (apiGwEvent.path.endsWith("/webhooks")) {
      ...
      if (apiGwEvent.httpMethod === "POST") {
        const body = JSON.parse(apiGwEvent.body || "{}");
        const { action, url: url2, secret, profileKey } = body;
        if (!action || !url2 || !secret) {
          return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ error: "action, url, and secret required" }) };
        }
        try {
          const result = await ayrshareRegisterWebhook(apiKey, action, url2, secret, profileKey);
          return { statusCode: 200, headers: corsHeaders, body: JSON.stringify(result) };
        } catch (error) {
          return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ error: error.message }) };
        }
    ...
    if (apiGwEvent.path.endsWith("/media/verify") && apiGwEvent.httpMethod === "POST") {
      ...
      try {
        const apiKey = await getApiKey();
        const result = await ayrshareVerifyMediaUrl(apiKey, url2);
        return { statusCode: 200, headers: corsHeaders, body: JSON.stringify(result) };
      } catch (error) {
        return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ error: error.message }) };
      }
