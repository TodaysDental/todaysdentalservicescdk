...
// src/services/chime/resume-call.ts
var resume_call_exports = {};
__export(resume_call_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(resume_call_exports);
...
function calculateTTL(durationSeconds) {
  ...
  try {
    ...
    try {
      ...
      if (callRecord.holdStartTime) {
        ...
        holdDuration = Math.floor((now - holdStart) / 1e3);
      }
      ...
      try {
        ...
        if (meetingId) {
          try {
            const attendeeResponse = await chimeClient.send(new import_client_chime_sdk_meetings2.CreateAttendeeCommand({
              MeetingId: meetingId,
        ...
        await chimeVoice2.send(new import_client_chime_sdk_voice2.UpdateSipMediaApplicationCallCommand({
          ...
          Arguments: {
            ...
            reconnectAgent: "true"
            // Convert to string as API expects string values
          }
        }));
        ...
        try {
            ...
            TransactItems: [
              {
                Update: {
                  TableName: CALL_QUEUE_TABLE_NAME2,
                  Key: { clinicId, queuePosition },
                  UpdateExpression: "SET #status = :status, callStatus = :status, holdEndTime = :endTime, holdDuration = :duration, #ttl = :ttl, resumeTime = :timestamp, lastUpdated = :timestamp REMOVE holdStartTime, heldByAgentId",
              ...
              },
              {
                Update: {
                  TableName: AGENT_PRESENCE_TABLE_NAME2,
                  Key: { agentId },
                  UpdateExpression: "SET callStatus = :status, lastActivityAt = :timestamp, currentMeetingAttendeeId = :attendeeId, currentCallId = :callId, #agentStatus = :onCall REMOVE heldCallMeetingId, heldCallId, heldCallAttendeeId",
          ...
          if (txnErr.name === "TransactionCanceledException") {
            ...
            await scheduleCompensatingAction({
              ...
              attendeeId: agentAttendeeId || void 0,
