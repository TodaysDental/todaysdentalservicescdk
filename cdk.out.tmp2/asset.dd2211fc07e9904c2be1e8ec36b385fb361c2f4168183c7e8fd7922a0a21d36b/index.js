...
// src/services/clinic/schedules.ts
...
var getCorsHeaders = (event) => buildCorsHeaders({ allowHeaders: ["x-api-key"] }, event.headers?.origin);
var getUserPermissions = (event) => {
...
var isAdminUser = (clinicRoles, isSuperAdmin, isGlobalSuperAdmin) => {
...
var hasModulePermission = (clinicRoles, permission, isSuperAdmin, isGlobalSuperAdmin, clinicId) => {
  ...
  for (const cr of clinicRoles) {
    ...
    const moduleAccess = cr.moduleAccess?.find((ma) => ma.module === "Operations");
...
var normalizeSchedulePayload = (payload, { isCreate = false } = {}) => {
  ...
  if (!Array.isArray(normalizedClinicIds) || normalizedClinicIds.length === 0) {
    if (input.clinicId)
      normalizedClinicIds = [input.clinicId];
  }
  const schedule = {
    id,
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path === "/schedules/{id}") {
      ...
      if (method === "GET")
        return await handleGet(id, event);
      if (method === "PUT")
        return await handleUpdate(id, event.body || "", event);
      if (method === "DELETE")
        return await handleDelete(id, event);
    }
...
async function handleGet(id, event) {
  if (!id)
    return json(400, { message: "Missing id" }, event);
...
async function handleCreate(body, event) {
...
async function handleUpdate(id, body, event) {
  if (!id)
    return json(400, { message: "Missing id" }, event);
  const payload = safeParseJson(body);
  const userPerms = getUserPermissions(event);
...
async function handleBatchDelete(body, event) {
  const payload = safeParseJson(body);
  const ids = Array.isArray(payload.scheduleIds) ? payload.scheduleIds : [];
  if (ids.length === 0)
    return json(400, { message: "No scheduleIds provided" }, event);
  for (const id of ids) {
    if (!id)
      continue;
    await docClient.send(new import_lib_dynamodb.DeleteCommand({ TableName: TABLE_NAME, Key: { id } }));
