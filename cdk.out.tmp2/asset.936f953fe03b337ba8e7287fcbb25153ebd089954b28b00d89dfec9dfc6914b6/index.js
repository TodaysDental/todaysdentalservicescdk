...
// src/services/query-generator/query-generator.ts
...
var CONFIG = {
  ...
  MAX_TOKENS: 4096,
  TEMPERATURE: 0.2
  // Lower temperature for more deterministic SQL output
};
...
function generateCompactSchema(tables) {
  const priorityTables = /* @__PURE__ */ new Set([
    "appointment",
    "patient",
    "procedurelog",
    "claimproc",
    ...
    if (priorityTables.has(table.name.toLowerCase())) {
      ...
      for (const col of table.columns) {
        ...
        if (col.enumeration && col.enumeration.values.length > 0) {
          const enumVals = col.enumeration.values.slice(0, 5).map((v) => `${v.value}=${v.name}`).join(", ");
  ...
  const otherTables = tables.filter((t) => !priorityTables.has(t.name.toLowerCase())).map((t) => t.name);
  for (let i = 0; i < otherTables.length; i += 8) {
    lines.push(otherTables.slice(i, i + 8).join(", "));
  }
  return lines.join("\n");
}
function findRelevantTables(query, tables) {
  ...
  const neededTableNames = /* @__PURE__ */ new Set();
...
function generateDetailedSchema(tables) {
  const lines = [];
  for (const table of tables) {
...
var SYSTEM_PROMPT = `You are an expert MySQL query generator specializing in OpenDental dental practice management software.
...
var EXAMPLE_QUERIES = [
  ...
  {
    request: `Active patients with scheduled or completed appointments described like "%Prophy%"\r
within a date range, with a specifically named employer (of guarantor).\r
...
var handler = async (event) => {
  ...
  try {
    ...
    const { query, includeExamples = true } = body;
    ...
    if (includeExamples && EXAMPLE_QUERIES.length > 0) {
      ...
      const relevantExamples = EXAMPLE_QUERIES.filter((ex) => {
        ...
        return queryLower.includes("production") && reqLower.includes("production") || queryLower.includes("collection") && reqLower.includes("collection") || queryLower.includes("payment") && reqLower.includes("payment") || queryLower.includes("patient") && reqLower.includes("patient") || queryLower.includes("provider") && reqLower.includes("provider") || queryLower.includes("appointment") && reqLower.includes("appointment") || queryLower.includes("claim") && reqLower.includes("claim") || queryLower.includes("insurance") && reqLower.includes("insurance") || queryLower.includes("aging") && reqLower.includes("aging") || queryLower.includes("referral") && reqLower.includes("referral");
      }).slice(0, 2);
...
${detailedSchema}
    ...
    generatedQuery = generatedQuery.replace(/^```sql\s*/i, "").replace(/^```\s*/i, "").replace(/\s*```$/i, "").trim();
