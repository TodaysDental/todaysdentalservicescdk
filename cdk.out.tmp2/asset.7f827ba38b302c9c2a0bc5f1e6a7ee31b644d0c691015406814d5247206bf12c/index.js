...
// src/services/marketing/validate.ts
...
async function ayrshareValidatePost(apiKey, content, platforms, mediaUrls) {
  try {
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/validate/post`,
...
var PLATFORM_MEDIA_REQUIREMENTS = {
  instagram: {
        ...
        if (ayrshareResult && ayrshareResult.platformResults) {
          for (const [platform, result] of Object.entries(ayrshareResult.platformResults)) {
            if (platformResults[platform]) {
              platformResults[platform] = { ...platformResults[platform], ...result };
            }
          }
        }
      } catch (err) {
