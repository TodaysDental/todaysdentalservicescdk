...
// src/services/ai-agents/conversation-history.ts
var conversation_history_exports = {};
__export(conversation_history_exports, {
  CONVERSATIONS_TABLE: () => CONVERSATIONS_TABLE,
  createDocClient: () => createDocClient,
  handler: () => handler,
  logConversationMessage: () => logConversationMessage
});
module.exports = __toCommonJS(conversation_history_exports);
...
async function handleListConversations(event, userPerms, corsHeaders) {
  const {
    clinicId,
    agentId,
    userId,
  ...
  try {
    ...
    if (clinicId) {
      ...
      const isPublicFilter = isPublic !== void 0 ? isPublic === "true" : void 0;
...
async function handleGetConversation(sessionId, event, userPerms, corsHeaders) {
  try {
    ...
    const responseTimes = messages.filter((m) => m.messageType === "assistant" && m.responseTimeMs).map((m) => m.responseTimeMs);
    const avgResponseTime = responseTimes.length > 0 ? Math.round(responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length) : void 0;
    const totalTokens = messages.filter((m) => m.tokenCount).reduce((sum, m) => sum + (m.tokenCount || 0), 0);
    const conversation = {
      sessionId,
...
async function handleGetStats(event, userPerms, corsHeaders) {
  ...
  try {
    ...
    if (startDate || endDate) {
      ...
      const end = endDate ? (/* @__PURE__ */ new Date(endDate + "T23:59:59")).getTime() : Date.now();
    ...
    do {
      ...
      const result = await docClient.send(new import_lib_dynamodb.ScanCommand(scanParams));
    ...
    const topAgents = Object.entries(sessionsByAgent).map(([agentId2, data]) => ({
      agentId: agentId2,
      name: data.name,
      sessions: data.count,
      messages: allMessages.filter((m) => m.agentId === agentId2).length
    ...
    const stats = {
      ...
      avgMessagesPerSession: sessionMap.size > 0 ? Math.round(allMessages.length / sessionMap.size * 10) / 10 : 0,
      avgSessionDuration: sessionMap.size > 0 ? Math.round(totalDuration / sessionMap.size) : 0,
...
async function handleDeleteConversation(sessionId, event, userPerms, corsHeaders) {
...
async function queryByClinic(clinicId, startDate, endDate, limit, nextToken) {
  const start = startDate ? new Date(startDate).getTime() : 0;
  const end = endDate ? (/* @__PURE__ */ new Date(endDate + "T23:59:59")).getTime() : Date.now();
  const result = await docClient.send(new import_lib_dynamodb.QueryCommand({
    TableName: CONVERSATIONS_TABLE,
  ...
  return {
    sessions,
    nextToken: result.LastEvaluatedKey ? Buffer.from(JSON.stringify(result.LastEvaluatedKey)).toString("base64") : void 0
  };
}
async function queryByAgent(agentId, startDate, endDate, limit, nextToken) {
  const start = startDate ? new Date(startDate).getTime() : 0;
  const end = endDate ? (/* @__PURE__ */ new Date(endDate + "T23:59:59")).getTime() : Date.now();
  const result = await docClient.send(new import_lib_dynamodb.QueryCommand({
    TableName: CONVERSATIONS_TABLE,
...
async function queryByUser(userId, startDate, endDate, limit, nextToken) {
  const start = startDate ? new Date(startDate).getTime() : 0;
  const end = endDate ? (/* @__PURE__ */ new Date(endDate + "T23:59:59")).getTime() : Date.now();
  const result = await docClient.send(new import_lib_dynamodb.QueryCommand({
    TableName: CONVERSATIONS_TABLE,
...
async function scanConversations(authorizedClinics, startDate, endDate, channel, isPublic, limit, nextToken) {
  const filterParts = [];
  const exprAttrValues = {};
  const exprAttrNames = {};
  ...
  const scanParams = {
    TableName: CONVERSATIONS_TABLE,
    Limit: (limit || 50) * 10
    // Get more items to aggregate into sessions
  };
  if (filterParts.length > 0) {
    scanParams.FilterExpression = filterParts.join(" AND ");
    scanParams.ExpressionAttributeValues = exprAttrValues;
...
function aggregateToSessions(messages) {
  const sessionMap = /* @__PURE__ */ new Map();
  ...
  sessionMap.forEach((msgs, sessionId) => {
    ...
    const responseTimes = assistantMsgs.filter((m) => m.responseTimeMs).map((m) => m.responseTimeMs);
    sessions.push({
      sessionId,
      ...
      avgResponseTimeMs: responseTimes.length > 0 ? Math.round(responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length) : void 0,
      totalTokens: sorted.reduce((sum, m) => sum + (m.tokenCount || 0), 0) || void 0
    });
  });
...
function getAuthorizedClinics(userPerms) {
  if (isAdminUser(userPerms.clinicRoles, userPerms.isSuperAdmin, userPerms.isGlobalSuperAdmin)) {
    return [];
  }
  const authorizedClinics = [];
...
async function logConversationMessage(message) {
...
function createDocClient() {
  return import_lib_dynamodb.DynamoDBDocumentClient.from(new import_client_dynamodb2.DynamoDBClient({}));
}
