...
// src/services/credentialing/verification-handler.ts
var verification_handler_exports = {};
__export(verification_handler_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(verification_handler_exports);
...
async function verifyStateLicense(state, licenseNumber, licenseType = "dental") {
  const checkedAt = (/* @__PURE__ */ new Date()).toISOString();
  const stateUpper = state.toUpperCase();
  const statesWithAPIs = {
...
async function runComprehensiveVerification(providerId, providerData, userId) {
...
var handler = async (event) => {
  ...
  const methodPermission = event.httpMethod === "GET" ? "read" : "write";
  ...
  try {
    ...
    if (method === "POST" && path === "/verify/comprehensive") {
      ...
      const result = await runComprehensiveVerification(
        providerId || `temp-${v4_default().slice(0, 8)}`,
        providerData,
        userPerms.email
      );
      return httpOk({ verification: result });
