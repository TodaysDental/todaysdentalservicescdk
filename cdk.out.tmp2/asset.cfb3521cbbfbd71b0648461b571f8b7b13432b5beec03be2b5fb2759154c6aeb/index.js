...
// src/services/push-notifications/unregister-device.ts
var unregister_device_exports = {};
__export(unregister_device_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(unregister_device_exports);
...
async function handleUnregisterByToken(event, userPerms) {
...
async function handleUnregisterById(event, userPerms, bodyDeviceId) {
...
async function handleUnregisterAll(event, userPerms) {
  const userId = userPerms.email || "unknown";
  try {
...
async function handleSmartUnregister(event, userPerms) {
  const body = parseBody(event.body);
