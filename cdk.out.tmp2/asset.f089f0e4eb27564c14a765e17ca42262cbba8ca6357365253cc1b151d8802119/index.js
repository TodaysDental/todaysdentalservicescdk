...
// src/services/chime/agent-inactive.ts
var agent_inactive_exports = {};
__export(agent_inactive_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(agent_inactive_exports);
...
async function deleteAgentClinicRow(agentId, clinicId) {
...
async function listClinicsForAgent(agentId) {
  if (!AGENT_ACTIVE_TABLE_NAME)
    return [];
  const { Items } = await ddb.send(new import_lib_dynamodb2.QueryCommand({
    TableName: AGENT_ACTIVE_TABLE_NAME,
  ...
  const clinicIds = (Items || []).map((i) => i?.clinicId).filter((v) => typeof v === "string" && v.length > 0);
...
var handler = async (event) => {
  ...
  const cleanedClinicIds = Array.isArray(clinicIds) ? clinicIds.map((c) => String(c || "").trim()).filter((c) => c.length > 0) : null;
  const clinicsToRemove = cleanedClinicIds && cleanedClinicIds.length > 0 ? cleanedClinicIds : await listClinicsForAgent(agentId);
  if (clinicsToRemove.length === 0) {
    return http(200, { message: "Agent already inactive", agentId, removed: 0 }, event);
  }
  const results = await Promise.allSettled(
    clinicsToRemove.map((clinicId) => deleteAgentClinicRow(agentId, clinicId))
  );
