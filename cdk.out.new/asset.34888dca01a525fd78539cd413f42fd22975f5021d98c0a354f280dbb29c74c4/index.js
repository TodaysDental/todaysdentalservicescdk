...
var handler = async (event) => {
  ...
  try {
    if (!CONNECT_INSTANCE_ARN) return httpErr(500, "connect not configured");
    const authz = event.headers?.authorization || event.headers?.Authorization || "";
    const verifyResult = await verifyIdToken(authz);
    if (!verifyResult.ok) return httpErr(verifyResult.code, verifyResult.message);
    const instanceId = arnTail(CONNECT_INSTANCE_ARN);
    const route = event.resource || "";
    const method = event.httpMethod;
    const body = parseBody(event.body);
    if (route.endsWith("/connect/contact/attributes") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      const attributes = body.attributes && typeof body.attributes === "object" ? body.attributes : {};
      if (!contactId) return httpErr(400, "contactId is required");
      await connectClient.send(new import_client_connect.UpdateContactAttributesCommand({ InstanceId: instanceId, InitialContactId: contactId, Attributes: attributes }));
      return httpOk({ updated: true });
    }
    if (route.endsWith("/connect/contact/transfer") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      const clinicId = String(body.clinicId || "").trim();
      const queueArnOverride = String(body.queueArn || "").trim();
      if (!contactId) return httpErr(400, "contactId is required");
      const queueArn = queueArnOverride || (clinicId ? CLINIC_QUEUE_ARN_MAP[clinicId] : "");
      if (!queueArn) return httpErr(400, "queueArn or clinicId must be provided");
      const queueId = arnTail(queueArn);
      await connectClient.send(new import_client_connect.TransferContactCommand({ InstanceId: instanceId, ContactId: contactId, QueueId: queueId }));
      return httpOk({ transferred: true });
    }
    if (route.endsWith("/connect/contact/recording/start") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) return httpErr(400, "contactId is required");
      await connectClient.send(new import_client_connect.StartContactRecordingCommand({
        InstanceId: instanceId,
        ContactId: contactId,
        VoiceRecordingConfiguration: { VoiceRecordingTrack: "FROM_AGENT" }
      }));
      return httpOk({ recording: "started" });
    }
    if (route.endsWith("/connect/contact/recording/stop") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) return httpErr(400, "contactId is required");
      await connectClient.send(new import_client_connect.StopContactRecordingCommand({ InstanceId: instanceId, ContactId: contactId }));
      return httpOk({ recording: "stopped" });
    }
