...
var handler = async (event) => {
  ...
  try {
    ...
    if (event.resource?.endsWith("/connect/me") && event.httpMethod === "GET") {
      ...
      const agentStatus = user?.AgentStatus?.Name || "Unknown";
      const contacts = (user?.Contacts || []).map((c) => ({
        ...
        queue: c.Queue,
        customerEndpoint: c.CustomerEndpoint
      }));
      return httpOk({ agentStatus, contacts });
    }
    if (event.resource?.endsWith("/connect/stop") && event.httpMethod === "POST") {
      const body = parseBody(event.body);
      const contactId = String(body.contactId || "").trim();
      if (!contactId) {
        return httpErr(400, "contactId is required");
      }
      await connectClient.send(new import_client_connect.StopContactCommand({ InstanceId: instanceId, ContactId: contactId }));
      return httpOk({ stopped: true });
    }
