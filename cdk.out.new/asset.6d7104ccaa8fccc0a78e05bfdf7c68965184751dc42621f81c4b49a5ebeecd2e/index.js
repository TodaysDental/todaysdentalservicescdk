...
var CONNECT = loadAwsConnect();
var connectClient = CONNECT.ConnectClient ? new CONNECT.ConnectClient({}) : void 0;
var handler = async (event) => {
  ...
  try {
    ...
    if (isUsersCollectionGet) {
      ...
      const listResp = await cognito.send(new import_client_cognito_identity_provider.ListUsersCommand({ UserPoolId: userPoolId, Limit: limit, PaginationToken: paginationToken }));
      const users = listResp.Users || [];
      const allowedClinics = caller.isSuperAdmin ? void 0 : new Set(Object.entries(caller.rolesByClinic).filter(([, code]) => code === "S" || code === "A").map(([cid]) => cid));
      const items = [];
      for (const u of users) {
        const username = String(u.Username || "").toLowerCase();
        const attrs = Object.fromEntries((u.Attributes || []).map((a) => [a.Name, a.Value]));
        const groupsResp = await cognito.send(new import_client_cognito_identity_provider.AdminListGroupsForUserCommand({ UserPoolId: userPoolId, Username: username }));
        let groupNames = (groupsResp.Groups || []).map((g) => g.GroupName).filter(Boolean);
        if (allowedClinics) {
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ items, nextToken: listResp?.PaginationToken || void 0 }) };
    }
    if (event.httpMethod === "GET") {
      if (!pathUsername) return httpErr(400, "username path parameter is required");
      const info = await cognito.send(new import_client_cognito_identity_provider.AdminGetUserCommand({ UserPoolId: userPoolId, Username: pathUsername }));
      const attrs = Object.fromEntries((info.UserAttributes || []).map((a) => [a.Name, a.Value]));
      const groupsResp = await cognito.send(new import_client_cognito_identity_provider.AdminListGroupsForUserCommand({ UserPoolId: userPoolId, Username: pathUsername }));
      const groupNames = (groupsResp.Groups || []).map((g) => g.GroupName).filter(Boolean);
      const { clinics, rolesByClinic, isSuperAdmin } = deriveClinicsFromGroups(groupNames);
      return httpOk({
    ...
    if (event.httpMethod === "PUT") {
      ...
      if (attribs.length > 0) {
        await cognito.send(new import_client_cognito_identity_provider.AdminUpdateUserAttributesCommand({ UserPoolId: userPoolId, Username: pathUsername, UserAttributes: attribs }));
      }
      ...
      const currentGroupsResp = await cognito.send(new import_client_cognito_identity_provider.AdminListGroupsForUserCommand({ UserPoolId: userPoolId, Username: pathUsername }));
      const currentGroups = (currentGroupsResp.Groups || []).map((g) => g.GroupName).filter(Boolean);
      const managed = /* @__PURE__ */ new Set(["GLOBAL__SUPER_ADMIN", ...currentGroups.filter((g) => g.startsWith("clinic_"))]);
      ...
      for (const g of toRemove) {
        await cognito.send(new import_client_cognito_identity_provider.AdminRemoveUserFromGroupCommand({ UserPoolId: userPoolId, Username: pathUsername, GroupName: g }));
      }
      for (const g of toAdd) {
        await cognito.send(new import_client_cognito_identity_provider.AdminAddUserToGroupCommand({ UserPoolId: userPoolId, Username: pathUsername, GroupName: g }));
      }
      const provisionResult = await provisionOrUpdateConnectUser({
        email: pathUsername,
      ...
      return httpOk({ username: pathUsername, groupsAssigned: desiredGroups, connect: provisionResult });
    }
    if (event.httpMethod === "DELETE") {
      await cognito.send(new import_client_cognito_identity_provider.AdminDeleteUserCommand({ UserPoolId: userPoolId, Username: pathUsername }));
      const deletedConnect = await deleteConnectUserIfExists(pathUsername);
      return httpOk({ username: pathUsername, deleted: true, connect: deletedConnect });
    }
...
function validatePutBody(body) {
  ...
  const allowedRoles = /* @__PURE__ */ new Set(["SUPER_ADMIN", "ADMIN", "PROVIDER", "MARKETING", "USER"]);
  const clinics = Array.isArray(body.clinics) ? body.clinics : [];
...
function buildGroupNames(clinics) {
...
function httpErr(code, message2) {
  return { statusCode: code, headers: buildCorsHeaders({ allowMethods: ["OPTIONS", "GET", "PUT", "DELETE"] }), body: JSON.stringify({ success: false, message: message2 }) };
}
...
function callerAuthContextFromClaims(payload) {
  const ctx = { isSuperAdmin: false, rolesByClinic: {} };
  if (String(payload["x_is_super_admin"]).toLowerCase() === "true" || String(payload["x_clinics"]).toUpperCase() === "ALL") {
    ctx.isSuperAdmin = true;
    return ctx;
  }
  const rbc = String(payload["x_rbc"] || "").trim();
  if (rbc) {
...
function hasAnyAdmin(rolesByClinic) {
  return Object.values(rolesByClinic).some((code) => code === "S" || code === "A");
}
function canAssignAll(requestedClinics, caller) {
  const errors = [];
...
function deriveClinicsFromGroups(groupNames) {
  const rolesByClinic = {};
  ...
  for (const g of groupNames || []) {
    ...
    const order = { S: 4, A: 3, P: 2, M: 1, U: 0 };
    if (existing === void 0 || order[code] > (order[existing] ?? -1)) {
...
var CONNECT_ROUTING_PROFILE_ID = process.env.CONNECT_ROUTING_PROFILE_ID || "";
var CONNECT_ROUTING_PROFILE_MAP_JSON = process.env.CONNECT_ROUTING_PROFILE_MAP || "";
var CONNECT_SECURITY_PROFILE_MAP_JSON = process.env.CONNECT_SECURITY_PROFILE_MAP || "";
var CONNECT_ALLOW_CREATE = (process.env.CONNECT_ALLOW_CREATE || "false").toLowerCase() === "true";
var CONNECT_TEMP_PASSWORD = process.env.CONNECT_TEMP_PASSWORD || void 0;
async function provisionOrUpdateConnectUser(args) {
  if (!CONNECT_INSTANCE_ARN) {
    return { enabled: false, reason: "CONNECT_INSTANCE_ARN not set" };
  }
  const securityProfileMap = safeParseJson(CONNECT_SECURITY_PROFILE_MAP_JSON) || {};
  const groupKeys = determineGroupKeys(args);
  const securityProfileIds = resolveSecurityProfiles(securityProfileMap, groupKeys);
  if (securityProfileIds.length === 0) {
    return { enabled: true, changed: false, reason: "no security profile mapping for any role/group" };
  }
  if (!CONNECT_ROUTING_PROFILE_ID) {
    return { enabled: true, changed: false, reason: "CONNECT_ROUTING_PROFILE_ID not set" };
  }
  if (!connectClient || !CONNECT.ListUsersCommand) {
    return { enabled: false, reason: "aws connect sdk not available" };
  }
  const existing = await findConnectUserByUsername(CONNECT_INSTANCE_ARN, args.email);
  if (existing) {
    const userId = existing.Id || existing.id || existing.UserId || existing.userId;
    if (!userId) return { enabled: true, changed: false, reason: "found user without id" };
    await connectClient.send(new CONNECT.UpdateUserSecurityProfilesCommand({ InstanceId: CONNECT_INSTANCE_ARN, UserId: userId, SecurityProfileIds: securityProfileIds }));
    const routingProfileId2 = resolveRoutingProfileId(args);
    await connectClient.send(new CONNECT.UpdateUserRoutingProfileCommand({ InstanceId: CONNECT_INSTANCE_ARN, UserId: userId, RoutingProfileId: routingProfileId2 }));
    if (args.givenName || args.familyName) {
      await connectClient.send(new CONNECT.UpdateUserIdentityInfoCommand({ InstanceId: CONNECT_INSTANCE_ARN, UserId: userId, IdentityInfo: { FirstName: args.givenName, LastName: args.familyName, Email: args.email } }));
    }
    return { enabled: true, changed: true, action: "updated", userId };
  }
  if (!CONNECT_ALLOW_CREATE) {
    return { enabled: true, changed: false, reason: "user not found and creation disabled" };
  }
  if (!CONNECT_TEMP_PASSWORD) {
    return { enabled: true, changed: false, reason: "CONNECT_TEMP_PASSWORD not set for create" };
  }
  const routingProfileId = resolveRoutingProfileId(args);
  const createResp = await connectClient.send(new CONNECT.CreateUserCommand({
    InstanceId: CONNECT_INSTANCE_ARN,
    Username: args.email,
    Password: CONNECT_TEMP_PASSWORD,
    IdentityInfo: { FirstName: args.givenName || args.email.split("@")[0], LastName: args.familyName || "", Email: args.email },
    PhoneConfig: { PhoneType: "SOFT_PHONE", AutoAccept: true, AfterContactWorkTimeLimit: 0 },
    RoutingProfileId: routingProfileId,
    SecurityProfileIds: securityProfileIds,
    Tags: { provisionedBy: "admin-users-api" }
  }));
  return { enabled: true, changed: true, action: "created", userId: createResp?.UserId };
}
async function deleteConnectUserIfExists(email) {
  if (!CONNECT_INSTANCE_ARN || !connectClient || !CONNECT.ListUsersCommand || !CONNECT.DeleteUserCommand) {
    return { enabled: false };
  }
  const existing = await findConnectUserByUsername(CONNECT_INSTANCE_ARN, email);
  if (!existing) return { enabled: true, deleted: false };
  const userId = existing.Id || existing.id || existing.UserId || existing.userId;
  if (!userId) return { enabled: true, deleted: false };
  await connectClient.send(new CONNECT.DeleteUserCommand({ InstanceId: CONNECT_INSTANCE_ARN, UserId: userId }));
  return { enabled: true, deleted: true, userId };
}
function determineGroupKeys(args) {
  const keys = [];
  if (args.makeGlobalSuperAdmin) {
    keys.push("GLOBAL__SUPER_ADMIN");
    keys.push("SUPER_ADMIN");
  }
  for (const c of args.clinics || []) {
    const clinicId = String(c.clinicId);
    const role = String(c.role || "USER").toUpperCase();
    keys.push(role);
    keys.push(`clinic_${clinicId}__${role}`);
  }
  if (keys.length === 0) keys.push("USER");
  return keys;
}
function resolveSecurityProfiles(map, keys) {
  ...
  if (ids.size === 0 && map["USER"]) {
    const base = map["USER"];
    if (Array.isArray(base)) base.forEach((id) => ids.add(String(id)));
    else ids.add(String(base));
  }
  return Array.from(ids);
}
function resolveRoutingProfileId(args) {
  const map = safeParseJson(CONNECT_ROUTING_PROFILE_MAP_JSON) || {};
  for (const c of args.clinics || []) {
    const key = `clinic_${String(c.clinicId)}`;
    if (map[key]) return routingIdFromArnOrId(String(map[key]));
  }
  const roleOrder = ["SUPER_ADMIN", "ADMIN", "PROVIDER", "MARKETING", "USER"];
  const roles = determineRoles(args);
  roles.sort((a, b) => roleOrder.indexOf(b) - roleOrder.indexOf(a));
  for (const r of roles) {
    if (map[r]) return routingIdFromArnOrId(String(map[r]));
  }
  return routingIdFromArnOrId(CONNECT_ROUTING_PROFILE_ID);
}
function determineRoles(args) {
  const roles = /* @__PURE__ */ new Set();
  for (const c of args.clinics || []) {
    const r = String(c.role || "USER").toUpperCase();
    if (r) roles.add(r);
  }
  if (args.makeGlobalSuperAdmin) roles.add("SUPER_ADMIN");
  if (roles.size === 0) roles.add("USER");
  return Array.from(roles);
}
async function findConnectUserByUsername(instanceId, username) {
  let nextToken = void 0;
  do {
    const resp = await connectClient.send(new CONNECT.ListUsersCommand({ InstanceId: instanceId, NextToken: nextToken }));
    const arr = (resp.UserSummaryList || []).filter((u) => String(u.Username || u.username).toLowerCase() === username.toLowerCase());
    if (arr.length > 0) return arr[0];
    nextToken = resp.NextToken;
  } while (nextToken);
  return void 0;
}
function routingIdFromArnOrId(value) {
  const v = String(value || "");
  if (v.includes(":routing-profile/")) {
    const parts = v.split("/");
