...
var handler = async (event) => {
  ...
  try {
    if (!CONNECT_INSTANCE_ARN) {
      return httpErr(500, "connect not configured");
    }
    ...
    const userEmail = String(verifyResult.payload.email || "").trim();
    if (!userEmail) {
      return httpErr(400, "email claim missing");
    }
    const userId = await getConnectUserIdByEmail(instanceId, userEmail);
    if (!userId) {
      return httpErr(404, "connect user not found");
    }
    if (route.endsWith("/connect/agent/states") && method === "GET") {
      const agentStatuses = await connectClient.send(new import_client_connect.ListAgentStatusesCommand({
        InstanceId: instanceId,
        MaxResults: 100
      }));
      const states = (agentStatuses.AgentStatusSummaryList || []).map((status) => ({
        id: status.Id,
        name: status.Name,
        description: status.Name,
        // AgentStatusSummary doesn't have Description field
        type: status.Type
      }));
      return httpOk({ states });
    }
    if (route.endsWith("/connect/agent/status") && method === "POST") {
      const agentStatusId = String(body.agentStatusId || "").trim();
      if (!agentStatusId) {
        return httpErr(400, "agentStatusId is required");
      }
      await connectClient.send(new import_client_connect.PutUserStatusCommand({
        InstanceId: instanceId,
        UserId: userId,
        AgentStatusId: agentStatusId
      }));
      return httpOk({ updated: true });
    }
    if (route.endsWith("/connect/agent/me") && method === "GET") {
      const userDetails = await connectClient.send(new import_client_connect.DescribeUserCommand({
        InstanceId: instanceId,
        UserId: userId
      }));
      ...
      return httpOk({
        ...
        routingProfile: {
          id: user?.RoutingProfileId,
          name: user?.RoutingProfileId
          // User doesn't have RoutingProfileName, using ID
        },
        directoryUserId: user?.DirectoryUserId
      });
    }
    if (route.endsWith("/connect/contact/accept") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) {
        return httpErr(400, "contactId is required");
      }
      return httpOk({
        accepted: true,
        note: "Accept operation tracked - actual answer typically handled by softphone/CCP"
      });
    }
    if (route.endsWith("/connect/contact/hold") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) {
        return httpErr(400, "contactId is required");
      }
      try {
        await connectClient.send(new import_client_connect.UpdateContactAttributesCommand({
          InstanceId: instanceId,
          InitialContactId: contactId,
          Attributes: { "AgentHoldState": "true" }
        }));
      } catch (err) {
        console.warn("Could not set hold attribute:", err.message);
      }
      return httpOk({
        held: true,
        note: "Hold state tracked via attributes - actual hold typically handled by softphone/CCP"
      });
    }
    if (route.endsWith("/connect/contact/resume") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) {
        return httpErr(400, "contactId is required");
      }
      try {
        await connectClient.send(new import_client_connect.UpdateContactAttributesCommand({
          InstanceId: instanceId,
          InitialContactId: contactId,
          Attributes: { "AgentHoldState": "false" }
        }));
      } catch (err) {
        console.warn("Could not set resume attribute:", err.message);
      }
      return httpOk({
        resumed: true,
        note: "Resume state tracked via attributes - actual resume typically handled by softphone/CCP"
      });
    }
    if (route.endsWith("/connect/contact/end") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) {
        return httpErr(400, "contactId is required");
      }
      await connectClient.send(new import_client_connect.StopContactCommand({
        InstanceId: instanceId,
        ContactId: contactId
      }));
      return httpOk({ ended: true });
    }
    if (route.endsWith("/connect/contact/mute") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) {
        return httpErr(400, "contactId is required");
      }
      try {
        await connectClient.send(new import_client_connect.UpdateContactAttributesCommand({
          InstanceId: instanceId,
          InitialContactId: contactId,
          Attributes: { "AgentMuted": "true" }
        }));
      } catch (err) {
        console.warn("Direct mute not supported via AWS API:", err.message);
      }
      return httpOk({
        muted: true,
        note: "Mute state tracked via attributes - actual audio muting typically handled by softphone"
      });
    }
    if (route.endsWith("/connect/contact/unmute") && method === "POST") {
      const contactId = String(body.contactId || "").trim();
      if (!contactId) {
        return httpErr(400, "contactId is required");
      }
      try {
        await connectClient.send(new import_client_connect.UpdateContactAttributesCommand({
          InstanceId: instanceId,
          InitialContactId: contactId,
          Attributes: { "AgentMuted": "false" }
        }));
      } catch (err) {
        console.warn("Direct unmute not supported via AWS API:", err.message);
      }
      return httpOk({
        unmuted: true,
        note: "Mute state tracked via attributes - actual audio unmuting typically handled by softphone"
      });
    }
...
function httpErr(code, message2) {
  return { statusCode: code, headers: corsHeaders, body: JSON.stringify({ success: false, message: message2 }) };
}
...
async function getConnectUserIdByEmail(instanceId, email) {
  try {
    const search = await connectClient.send(
      new import_client_connect.SearchUsersCommand({
        InstanceId: instanceId,
        SearchCriteria: {
          StringCondition: {
