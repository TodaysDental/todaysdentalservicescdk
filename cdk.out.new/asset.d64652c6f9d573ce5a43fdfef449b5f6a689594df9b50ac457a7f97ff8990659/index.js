...
var CONNECT_OUTBOUND_FLOW_ID = process.env.CONNECT_OUTBOUND_FLOW_ID || "";
var CLINIC_QUEUE_ARN_MAP_JSON = process.env.CLINIC_QUEUE_ARN_MAP || "{}";
var CLINIC_SOURCE_NUMBER_ARN_MAP_JSON = process.env.CLINIC_SOURCE_NUMBER_ARN_MAP || "{}";
var CLINIC_QUEUE_ARN_MAP = safeParseJson(CLINIC_QUEUE_ARN_MAP_JSON) || {};
var CLINIC_SOURCE_NUMBER_ARN_MAP = safeParseJson(CLINIC_SOURCE_NUMBER_ARN_MAP_JSON) || {};
...
var handler = async (event) => {
  ...
  try {
    if (!CONNECT_INSTANCE_ARN || !CONNECT_OUTBOUND_FLOW_ID) {
    ...
    if (!phone || !clinicId) {
      return httpErr(400, "phoneNumber and clinicId are required");
    }
    ...
    const canCall = callerHasClinicAccess(verifyResult.payload, clinicId);
    if (!canCall) {
      return httpErr(403, "forbidden: no access to clinic");
    }
    const queueArn = CLINIC_QUEUE_ARN_MAP[clinicId];
    const sourceNumberArn = CLINIC_SOURCE_NUMBER_ARN_MAP[clinicId];
    if (!queueArn || !sourceNumberArn) {
      return httpErr(400, "clinic routing not configured");
    }
    const instanceId = arnTail(CONNECT_INSTANCE_ARN);
    const queueId = arnTail(queueArn);
    const contactFlowId = await resolveContactFlowId(connectClient, instanceId);
    if (!contactFlowId) {
      return httpErr(500, "connect outbound flow not configured");
    }
    const cmd = new import_client_connect.StartOutboundVoiceContactCommand({
      InstanceId: instanceId,
      ContactFlowId: contactFlowId,
      DestinationPhoneNumber: phone,
      QueueId: queueId,
      SourcePhoneNumber: void 0,
      // use caller id from queue/flow; set to E.164 if you prefer
      AnswerMachineDetectionConfig: { EnableAnswerMachineDetection: false },
      Attributes: { clinicId }
    });
    const resp = await connectClient.send(cmd);
    return httpOk({ contactId: resp.ContactId });
  } catch (err) {
    return httpErr(500, err?.message || "failed to start outbound call");
  }
};
...
function callerHasClinicAccess(payload, clinicId) {
  ...
  if (xClinics) return xClinics.split(",").includes(clinicId);
  const xRbc = String(payload["x_rbc"] || "").trim();
  if (xRbc) return xRbc.split(",").some((pair) => pair.split(":")[0] === clinicId);
  const groups = Array.isArray(payload["cognito:groups"]) ? payload["cognito:groups"] : [];
  return groups.some((g) => new RegExp(`^clinic_${clinicId}__`).test(String(g)));
}
...
async function resolveContactFlowId(connectClient2, instanceId) {
  if (CONNECT_OUTBOUND_FLOW_ID) {
    return CONNECT_OUTBOUND_FLOW_ID;
  }
  try {
    const listed = await connectClient2.send(new import_client_connect.ListContactFlowsCommand({ InstanceId: instanceId }));
    const flows = listed.ContactFlowSummaryList || [];
    const preferred = flows.find((f) => /outbound|call|dial|sales|default/i.test(String(f.Name)));
    return preferred?.Id || flows[0]?.Id;
  } catch {
    return void 0;
  }
}
