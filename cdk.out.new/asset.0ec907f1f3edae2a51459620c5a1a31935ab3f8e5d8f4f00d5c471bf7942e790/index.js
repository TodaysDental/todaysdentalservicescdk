...
var handler = async (event) => {
  ...
  try {
    switch (routeKey) {
      case "$connect":
        return await handleConnect(event, connectionId, domain, stage);
      case "$disconnect":
        return await handleDisconnect(connectionId);
      case "subscribe":
        return await handleSubscribe(event, connectionId, domain, stage);
      case "unsubscribe":
...
async function handleConnect(event, connectionId, domain, stage) {
  console.log("WebSocket connection request:", connectionId);
  const token = event.queryStringParameters?.token || event.headers?.authorization?.replace("Bearer ", "") || event.headers?.Authorization?.replace("Bearer ", "");
  if (!token) {
    return { statusCode: 401, body: JSON.stringify({ message: "Missing authentication token" }) };
  }
  const verifyResult = await verifyIdToken(`Bearer ${token}`);
  if (!verifyResult.ok) {
    return { statusCode: 401, body: JSON.stringify({ message: "Invalid token" }) };
  }
  const userEmail = String(verifyResult.payload.email || "").trim();
  if (!userEmail) {
    return { statusCode: 400, body: JSON.stringify({ message: "Email claim missing from token" }) };
  }
  try {
    await dynamoClient.send(new import_client_dynamodb.PutItemCommand({
      ...
      Item: {
        ...
        stage: { S: stage },
        subscriptions: { SS: [] }
        // Set of subscription types
      }
    }));
    console.log(`Connection ${connectionId} stored for user ${userEmail}`);
    return { statusCode: 200, body: JSON.stringify({ message: "Connected" }) };
  } catch (err) {
    console.error("Failed to store connection:", err);
    return { statusCode: 500, body: JSON.stringify({ message: "Failed to establish connection" }) };
  }
}
...
async function handleSubscribe(event, connectionId, domain, stage) {
  const body = parseBody(event.body);
  const subscriptionType = body.type || "all";
  console.log(`Subscription request for ${connectionId}: ${subscriptionType}`);
  try {
    ...
    const currentSubscriptions = existingConnection.subscriptions || [];
    const updatedSubscriptions = [.../* @__PURE__ */ new Set([...currentSubscriptions, subscriptionType])];
    await dynamoClient.send(new import_client_dynamodb.PutItemCommand({
      TableName: CONNECTIONS_TABLE,
      Item: {
        ...existingConnection,
        subscriptions: { SS: updatedSubscriptions },
        lastSubscribed: { N: Date.now().toString() }
      }
    }));
    const apiGateway = new import_client_apigatewaymanagementapi.ApiGatewayManagementApiClient({
      endpoint: `https://${domain}/${stage}`
    });
    if (subscriptionType === "agent" || subscriptionType === "all") {
      await sendAgentData(apiGateway, connectionId, existingConnection.userEmail);
    }
    if (subscriptionType === "contacts" || subscriptionType === "all") {
      await sendContactData(apiGateway, connectionId, existingConnection.userEmail);
    }
    console.log(`Subscription ${subscriptionType} added for ${connectionId}`);
    return { statusCode: 200, body: JSON.stringify({ message: "Subscribed" }) };
  } catch (err) {
    console.error("Failed to handle subscription:", err);
    return { statusCode: 500, body: JSON.stringify({ message: "Failed to subscribe" }) };
  }
}
async function handleUnsubscribe(connectionId) {
  console.log(`Unsubscribe request for ${connectionId}`);
  try {
    const existingConnection = await getConnection(connectionId);
    if (!existingConnection) {
      return { statusCode: 404, body: JSON.stringify({ message: "Connection not found" }) };
    ...
    await dynamoClient.send(new import_client_dynamodb.PutItemCommand({
      ...
      Item: {
        ...existingConnection,
        subscriptions: { SS: [] },
        lastUnsubscribed: { N: Date.now().toString() }
      }
    }));
    return { statusCode: 200, body: JSON.stringify({ message: "Unsubscribed" }) };
  } catch (err) {
    console.error("Failed to unsubscribe:", err);
    return { statusCode: 500, body: JSON.stringify({ message: "Failed to unsubscribe" }) };
  }
}
async function getConnection(connectionId) {
  try {
    ...
    return {
      ...
      connectedAt: item.connectedAt?.N,
...
async function sendAgentData(apiGateway, connectionId, userEmail) {
  try {
    if (!CONNECT_INSTANCE_ARN) return;
    ...
    if (!userId) {
      await sendToConnection(apiGateway, connectionId, {
        type: "agent",
        data: { error: "Connect user not found" },
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
      });
      return;
    }
    const currentData = await connectClient.send(
    ...
    const agentStatus = user?.Status?.StatusName || "Unknown";
    await sendToConnection(apiGateway, connectionId, {
      type: "agent",
      data: { agentStatus, userId },
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    });
  } catch (err) {
    console.error("Failed to send agent data:", err);
  }
}
async function sendContactData(apiGateway, connectionId, userEmail) {
  try {
    if (!CONNECT_INSTANCE_ARN) return;
    const instanceId = arnTail(CONNECT_INSTANCE_ARN);
    const userId = await getConnectUserIdByEmail(instanceId, userEmail);
...
async function sendToConnection(apiGateway, connectionId, message2) {
  try {
