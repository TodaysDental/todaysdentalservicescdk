...
var handler = async (event) => {
  ...
  try {
    ...
    return httpOk({ username, groupsAssigned: groupNames, connect: provisionResult });
  } catch (err) {
...
async function ensureUserExists({ userPoolId, username, body }) {
  try {
    await cognito.send(new import_client_cognito_identity_provider.AdminGetUserCommand({ UserPoolId: userPoolId, Username: username }));
    return;
  } catch (_) {
    await cognito.send(
...
function httpErr(code, message2) {
  return { statusCode: code, headers: buildCorsHeaders({ allowMethods: ["OPTIONS", "POST"] }), body: JSON.stringify({ success: false, message: message2 }) };
}
...
var CONNECT_INSTANCE_ID = process.env.CONNECT_INSTANCE_ID || (() => {
  const m = /instance\/([a-zA-Z0-9-]+)/.exec(CONNECT_INSTANCE_ARN);
  return m ? m[1] : "";
})();
...
async function provisionOrUpdateConnectUser(args) {
  ...
  const identityType = await getIdentityManagementTypeSafe();
  const includeEmailInIdentityInfo = identityType === "CONNECT_MANAGED" || identityType === "DIRECTORY_SERVICE";
  const securityProfileMap = safeParseJson(CONNECT_SECURITY_PROFILE_MAP_JSON) || {};
  const defaultProfileIds = await getDefaultAgentAdminSecurityProfiles();
  if (defaultProfileIds.agentId || defaultProfileIds.adminId) {
    const ensure = (key, id) => {
      if (id && !securityProfileMap[key]) securityProfileMap[key] = id;
    };
    ensure("USER", defaultProfileIds.agentId);
    ensure("PROVIDER", defaultProfileIds.agentId);
    ensure("MARKETING", defaultProfileIds.agentId);
    ensure("ADMIN", defaultProfileIds.adminId);
    ensure("SUPER_ADMIN", defaultProfileIds.adminId);
    ensure("GLOBAL__SUPER_ADMIN", defaultProfileIds.adminId);
  }
  ...
  const resolvedRoutingProfileIdForRequest = resolveRoutingProfileId(args);
  if (!resolvedRoutingProfileIdForRequest) {
    return { enabled: true, changed: false, reason: "no routing profile mapping available and default not set" };
  }
  if (!connectClient || !CONNECT.ListUsersCommand) {
    return { enabled: false, reason: "aws connect sdk not available" };
  }
  const existing = await findConnectUserByUsername(CONNECT_INSTANCE_ID, args.email);
  if (existing) {
    ...
    await connectClient.send(new CONNECT.UpdateUserSecurityProfilesCommand({
      InstanceId: CONNECT_INSTANCE_ID,
      UserId: userId,
      SecurityProfileIds: securityProfileIds
    }));
    const routingProfileId2 = resolvedRoutingProfileIdForRequest;
    await connectClient.send(new CONNECT.UpdateUserRoutingProfileCommand({
      InstanceId: CONNECT_INSTANCE_ID,
      UserId: userId,
      RoutingProfileId: routingProfileId2
    }));
    if (args.givenName || args.familyName) {
      await connectClient.send(new CONNECT.UpdateUserIdentityInfoCommand({
        InstanceId: CONNECT_INSTANCE_ID,
        UserId: userId,
        IdentityInfo: {
          FirstName: args.givenName,
          LastName: args.familyName,
          ...includeEmailInIdentityInfo ? { Email: args.email } : {}
        }
      }));
    }
  ...
  const routingProfileId = resolvedRoutingProfileIdForRequest;
  const createParams = {
    InstanceId: CONNECT_INSTANCE_ID,
    Username: args.email,
    IdentityInfo: {
      FirstName: args.givenName || args.email.split("@")[0],
      LastName: args.familyName || "",
      ...includeEmailInIdentityInfo ? { Email: args.email } : {}
    },
    PhoneConfig: {
    ...
    Tags: {
      ...
      roles: determineRoles(args).join(",")
    }
  };
  if (CONNECT_TEMP_PASSWORD) {
    createParams.Password = CONNECT_TEMP_PASSWORD;
  }
  const createResp = await connectClient.send(new CONNECT.CreateUserCommand(createParams));
  return { enabled: true, changed: true, action: "created", userId: createResp?.UserId };
}
var cachedIdentityType;
async function getIdentityManagementTypeSafe() {
  if (!connectClient || !CONNECT.DescribeInstanceCommand) return void 0;
  if (cachedIdentityType) return cachedIdentityType;
  try {
    const resp = await connectClient.send(new CONNECT.DescribeInstanceCommand({ InstanceId: CONNECT_INSTANCE_ID }));
    cachedIdentityType = resp?.Instance?.IdentityManagementType;
    return cachedIdentityType;
  } catch (_e) {
    return void 0;
  }
}
async function getDefaultAgentAdminSecurityProfiles() {
  if (!connectClient || !CONNECT.ListSecurityProfilesCommand) return {};
  try {
    const list = await connectClient.send(new CONNECT.ListSecurityProfilesCommand({ InstanceId: CONNECT_INSTANCE_ID }));
    const profiles = list?.SecurityProfileSummaryList || [];
    let agentId;
    let adminId;
    for (const p of profiles) {
      const name = String(p?.Name || "").toUpperCase();
      if (!agentId && (name.includes("AGENT") || name.includes("BASIC AGENT"))) agentId = p?.Id;
      if (!adminId && name.includes("ADMIN")) adminId = p?.Id;
    }
