...
import { clinics as allClinics } from '../clinic-config/clinics';
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    if ((event.resource?.endsWith('/me/chime') || event.resource?.endsWith('/me/connect') || event.resource === '/connect/me') && event.httpMethod === 'GET') {
      const region = process.env.AWS_REGION || 'us-east-1';
      const voiceAgentsTable = process.env.VOICE_AGENTS_TABLE || '';
      const voiceQueueTable = process.env.VOICE_QUEUE_TABLE || '';
      const clinicHoursTable = process.env.CLINIC_HOURS_TABLE || '';
      
      // Generate user info from authentication
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ 
        ...
        region,
        voiceConfig: {
          agentsTable: voiceAgentsTable,
          queueTable: voiceQueueTable,
          hoursTable: clinicHoursTable
        },
        // Include user authentication info for frontend
        cognitoAuthenticated: true,
        userEmail: userEmail,
        userId: userId,
        // Chime SDK endpoints
        voiceEndpoints: {
          agentLogin: '/voice/agent/login',
          agentLogout: '/voice/agent/logout',
          agentState: '/voice/agent/state',
          agentStatus: '/voice/agent/status',
          agents: '/voice/agents',
          outbound: '/voice/outbound',
          endCall: '/voice/call/end'
        }
      }) };
    }
    ...
    const clinics = allClinics
      ...
      .map((c) => ({ 
        ...
        phoneNumber: c.phoneNumber,  // Chime calling phone number (E.164)
