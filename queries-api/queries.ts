...
async function listQueries(): Promise<APIGatewayProxyResult> {
...
async function getQuery(queryName: string): Promise<APIGatewayProxyResult> {
...
async function deleteQuery(queryName: string): Promise<APIGatewayProxyResult> {
  if (!queryName) return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ error: 'queryName required' }) };
