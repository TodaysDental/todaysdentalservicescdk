...
// src/services/chime/hold-call.ts
var hold_call_exports = {};
__export(hold_call_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(hold_call_exports);
...
// src/services/shared/utils/state-machine.ts
var CALL_STATE_MACHINE = {
  queued: ["ringing", "abandoned", "escalated"],
  ringing: ["accepting", "connected", "queued", "abandoned"],
  // FIX: ringing -> accepting
  accepting: ["connected", "ringing", "abandoned"],
  // FIX: accepting -> connected (success) or ringing (rollback)
  ...
  abandoned: [],
  // Terminal state
  escalated: ["ringing", "abandoned"],
  // Supervisor can reassign
  failed: []
  // Terminal state
};
function isValidStateTransition(from, to, stateMachine) {
...
function getValidNextStates(currentState, stateMachine) {
  return stateMachine[currentState] || [];
}
function validateStateTransition(from, to, stateMachine, entityType = "entity") {
  if (!isValidStateTransition(from, to, stateMachine)) {
    const validStates = getValidNextStates(from, stateMachine);
    throw new Error(
      `Invalid ${entityType} state transition: ${from} -> ${to}. Valid transitions from ${from}: ${validStates.join(", ") || "none (terminal state)"}`
    );
  }
}
...
var DistributedLock = class {
  ...
  async acquireWithFencingToken() {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        try {
          const { Item } = await this.ddb.send(new import_lib_dynamodb3.GetCommand({
            TableName: tableName,
            Key: { lockKey },
        ...
        await this.ddb.send(new import_lib_dynamodb3.PutCommand({
          TableName: tableName,
          Item: {
            lockKey,
            lockId: this.lockId,
  ...
  async release() {
    ...
    for (let attempt = 0; attempt < maxReleaseRetries; attempt++) {
      try {
        await this.ddb.send(new import_lib_dynamodb3.DeleteCommand({
          TableName: tableName,
          Key: { lockKey },
...
// src/services/chime/hold-call.ts
var sqs = new import_client_sqs2.SQSClient({});
var ddb2 = getDynamoDBClient();
var CHIME_MEDIA_REGION2 = process.env.CHIME_MEDIA_REGION || "us-east-1";
var chimeVoice2 = new import_client_chime_sdk_voice2.ChimeSDKVoiceClient({ region: CHIME_MEDIA_REGION2 });
var chimeClient = new import_client_chime_sdk_meetings2.ChimeSDKMeetingsClient({ region: CHIME_MEDIA_REGION2 });
var AGENT_PRESENCE_TABLE_NAME2 = process.env.AGENT_PRESENCE_TABLE_NAME;
var CALL_QUEUE_TABLE_NAME2 = process.env.CALL_QUEUE_TABLE_NAME;
var COMPENSATING_ACTIONS_QUEUE_URL = process.env.COMPENSATING_ACTIONS_QUEUE_URL;
var handler = async (event) => {
  ...
  try {
    ...
    const lock = new DistributedLock(ddb2, {
    ...
    try {
      const { Items: callRecords } = await ddb2.send(new import_lib_dynamodb4.QueryCommand({
        TableName: CALL_QUEUE_TABLE_NAME2,
      ...
      const { Item: agentRecord } = await ddb2.send(new import_lib_dynamodb4.GetCommand({
        TableName: AGENT_PRESENCE_TABLE_NAME2,
        Key: { agentId }
      }));
      if (!agentRecord) {
        return {
      ...
      if (!meetingExists) {
        ...
        await ddb2.send(new import_lib_dynamodb4.UpdateCommand({
          TableName: CALL_QUEUE_TABLE_NAME2,
      ...
      const holdOperationId = (0, import_crypto2.randomUUID)();
      ...
      try {
        await chimeVoice2.send(new import_client_chime_sdk_voice2.UpdateSipMediaApplicationCallCommand({
      ...
      if (agentAttendeeId && meetingId) {
        try {
          await chimeClient.send(new import_client_chime_sdk_meetings2.DeleteAttendeeCommand({
            MeetingId: meetingId,
      ...
      try {
        await ddb2.send(new import_lib_dynamodb4.TransactWriteCommand({
          TransactItems: [
            {
              Update: {
                TableName: CALL_QUEUE_TABLE_NAME2,
            ...
            },
            {
              Update: {
                ...
                UpdateExpression: "SET #agentStatus = :onHoldStatus, callStatus = :status, lastActivityAt = :timestamp, heldCallMeetingId = :meetingId, heldCallId = :callId REMOVE currentCallId",
        ...
        await scheduleCompensatingAction({
          ...
          intent: "INTENDED_HOLD" /* INTENDED_HOLD */,
          // Explicit intent: the hold was intended
...
async function scheduleCompensatingAction(action) {
  ...
  try {
    await sqs.send(new import_client_sqs2.SendMessageCommand({
      ...
      DelaySeconds: 5
      // Small delay to allow potential race conditions to settle
    }));
        ...
        TransactItems: [
          ...
          // FIX: Also update agent presence table to match
          {
            Update: {
              TableName: AGENT_PRESENCE_TABLE_NAME2,
              Key: { agentId },
              UpdateExpression: "SET #agentStatus = :onHoldStatus, callStatus = :onhold, lastActivityAt = :timestamp, heldCallMeetingId = :meetingId, heldCallId = :callId, directRecoveryApplied = :true REMOVE currentCallId",
              ExpressionAttributeNames: {
                "#agentStatus": "status"
              },
              ExpressionAttributeValues: {
...
async function validateMeetingExists(meetingId) {
  try {
    await chimeClient.send(new import_client_chime_sdk_meetings2.GetMeetingCommand({ MeetingId: meetingId }));
    return true;
  } catch (err) {
