...
async function getAllClinicIds() {
  try {
    ...
    const uniqueClinicIds = new Set((result.Items || []).map((item) => item.clinicId));
...
async function getRecentEmails(clinicIds, limit) {
  const filterExpressions = [];
  const expressionValues = {};
...
async function getStatusBreakdown(clinicIds, period) {
  const breakdown = {
    "QUEUED": 0,
    "SENT": 0,
    "DELIVERED": 0,
