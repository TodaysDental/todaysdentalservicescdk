...
async function handleSendToUser(userId, notification, dryRun, skipPreferenceCheck = false) {
  ...
  if (dryRun) {
    const results2 = devices.map((device) => ({
      deviceId: device.deviceId,
      platform: device.platform,
      success: true,
      dryRun: true
    }));
    return { sent: devices.length, failed: 0, results: results2 };
  }
...
async function handleSendToClinic(clinicId, notification, dryRun, skipPreferenceCheck = false) {
