...
// src/services/patient-portal/public-appttypes.ts
var public_appttypes_exports = {};
__export(public_appttypes_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(public_appttypes_exports);
...
var TABLE_NAME = process.env.APPTTYPES_TABLE_NAME || "";
var PARTITION_KEY = "clinicId";
var SORT_KEY = "label";
...
var handler = async (event) => {
  ...
  try {
    ...
    if (label) {
      ...
      const data = await dynamo.send(new import_lib_dynamodb.GetCommand({
        TableName: TABLE_NAME,
        Key: {
          [PARTITION_KEY]: clinicId,
          [SORT_KEY]: decodedLabel
          // Use the decoded label as the Sort Key
        }
      }));
      if (!data.Item) {
