...
// src/services/chime/heartbeat.ts
...
function extendSessionForActiveCall() {
...
function calculateHeartbeatTTL(currentSessionExpiresAt) {
  const nowSeconds = Math.floor(Date.now() / 1e3);
...
var handler = async (event) => {
  ...
  try {
    ...
    const { Item: agentRecord } = await ddb.send(new import_lib_dynamodb2.GetCommand({
    ...
    const hasActiveCallFields = agentRecord.currentCallId || agentRecord.heldCallId || agentRecord.ringingCallId;
    const hasActiveCallStatus = agentRecord.callStatus === "on_hold" || agentRecord.callStatus === "connected" || agentRecord.callStatus === "ringing";
    const hasInProgressOperation = agentRecord.transferringCallId || agentRecord.incomingTransferId || agentRecord.heldCallMeetingId || // Has meeting info for held call
    agentRecord.status === "OnHold";
    const isOnActiveCall = hasActiveCallFields || hasActiveCallStatus || hasInProgressOperation;
    let newTtl;
    let newSessionExpiresAt;
    if (isOnActiveCall) {
    ...
    try {
      const shouldHealStatus = !isOnActiveCall && agentRecord.status !== "Online" && agentRecord.status !== "Offline";
      const baseUpdateExpression = "SET lastActivityAt = :timestamp, lastHeartbeatAt = :timestamp, #ttl = :ttl, sessionExpiresAtEpoch = :sessionExpiry, heartbeatCount = if_not_exists(heartbeatCount, :zero) + :one";
      const updateExpression = shouldHealStatus ? baseUpdateExpression + ", #status = :healedStatus" : baseUpdateExpression;
      ...
      await ddb.send(new import_lib_dynamodb2.UpdateCommand({
        ...
        ConditionExpression: "attribute_exists(agentId) AND (attribute_not_exists(sessionExpiresAtEpoch) OR sessionExpiresAtEpoch > :nowSeconds)",
      ...
      if (error.name === "ConditionalCheckFailedException") {
        try {
          ...
          await ddb.send(new import_lib_dynamodb2.UpdateCommand({
            TableName: AGENT_PRESENCE_TABLE_NAME,
            Key: { agentId },
            UpdateExpression: "SET #status = :offline, lastActivityAt = :timestamp, cleanupReason = :reason, expiredWithCallId = :expiredCallId REMOVE currentCallId, ringingCallId, callStatus, heldCallId, heldCallMeetingId, inboundMeetingInfo, inboundAttendeeInfo",
            ExpressionAttributeNames: { "#status": "status" },
