...
// src/services/ai-agents/insurance-textract-handler.ts
var insurance_textract_handler_exports = {};
__export(insurance_textract_handler_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(insurance_textract_handler_exports);
...
var IMAGE_BUCKET = process.env.IMAGE_BUCKET || "";
var OPEN_DENTAL_API_BASE = "https://api.opendental.com/api/v1";
...
function getCorsHeaders(event) {
  const origin2 = event.headers?.origin || event.headers?.Origin;
  const allowOrigin = origin2 && ALLOWED_ORIGINS_LIST.includes(origin2) ? origin2 : "https://todaysdentalinsights.com";
  return buildCorsHeaders({ allowOrigin, allowMethods: ["OPTIONS", "POST"] }, origin2);
}
function parseInsuranceInfo(textBlocks) {
  ...
  if (rxBinMatch)
    result.rxBin = rxBinMatch[1];
  const rxPcnMatch = allText.match(/pcn\s*[:\s]?\s*([A-Z0-9]+)/i);
  if (rxPcnMatch)
    result.rxPcn = rxPcnMatch[1];
  const filledFields = [
...
function formatInsuranceForChatbot(info) {
  const parts = [];
...
async function uploadToOpenDental(clinicCreds, patNum, imageBase64, extension, description) {
  ...
  const uploadPayload = {
    ...
    ImgType: imgType,
    // 'Document' for PDFs, 'Photo' for images
    DocCategory: 1,
    // Default category - adjust based on clinic setup
    DateCreated: (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").substring(0, 19)
  };
  const response = await axios_default.post(`${OPEN_DENTAL_API_BASE}/documents/Upload`, uploadPayload, {
    headers: {
...
var handler = async (event) => {
  ...
  try {
    ...
    const {
      images,
      // Array of base64 images (front/back of card)
      clinicId,
      // Clinic identifier
      patNum,
      // Optional: Patient number for OpenDental upload
      uploadToOD,
      // Optional: Whether to upload to OpenDental
      sessionId
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      if (base64Content.includes(",")) {
        ...
        if (header.includes("application/pdf")) {
          ...
          if (typeMatch)
            fileType = `image/${typeMatch[1].toLowerCase()}`;
        }
      ...
      const s3Key = `insurance-cards/${clinicId}/${v4_default()}${extension}`;
      ...
      try {
        let textBlocks = [];
        try {
          const textractResponse = await textractClient.send(new import_client_textract.AnalyzeDocumentCommand({
            Document: {
              S3Object: {
                Bucket: IMAGE_BUCKET,
                Name: s3Key
          ...
          const detectResponse = await textractClient.send(new import_client_textract.DetectDocumentTextCommand({
            Document: {
              S3Object: {
                Bucket: IMAGE_BUCKET,
                Name: s3Key
    ...
    const combinedInfo = allExtractedInfo.reduce((acc, info) => {
      return {
...
${info.rawText}`,
    ...
    const response = {
      ...
      chatbotMessage,
      uploadedDocuments: uploadedDocuments.length > 0 ? uploadedDocuments : void 0,
...
function buildInsurancePrompt(info) {
  return `
