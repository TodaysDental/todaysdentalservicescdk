...
// src/services/clinic/clinicHours.ts
...
var xhr_default = isXHRAdapterSupported && function(config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    request.onerror = function handleError(event) {
      ...
      const err2 = new AxiosError_default(msg, AxiosError_default.ERR_NETWORK, config, request);
...
var TABLE_NAME = process.env.CLINIC_HOURS_TABLE || "ClinicHours";
var OPEN_DENTAL_API_HOST = "https://api.opendental.com";
var OPEN_DENTAL_API_BASE = "/api/v1";
var ALL_CLINIC_IDS = process.env.ALL_CLINIC_IDS?.split(",").map((id) => id.trim()).filter((id) => id.length > 0) || [];
var clinicCredsCache = {};
...
var schedulerHandler = async () => {
  ...
  for (const clinicId of ALL_CLINIC_IDS) {
    ...
    try {
      ...
      const response = await axios_default.get(`${OPEN_DENTAL_API_HOST}${schedulesPath}`, {
        headers: {
      ...
      const fullWeekScheduleData = response.data || [];
      let finalHoursItem = {
        clinicId,
        updatedAt: Date.now(),
      ...
      daysOfWeek.forEach((day) => {
        finalHoursItem[day] = closedHoursDefault;
      });
  ...
  try {
    ...
    if (method === "GET") {
      if (path.endsWith("/hours") && !path.includes("/clinics/")) {
        return listHoursPublic(event);
      }
      if (path.endsWith("/hours") && path.includes("/clinics/") && clinicId) {
        return getHoursPublic(event, clinicId);
      }
    ...
    if (path.endsWith("/hours")) {
      if (method === "POST")
        return createHours(event, userPerms);
      if (method === "PUT")
        return updateHours(event, userPerms, clinicId);
    }
    if (path.endsWith("/hours/{clinicId}")) {
      if (method === "PUT")
        return updateHours(event, userPerms, clinicId);
      if (method === "DELETE")
        return deleteHours(event, userPerms, clinicId);
    }
...
function deriveDailyHours(dailyScheduleBlocks, date) {
  ...
  for (const block of dailyScheduleBlocks) {
    if (!block.StartTime || !block.StopTime)
      continue;
    if (minOpenTime === null || block.StartTime < minOpenTime)
      minOpenTime = block.StartTime;
    if (maxCloseTime === null || block.StopTime > maxCloseTime)
      maxCloseTime = block.StopTime;
  }
...
function validateHoursData(body) {
  const days = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"];
  for (const day of days) {
    const dayData = body[day];
    if (dayData && !dayData.closed) {
      if (!dayData.open || !dayData.close)
        return { valid: false, message: `${day} requires times` };
      if (!/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(dayData.open))
        return { valid: false, message: "Invalid time format" };
      if (dayData.open >= dayData.close)
        return { valid: false, message: "Close time must be after open" };
    }
  }
...
function err(code, message, event) {
