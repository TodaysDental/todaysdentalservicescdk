...
async function ayrsharePost(apiKey, profileKey, postData) {
  try {
...
async function ayrshareDeletePost(apiKey, profileKey, postId) {
  try {
    const res = await axios_default.delete(
      `${AYRSHARE_URL}/post`,
      {
        headers: {
          "Authorization": `Bearer ${apiKey}`,
...
async function ayrshareGetHistory(apiKey, profileKey, params) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/history`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
async function ayrshareGetAnalytics(apiKey, profileKey, postId) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/analytics/post`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
async function ayrshareGetSocialStats(apiKey, profileKey, platforms) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/analytics/social`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
async function ayrshareGetComments(apiKey, profileKey, postId) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/comments`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
function isRetryableError(error) {
  ...
  return message.includes("rate limit") || message.includes("too many requests") || message.includes("429") || message.includes("timeout") || message.includes("network");
}
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        const batchResults = await Promise.all(
          batch.map(async (config) => {
            const resolvedPostData = resolvePlaceholders ? resolvePostPlaceholders(postData, config) : postData;
        ...
        const batchResults = await Promise.all(
          batch.map(async (config) => {
            try {
              ...
              return { clinicId: config.clinicId, status: "success", id: res.id };
            } catch (error) {
              console.error(`Bulk post failed for clinic ${config.clinicId}:`, error);
              return { clinicId: config.clinicId, status: "failed", error: error.message };
            }
          })
        );
        allResults.push(...batchResults);
