...
async function updateEmailStatus(trackingId, status, sesMessageId, errorMessage) {
  if (!EMAIL_ANALYTICS_TABLE || !trackingId)
    return;
  try {
    const updateExpr = errorMessage ? "SET #status = :status, sentAt = :now, errorMessage = :err" + (sesMessageId ? ", sesMessageId = :sesId" : "") : "SET #status = :status, sentAt = :now" + (sesMessageId ? ", sesMessageId = :sesId" : "");
...
async function processEmailTask(task) {
  try {
    ...
    await updateEmailStatus(task.trackingId, "FAILED", void 0, error.message);
    throw error;
  }
}
