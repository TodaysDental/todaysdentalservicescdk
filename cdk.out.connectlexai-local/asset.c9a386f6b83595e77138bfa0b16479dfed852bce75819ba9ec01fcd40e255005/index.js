...
// src/services/chatbot/websocket-disconnect.ts
...
// src/services/chatbot/websocket-disconnect.ts
var dynamoClient = new import_client_dynamodb2.DynamoDBClient({ region: process.env.AWS_REGION });
...
var handler = async (event) => {
  ...
  try {
    ...
    try {
      const connectionQuery = await docClient.send(new import_lib_dynamodb.QueryCommand({
    ...
    const disconnectionRecord = {
      ...
      ttl,
      metadata: {
        disconnectionTime: (/* @__PURE__ */ new Date()).toISOString(),
        originalSessionId: sessionId !== `disconnect-${connectionId}-${timestamp}` ? sessionId : void 0,
