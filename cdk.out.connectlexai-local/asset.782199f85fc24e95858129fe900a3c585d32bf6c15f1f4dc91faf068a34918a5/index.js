...
// src/services/credentialing/expiration-monitor-handler.ts
var expiration_monitor_handler_exports = {};
__export(expiration_monitor_handler_exports, {
  apiHandler: () => apiHandler,
  scheduledHandler: () => scheduledHandler
});
module.exports = __toCommonJS(expiration_monitor_handler_exports);
...
var EXPIRATION_ALERTS_TABLE = process.env.EXPIRATION_ALERTS_TABLE;
var STAFF_USER_TABLE = process.env.STAFF_USER_TABLE || "StaffUser";
var SYSTEM_SENDER_EMAIL = process.env.SYSTEM_SENDER_EMAIL || "credentialing@todaysdentalinsights.com";
var CREDENTIALING_TEAM_EMAIL = process.env.CREDENTIALING_TEAM_EMAIL || "credentialing@todaysdentalinsights.com";
...
async function sendEmailViaSMTP(creds, to, subject, body, senderName) {
...
async function scanExpiringCredentials() {
  const now = /* @__PURE__ */ new Date();
  const scannedAt = now.toISOString();
  const in30Days = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
  const in60Days = new Date(now.getTime() + 60 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
  const in90Days = new Date(now.getTime() + 90 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
  ...
  do {
    ...
    for (const provider of Items) {
      ...
      for (const { field, label } of EXPIRATION_FIELDS) {
        ...
        if (!expiryDateStr)
          continue;
        if (expiryDateStr < today || expiryDateStr > in90Days)
          continue;
        const expiryTime = new Date(expiryDateStr).getTime();
        const daysUntilExpiry = Math.ceil((expiryTime - now.getTime()) / (24 * 60 * 60 * 1e3));
        const credential = {
          providerId,
          providerName,
  ...
  const sortByDays = (a, b) => a.daysUntilExpiry - b.daysUntilExpiry;
...
async function sendTeamSummaryEmail(summary) {
  if (summary.totalExpiring === 0) {
    console.log("No expiring credentials found, skipping team email");
    return;
  }
  const criticalList = summary.critical.map(
    (c) => `\u2022 ${c.providerName}: ${c.credentialType} expires in ${c.daysUntilExpiry} days (${c.expirationDate})`
  ).join("\n");
  const warningList = summary.warning.slice(0, 10).map(
    (c) => `\u2022 ${c.providerName}: ${c.credentialType} expires in ${c.daysUntilExpiry} days`
  ).join("\n");
...
async function sendProviderReminderEmail(credential) {
  if (!credential.email) {
...
async function createRenewalTask(credential) {
  const taskId = v4_default();
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const priority = credential.urgency === "critical" ? "urgent" : credential.urgency === "warning" ? "high" : "medium";
...
async function wasAlertSentToday(providerId, credentialType, alertType) {
...
var scheduledHandler = async (event) => {
  ...
  try {
    ...
    if (method === "GET" && path === "/expirations/alerts") {
      ...
      if (providerId) {
        ...
        const { Items } = await ddb.send(new import_lib_dynamodb.ScanCommand({
          TableName: EXPIRATION_ALERTS_TABLE,
