...
async function createTemplate(event, userPerms) {
  ...
  const hasAnyContent = typeof body.email_body === "string" && body.email_body.trim().length > 0 || typeof body.text_message === "string" && body.text_message.trim().length > 0 || typeof body.rcs_message === "string" && body.rcs_message.trim().length > 0 || !!body.rcs_rich_card || !!body.rcs_carousel || typeof body.voice_message === "string" && body.voice_message.trim().length > 0;
  if (!hasAnyContent) {
    return {
      statusCode: 400,
      headers: getCorsHeaders(event),
      body: JSON.stringify({
        error: "At least one template content field is required (email_body, text_message, rcs_message, rcs_rich_card, rcs_carousel, voice_message)"
      })
    };
  }
  if (!body.module) {
