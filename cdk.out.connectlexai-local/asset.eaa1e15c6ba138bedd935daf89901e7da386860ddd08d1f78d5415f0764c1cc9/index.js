...
// src/services/patient-portal/appttypes.ts
var appttypes_exports = {};
__export(appttypes_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(appttypes_exports);
...
var PARTITION_KEY = process.env.PARTITION_KEY || "clinicId";
var SORT_KEY = process.env.SORT_KEY || "label";
var createResponse = (statusCode, body, requestOrigin) => ({
  statusCode,
  headers: {
    ...
    switch (method) {
      case "GET": {
        ...
        if (pathParameters && pathParameters.id) {
          ...
          return data.Item ? createResponse(200, data.Item, requestOrigin) : createResponse(404, { message: "Appointment type not found" }, requestOrigin);
        } else {
          const data = await dynamo.send(new import_lib_dynamodb.QueryCommand({
            TableName: TABLE_NAME,
      ...
      case "POST": {
        ...
        if (item["AppointmentTypeNum"])
          item["AppointmentTypeNum"] = Number(item["AppointmentTypeNum"]);
        try {
      ...
      case "PUT": {
        ...
        if (item["AppointmentTypeNum"])
          item["AppointmentTypeNum"] = Number(item["AppointmentTypeNum"]);
        await dynamo.send(new import_lib_dynamodb.PutCommand({ TableName: TABLE_NAME, Item: item }));
