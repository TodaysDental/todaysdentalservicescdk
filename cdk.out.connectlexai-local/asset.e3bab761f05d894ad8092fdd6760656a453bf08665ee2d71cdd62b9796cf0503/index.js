...
// src/services/clinic/budgetCrud.ts
var budgetCrud_exports = {};
__export(budgetCrud_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(budgetCrud_exports);
...
async function getAllBudgets(event, userPerms) {
...
async function getBudget(event, userPerms, clinicName) {
  if (!clinicName)
...
async function updateBudget(event, userPerms, clinicName) {
  if (!clinicName)
    return err(400, "clinicName required", event);
  const decodedClinicName = decodeURIComponent(clinicName);
