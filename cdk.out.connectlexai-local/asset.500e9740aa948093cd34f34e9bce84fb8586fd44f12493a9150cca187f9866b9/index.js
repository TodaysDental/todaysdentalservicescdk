...
// src/integrations/communication/unsubscribe-handler.ts
var unsubscribe_handler_exports = {};
__export(unsubscribe_handler_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(unsubscribe_handler_exports);
...
function verifyUnsubscribeToken(token) {
  try {
    ...
    if (!base64Data || !signature)
      return null;
    const expectedSignature = (0, import_crypto.createHmac)("sha256", UNSUBSCRIBE_SECRET).update(base64Data).digest("base64url");
...
async function recordUnsubscribe(ddb2, tableName, identifier, clinicId, channels, reason, token) {
  let pk;
  ...
  const existing = await ddb2.send(new import_lib_dynamodb.GetCommand({
    TableName: tableName,
    Key: { pk, sk }
  }));
  if (existing.Item) {
    const existingChannels = existing.Item.unsubscribedChannels || [];
    const mergedChannels = [.../* @__PURE__ */ new Set([...existingChannels, ...channels])];
    await ddb2.send(new import_lib_dynamodb.UpdateCommand({
      TableName: tableName,
      Key: { pk, sk },
      UpdateExpression: "SET unsubscribedChannels = :channels, updatedAt = :now, unsubscribedAt = :now" + (reason ? ", unsubscribeReason = :reason" : "") + (token ? ", unsubscribeToken = :token" : ""),
    ...
    await ddb2.send(new import_lib_dynamodb.PutCommand({
      TableName: tableName,
      Item: item
    }));
  }
}
async function recordResubscribe(ddb2, tableName, identifier, clinicId, channels) {
  let pk;
  if (identifier.patientId) {
    pk = `PREF#${identifier.patientId}`;
  } else if (identifier.email) {
...
async function getPreferences(ddb2, tableName, identifier) {
  let pk;
  if (identifier.patientId) {
    pk = `PREF#${identifier.patientId}`;
  } else if (identifier.email) {
  ...
  const result = await ddb2.send(new import_lib_dynamodb.QueryCommand({
    TableName: tableName,
...
var clinicInfoCache = /* @__PURE__ */ new Map();
...
function http(code, body, event, contentType) {
...
async function renderUnsubscribePage(tokenPayload, success, error) {
  const clinicInfo = tokenPayload.clinicId ? await getClinicInfo(tokenPayload.clinicId) : { name: "Dental Clinic", email: "", phone: "" };
  const channelName = tokenPayload.channel === "EMAIL" ? "emails" : tokenPayload.channel === "SMS" ? "text messages" : "RCS messages";
  if (success) {
    return `
<!DOCTYPE html>
<html lang="en">
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>\xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} ${clinicInfo.name}. All rights reserved.</p>
    </div>
  </div>
</body>
</html>`;
...
async function handleGetUnsubscribe(event) {
  const token = event.pathParameters?.token;
  if (!token) {
    return http(400, await renderUnsubscribePage({}, false, "Invalid unsubscribe link."), event, "text/html");
  }
  const payload = verifyUnsubscribeToken(decodeURIComponent(token));
  if (!payload) {
    return http(400, await renderUnsubscribePage(
      {},
      false,
      "This unsubscribe link has expired or is invalid. Please contact the clinic directly."
    ), event, "text/html");
  }
  return http(200, await renderUnsubscribePage(payload), event, "text/html");
}
async function handlePostUnsubscribe(event) {
  const token = event.pathParameters?.token;
  if (!token) {
    return http(400, await renderUnsubscribePage({}, false, "Invalid unsubscribe link."), event, "text/html");
  }
  ...
  try {
    ...
    const reason = body.reason || void 0;
    let channels = [];
    ...
    if (payload.patientId)
      identifier.patientId = payload.patientId;
    if (payload.email)
      identifier.email = payload.email;
    if (payload.phone)
      identifier.phone = payload.phone;
    ...
    return http(500, await renderUnsubscribePage(
      payload,
      false,
      "We encountered an error processing your request. Please try again or contact the clinic directly."
    ), event, "text/html");
  }
}
async function handleGetPreferences(event, userPerms) {
...
async function handlePutPreferences(event, userPerms) {
  const body = parseBody(event.body);
  const email = body.email || userPerms.email;
  const phone = body.phone;
  const patientId = body.patientId;
  const clinicId = body.clinicId || "GLOBAL";
  const subscribeChannels = body.subscribe || [];
  const unsubscribeChannels = body.unsubscribe || [];
  if (!email && !phone && !patientId) {
    return http(400, { error: "At least one identifier (email, phone, or patientId) is required" }, event);
  }
  try {
