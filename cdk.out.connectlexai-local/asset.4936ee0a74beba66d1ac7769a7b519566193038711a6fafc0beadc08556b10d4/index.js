...
async function handleRegister(event, userPerms) {
  ...
  try {
    ...
    for (const clinicId of clinicIdsToRegister) {
      try {
        const existingResult = await ddb.send(new import_lib_dynamodb.GetCommand({
          TableName: DEVICE_TOKENS_TABLE,
          Key: { userId, deviceId }
        }));
        ...
        const newClinicIds = [.../* @__PURE__ */ new Set([...existingClinicIds, clinicId])];
        const record = {
          userId,
          deviceId,
          clinicId: clinicIdsToRegister[0],
          // Primary clinic (first one)
        ...
        if (clinicError instanceof import_client_dynamodb2.ConditionalCheckFailedException) {
          const updateNow = (/* @__PURE__ */ new Date()).toISOString();
          await ddb.send(new import_lib_dynamodb.UpdateCommand({
            TableName: DEVICE_TOKENS_TABLE,
            Key: { userId, deviceId },
