...
var handler = async (event) => {
  ...
  try {
    ...
    await ddb.send(new import_lib_dynamodb6.PutCommand({
    ...
    try {
      const chimeVoiceClient = new import_client_chime_sdk_voice.ChimeSDKVoiceClient({ region: CHIME_MEDIA_REGION });
      ...
      await checkQueueForWork(agentId, presenceItem);
    } catch (queueError) {
      console.error("[start-session] Error checking queue for work (non-fatal):", queueError);
    }
    const responseBody = {
