...
var credentialing_doc_processor_exports = {};
__export(credentialing_doc_processor_exports, {
  apiHandler: () => apiHandler,
  classifyDocumentFromPath: () => classifyDocumentFromPath,
  extractFieldsWithBedrock: () => extractFieldsWithBedrock,
  extractTextFromDocument: () => extractTextFromDocument,
  handler: () => handler
});
module.exports = __toCommonJS(credentialing_doc_processor_exports);
...
async function extractTextFromDocument(bucket, key) {
  ...
  if (isImage) {
    const response = await textract.send(new import_client_textract.AnalyzeDocumentCommand({
    ...
    const startResponse = await textract.send(new import_client_textract.StartDocumentAnalysisCommand({
    ...
    while (status === "IN_PROGRESS" && attempts < maxAttempts) {
      ...
      const getResponse = await textract.send(new import_client_textract.GetDocumentAnalysisCommand({ JobId: jobId }));
      status = getResponse.JobStatus || "FAILED";
      if (status === "SUCCEEDED") {
        blocks = getResponse.Blocks || [];
        let nextToken = getResponse.NextToken;
        while (nextToken) {
          const nextPage = await textract.send(new import_client_textract.GetDocumentAnalysisCommand({
            JobId: jobId,
            NextToken: nextToken
          }));
    ...
    if (status !== "SUCCEEDED") {
      ...
      const detectResponse = await textract.send(new import_client_textract.DetectDocumentTextCommand({
        Document: { S3Object: { Bucket: bucket, Name: key } }
      }));
...
Return only the JSON object, no explanation.`;
  try {
    const response = await bedrock.send(new import_client_bedrock_runtime.InvokeModelCommand({
...
async function storeExtractedData(documentId, providerId, documentType, extractedFields, rawText) {
  ...
  const highConfidenceFields = Object.entries(extractedFields).filter(([_, data]) => data.confidence >= 0.85 && data.value !== null);
...
var apiHandler = async (event) => {
  ...
  try {
    ...
    const { Item: document } = await ddb.send(new import_lib_dynamodb.GetCommand({
      TableName: DOCUMENTS_TABLE,
      Key: { documentId }
    }));
    if (!document) {
