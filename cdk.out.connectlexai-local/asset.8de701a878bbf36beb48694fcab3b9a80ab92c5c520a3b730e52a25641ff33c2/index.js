...
// src/services/clinic/queries.ts
var queries_exports = {};
...
async function listQueries(event, _userPerms) {
...
async function getQuery(event, _userPerms, queryName) {
  if (!queryName)
    return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ error: "queryName required" }) };
...
async function createQuery(event, _userPerms) {
...
async function updateQuery(event, _userPerms, queryName) {
  if (!queryName)
    return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ error: "queryName required" }) };
  const body = parseBody(event.body);
...
async function deleteQuery(event, _userPerms, queryName) {
  if (!queryName)
    return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ error: "queryName required" }) };
