...
// src/services/lease-management/deleteLease.ts
var deleteLease_exports = {};
__export(deleteLease_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(deleteLease_exports);
...
async function cleanupLeaseData(clinicId, leaseId, documents) {
  let s3Deleted = 0;
  let extractedDeleted = 0;
  try {
    if (LEASE_DOCUMENTS_BUCKET) {
      const s3Prefix = `${clinicId}/${leaseId}/`;
      const listResult = await s3Client.send(new import_client_s3.ListObjectsV2Command({
