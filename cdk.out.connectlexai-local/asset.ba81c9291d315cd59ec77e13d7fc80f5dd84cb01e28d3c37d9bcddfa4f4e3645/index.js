...
// src/services/marketing/analytics.ts
...
async function ayrshareGetLinkAnalytics(apiKey, profileKey, postId) {
  try {
    ...
    if (postId)
      params.id = postId;
    const res = await axios_default.get(`${AYRSHARE_URL}/analytics/links`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
var ANALYTICS_TABLE = process.env.MARKETING_ANALYTICS_TABLE;
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.includes("/dashboard") && method === "GET") {
      ...
      const topPerformingClinics = Object.entries(clinicStats).map(([clinicId, stats]) => ({
        clinicId,
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          platformPerformance: {
            facebook: { totalEngagement: Math.floor(totalLikes * 0.3), avgEngagementRate: 5.8, reach: Math.floor(totalReach * 0.35) },
    ...
    if (path.includes("/posts/") && event.pathParameters?.postId && method === "GET") {
      ...
      const postRes = await ddb.send(new import_lib_dynamodb.QueryCommand({
        TableName: POSTS_TABLE,
      ...
      const analyticsRes = await ddb.send(new import_lib_dynamodb.QueryCommand({
    ...
    if (path.includes("/clinics/") && event.pathParameters?.clinicId && method === "GET") {
      ...
      const postsRes = await ddb.send(new import_lib_dynamodb.QueryCommand({
        TableName: POSTS_TABLE,
        IndexName: "ByClinic",
        ...
        body: JSON.stringify({
          ...
          byPlatform: {
            ...
            gbusiness: {
              ...
              actions: Math.floor(totalShares * 0.2),
