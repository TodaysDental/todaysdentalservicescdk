...
// src/services/chatbot/chat-history.ts
var chat_history_exports = {};
__export(chat_history_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(chat_history_exports);
...
var TABLE_NAME = process.env.CONVERSATIONS_TABLE || "chatbot-conversations";
  ...
  try {
    ...
    if (clinicId) {
      ...
      if (authorizedClinics.length === 0) {
        ...
        const clinicPromises = authorizedClinics.map(
          (clinic) => getConversationsByClinic(clinic, startDate, endDate)
        );
        const clinicResults = await Promise.all(clinicPromises);
        conversations = clinicResults.flat().sort((a, b) => (b.lastActivity || 0) - (a.lastActivity || 0));
      }
    }
    return {
      statusCode: 200,
...
async function getConversationsByClinic(clinicId, startDate, endDate) {
...
async function getAllConversations(startDate, endDate) {
  const params = {
  ...
  const result = await docClient.send(new import_lib_dynamodb.ScanCommand(params));
  ...
  const conversations = await Promise.all(
    Array.from(sessionMap.values()).map((session) => getConversationDetail(session.sessionId, session.clinicId))
  );
  return conversations.filter((conv) => conv !== null);
}
async function getSessions(clinicId, startDate, endDate) {
  const params = {
    TableName: TABLE_NAME,
...
async function getConversationDetail(sessionId, clinicId) {
  try {
    ...
    const messages = items.filter(
      (item) => item.messageType === "user" || item.messageType === "assistant"
    ).map((item) => ({
