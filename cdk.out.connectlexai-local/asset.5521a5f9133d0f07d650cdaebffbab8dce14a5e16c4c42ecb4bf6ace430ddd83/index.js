  ...
  async acquireWithFencingToken() {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        const isRetryable2 = RETRYABLE_ERRORS.includes(errorName);
        if (isRetryable2) {
...
var MAX_SIMUL_RING_CALLS = Math.max(1, Number.parseInt(process.env.MAX_SIMUL_RING_CALLS || "10", 10));
...
function createCheckQueueForWork(deps) {
  ...
  async function getRankedQueuedCalls(clinicId) {
    const { Items: queuedCalls } = await ddb2.send(new import_lib_dynamodb4.QueryCommand({
      TableName: callQueueTableName,
  ...
  async function fetchIdleAgentsForClinic(clinicId, maxAgentsToFetch) {
    ...
    do {
      const result = await ddb2.send(new import_lib_dynamodb4.QueryCommand({
...
var docClient = import_lib_dynamodb5.DynamoDBDocumentClient.from(dynamoClient);
...
async function isAiInboundEnabled(clinicId) {
  const response = await docClient.send(new import_lib_dynamodb5.GetCommand({
...
async function publishMetric(name, value, dimensions = {}, unit = import_client_cloudwatch.StandardUnit.Count) {
  if (!METRICS_ENABLED) {
    return;
  }
  try {
...
async function publishMetrics(metrics) {
  ...
  for (const metric of metrics) {
    await publishMetric(
      ...
      metric.value,
      metric.dimensions || {},
      metric.unit || import_client_cloudwatch.StandardUnit.Count
    );
  }
}
    ...
    ...callType === "answered" ? [
      {
        ...
        value: duration,
        dimensions: { ClinicId: clinicId },
        unit: import_client_cloudwatch.StandardUnit.Seconds
      },
      {
...
async function saveQualityMetrics(ddb2, callId, clinicId, timestamp, metrics, callAnalyticsTableName) {
  try {
    await ddb2.send(new import_lib_dynamodb12.UpdateCommand({
...
async function logAuditEvent(ddb2, event, auditTableName, config = {}) {
  ...
  try {
    ...
    await ddb2.send(new import_lib_dynamodb13.PutCommand({
...
// src/services/chime/inbound-router.ts
var ddb = import_lib_dynamodb14.DynamoDBDocumentClient.from(new import_client_dynamodb3.DynamoDBClient({}));
...
async function addToQueue(clinicId, callId, phoneNumber) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    const { Items: existingCalls } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
    ...
    try {
      await ddb.send(new import_lib_dynamodb14.PutCommand({
        TableName: CALL_QUEUE_TABLE_NAME,
        Item: entry,
  ...
  const { Items: finalCheck } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
    TableName: CALL_QUEUE_TABLE_NAME,
    IndexName: "callId-index",
    KeyConditionExpression: "callId = :callId",
    ExpressionAttributeValues: { ":callId": callId }
...
async function getQueuePosition(clinicId, callId) {
  try {
    const { Items: thisCallItems } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
      TableName: CALL_QUEUE_TABLE_NAME,
      IndexName: "callId-index",
      KeyConditionExpression: "callId = :callId",
      ExpressionAttributeValues: { ":callId": callId }
    ...
    if (status !== "queued" || !queueEntryTime)
      return null;
    const { Items: allQueuedCalls } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
      TableName: CALL_QUEUE_TABLE_NAME,
    ...
    const { Items: onlineAgents } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
...
async function isClinicOpen(clinicId) {
  ...
  try {
    const { Item } = await ddb.send(new import_lib_dynamodb14.GetCommand({
  ...
  try {
    switch (eventType) {
      case "NEW_INBOUND_CALL": {
        ...
        const { Items: clinics } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
        ...
        try {
          ...
          await ddb.send(new import_lib_dynamodb14.UpdateCommand({
            TableName: CALL_QUEUE_TABLE_NAME,
        ...
        try {
          ...
          await checkQueueForWork("SYSTEM", { activeClinicIds: [clinicId] });
        } catch (dispatchErr) {
          console.warn("[NEW_INBOUND_CALL] Fair-share dispatch failed (non-fatal):", dispatchErr);
        }
        try {
          const { Item: refreshedCall } = await ddb.send(new import_lib_dynamodb14.GetCommand({
            TableName: CALL_QUEUE_TABLE_NAME,
            Key: { clinicId, queuePosition: queueEntry.queuePosition },
            ConsistentRead: true
          }));
          assignmentSucceeded = !!(refreshedCall && refreshedCall.status === "ringing" && Array.isArray(refreshedCall.agentIds) && refreshedCall.agentIds.length > 0);
        } catch (refreshErr) {
        ...
        if (assignmentSucceeded) {
          ...
          if (enableRecording && recordingsBucket && pstnLegCallId) {
            ...
            try {
              const { Items: callRecords } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
                TableName: CALL_QUEUE_TABLE_NAME,
                IndexName: "callId-index",
                KeyConditionExpression: "callId = :callId",
                ExpressionAttributeValues: { ":callId": callId }
      ...
      case "CALL_ANSWERED": {
        ...
        if (status === "dialing" && meetingInfo?.MeetingId && assignedAgentId) {
          ...
          try {
            ...
            try {
              const { Items: callRecords2 } = await ddb.send(new import_lib_dynamodb14.QueryCommand({
                TableName: CALL_QUEUE_TABLE_NAME,
                IndexName: "callId-index",
                KeyConditionExpression: "callId = :id",
                ExpressionAttributeValues: { ":id": callId }
      ...
      case "CALL_ENDED": {
        ...
        if (callRecords && callRecords[0]) {
          ...
          const shouldCleanupMeeting = (status === "queued" || status === "ringing") && !isOutboundCall && meetingInfo?.MeetingId;
          ...
          if (status === "ringing" && agentIds && Array.isArray(agentIds) && agentIds.length > 0) {
            ...
            await Promise.all(agentIds.map(
              (agentId) => ddb.send(new import_lib_dynamodb14.UpdateCommand({
                TableName: AGENT_PRESENCE_TABLE_NAME,
                Key: { agentId },
