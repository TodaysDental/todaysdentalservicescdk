...
const corsHeaders = buildCorsHeaders({ allowHeaders: ['x-api-key'] });
...
async function handleCreate(body: string): Promise<APIGatewayProxyResult> {
...
async function handleUpdate(id: string | undefined, body: string): Promise<APIGatewayProxyResult> {
  if (!id) return json(400, { message: 'Missing id' });
  const payload = safeParseJson(body);
...
async function handleBatchDelete(body: string): Promise<APIGatewayProxyResult> {
  const payload = safeParseJson(body);
