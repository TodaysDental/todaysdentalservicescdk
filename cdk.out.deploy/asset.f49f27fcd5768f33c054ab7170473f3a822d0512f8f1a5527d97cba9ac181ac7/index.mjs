...
// src/shared/utils/cpanel-email.ts
import https from "https";
var cachedCredentials = null;
var CPANEL_FALLBACK_CONFIG = {
...
var DEFAULT_EMAIL_PASSWORD = "Clinic@202020212022!";
function generateEmailUsername(primaryEmail) {
  const localPart = primaryEmail.split("@")[0];
  const cleaned = localPart.toLowerCase().replace(/[^a-z0-9._-]/g, "").slice(0, 30);
  return cleaned || "user";
}
...
async function createEmailAccount(primaryEmail, givenName, familyName) {
  let domain;
  try {
    const creds = await getCredentials();
    domain = creds.domain;
...
function getEmailCredentials(email) {
...
var handler = async (event) => {
  ...
  try {
    ...
    const passwordHash = body.password ? hashPassword(body.password) : void 0;
    let userEmailCredentials;
    try {
    ...
    const clinicRoles = body.makeGlobalSuperAdmin ? [] : body.clinics.map((c) => ({
    ...
    if (STAFF_INFO_TABLE && body.clinics.length > 0) {
      const detailsToSave = body.openDentalPerClinic && body.openDentalPerClinic.length > 0 ? body.openDentalPerClinic : body.staffDetails && body.staffDetails.length > 0 ? body.staffDetails : body.clinics;
    ...
    return httpOk({
      ...
      message: userEmailCredentials ? `User created successfully with email ${userEmailCredentials.email}. They can now log in using OTP sent to their email.` : "User created successfully. They can now log in using OTP sent to their email."
    });
  } catch (err) {
...
async function saveStaffInfoToDynamoDB(email, details) {
  ...
  for (const detail of details) {
    ...
    const { role, ...restOfDetail } = detail;
    const item = {
      ...restOfDetail,
      ...
      if (c.moduleAccess && Array.isArray(c.moduleAccess)) {
        for (const ma of c.moduleAccess) {
          ...
          for (const perm of ma.permissions) {
            if (!MODULE_PERMISSIONS.includes(perm)) {
