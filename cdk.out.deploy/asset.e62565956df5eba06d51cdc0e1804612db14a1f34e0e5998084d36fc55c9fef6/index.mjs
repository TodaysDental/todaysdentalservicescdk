...
import { DynamoDBDocumentClient as DynamoDBDocumentClient2, QueryCommand as QueryCommand2, GetCommand as GetCommand2 } from "@aws-sdk/lib-dynamodb";
...
var dynamodbClient = new DynamoDBClient2({});
...
async function getCallAnalytics(event, userPerms, corsHeaders) {
  ...
  const queryResult = await ddb.send(new QueryCommand2({
...
async function getClinicAnalytics(event, userPerms, corsHeaders) {
  ...
  const queryResult = await ddb.send(new QueryCommand2(queryCommand));
...
async function getAgentAnalytics(event, userPerms, corsHeaders) {
  ...
  if (isAdmin) {
    ...
    const MAX_PAGES = 10;
    do {
      const sampleQuery2 = await ddb.send(new QueryCommand2({
        TableName: ANALYTICS_TABLE_NAME,
  ...
  if (isPaginated && AGENT_PERFORMANCE_TABLE_NAME) {
    try {
      const perfResult = await ddb.send(new QueryCommand2({
