// src/services/admin/me.ts
import { DynamoDBClient as DynamoDBClient2 } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient, GetCommand, QueryCommand as QueryCommand2 } from "@aws-sdk/lib-dynamodb";
...
// src/infrastructure/configs/clinics.ts
var clinics = clinic_config_default;
...
var handler = async (event) => {
  ...
  try {
    ...
    if (isGlobalSuperAdmin || isSuperAdmin) {
      const allClinicIds = isGlobalSuperAdmin ? clinics.map((c) => c.clinicId) : clinicIds;
      ...
      for (const detail of staffDetails) {
        if (!rolesByClinic[detail.clinicId]) {
          rolesByClinic[detail.clinicId] = ["patient coordinator"];
        }
      }
    }
    const userClinicIds = isGlobalSuperAdmin ? clinics.map((c) => c.clinicId) : Array.from(/* @__PURE__ */ new Set([...clinicIds, ...staffDetails.map((d) => d.clinicId)]));
    const clinicIdSet = new Set(userClinicIds);
    const clinics2 = clinics.filter((c) => clinicIdSet.has(c.clinicId)).map((c) => {
      const clinicRole = clinicRoles.find((cr) => cr.clinicId === c.clinicId);
      return {
        ...
        // Clinic's dedicated phone number
        // Role information
        role: clinicRole?.role || "patient coordinator",
        roles: rolesByClinic[c.clinicId] || [],
        // Legacy format
...
function buildAccessLevel(clinicRole, isSuperAdmin, isGlobalSuperAdmin) {
  if (isGlobalSuperAdmin || isSuperAdmin) {
    return {
      isAdmin: true,
      isSuperAdmin: true,
