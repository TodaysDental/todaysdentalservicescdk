// src/services/admin/list-requests.ts
import { DynamoDBClient as DynamoDBClient2 } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient, QueryCommand as QueryCommand2, ScanCommand as ScanCommand2 } from "@aws-sdk/lib-dynamodb";
...
var FAVORS_TABLE_NAME = process.env.FAVORS_TABLE_NAME || "";
var TEAMS_TABLE_NAME = process.env.TEAMS_TABLE_NAME || "";
var ddb = DynamoDBDocumentClient.from(new DynamoDBClient2({ region: REGION }));
var corsHeaders = buildCorsHeaders({ allowMethods: ["OPTIONS", "GET"] });
var handler = async (event) => {
  ...
  try {
    const callerID = event.requestContext.authorizer?.email || event.requestContext.authorizer?.sub;
    if (!callerID) {
      return httpErr(401, "Could not determine authenticated user ID");
    }
    const qs = event.queryStringParameters || {};
    const roleRaw = (qs.role || qs.type || "all").toLowerCase();
    const role = roleRaw === "sent" || roleRaw === "received" || roleRaw === "group" ? roleRaw : "all";
    const limit = qs.limit && !Number.isNaN(parseInt(qs.limit, 10)) ? Math.min(parseInt(qs.limit, 10), 100) : 50;
    ...
    const fetchGroupRequests = async (teamIDs2) => {
      if (teamIDs2.length === 0)
        return [];
      const groupRequestPromises = teamIDs2.map(
    ...
    if (role === "sent") {
      const sentResult2 = await queryByIndex(
        "SenderIndex",
        "senderID",
        callerID,
        exclusiveStartKey
      );
      const items = sentResult2.Items || [];
      return httpOk({
        role: "sent",
        items,
        nextToken: sentResult2.LastEvaluatedKey ? JSON.stringify(sentResult2.LastEvaluatedKey) : void 0
      });
    }
    if (role === "received") {
      const recvResult2 = await queryByIndex(
        "ReceiverIndex",
        "receiverID",
        callerID,
        exclusiveStartKey
      );
      const items = recvResult2.Items || [];
      return httpOk({
        role: "received",
        items,
        nextToken: recvResult2.LastEvaluatedKey ? JSON.stringify(recvResult2.LastEvaluatedKey) : void 0
      });
    }
    if (role === "group") {
      const teamIDs2 = await getUserTeamIDs();
      let groupItems2 = await fetchGroupRequests(teamIDs2);
      groupItems2.sort((a, b) => {
      ...
      return httpOk({
        role: "group",
        items: groupItems2.slice(0, limit),
        nextToken: void 0
      });
    }
