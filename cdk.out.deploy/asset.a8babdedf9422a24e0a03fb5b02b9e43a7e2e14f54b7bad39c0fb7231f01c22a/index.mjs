...
import { DynamoDBDocumentClient, QueryCommand as QueryCommand2, ScanCommand as ScanCommand2, PutCommand, DeleteCommand, UpdateCommand, GetCommand } from "@aws-sdk/lib-dynamodb";
...
// src/services/hr/index.ts
var SHIFTS_TABLE = process.env.SHIFTS_TABLE;
var LEAVE_TABLE = process.env.LEAVE_TABLE;
...
var currentCorsHeaders = corsHeaders;
...
async function isDateBlocked(staffId, date) {
  ...
  return Items.some((leave) => {
    ...
    const endTime = new Date(leave.endDate).getTime() + (24 * 60 * 60 * 1e3 - 1);
    return dateTime >= startTime && dateTime <= endTime;
  });
}
async function getOverlappingShifts(staffId, startDate, endDate) {
  const { Items } = await ddb.send(new QueryCommand2({
...
async function sendShiftNotificationEmail(recipientEmail, shiftDetails, staffName) {
  ...
  const command = new SendEmailCommand({
    ...
    Content: {
      // Content replaces Message in v1
      Simple: {
    ...
    // FromEmailAddress replaces Source in v1
  });
  try {
...
var handler = async (event) => {
  ...
  try {
    ...
    if (method === "GET" && path === "/clinics") {
      const clinicIds = userPerms.clinicRoles.map((cr) => cr.clinicId);
    ...
    if (method === "POST" && path === "/shifts") {
      ...
      return createShift(parsedBody, allowedClinics);
    }
    ...
    if (method === "PUT" && path.match(/^\/shifts\/[^\/]+\/reject$/)) {
      const shiftId = path.split("/")[2];
      return rejectShift(shiftId, userPerms.email);
    }
    ...
    if (method === "POST" && path === "/leave") {
      if (!event.body)
        return httpErr(400, "Missing request body");
      const parsedBody = JSON.parse(event.body);
      return createLeave(userPerms.email, parsedBody);
    }
...
async function getDashboard(userPerms, isAdmin) {
  if (isAdmin) {
    ...
    const staffCountPromise = ddb.send(new ScanCommand2({
    ...
    const shiftQueryPromises = adminClinics.map(
      (clinicId) => ddb.send(new QueryCommand2({
        TableName: SHIFTS_TABLE,
    ...
    const { Items: shifts } = await ddb.send(new QueryCommand2({
      TableName: SHIFTS_TABLE,
      IndexName: "byStaff",
      KeyConditionExpression: "staffId = :staffId",
...
async function getShifts(userPerms, isAdmin, queryParams, allowedClinics) {
  ...
  if (isAdmin && clinicId) {
    ...
    const { Items: Items2 } = await ddb.send(new QueryCommand2({
      TableName: SHIFTS_TABLE,
      IndexName: "byClinicAndDate",
    ...
    return httpOk({ shifts: Items2 || [] });
  }
  let KeyConditionExpression = "staffId = :staffId";
  const ExpressionAttributeValues = { ":staffId": userPerms.email };
  if (startDate && endDate) {
...
async function createShift(body, allowedClinics) {
  ...
  const totalHours = (new Date(endTime).getTime() - new Date(startTime).getTime()) / (1e3 * 60 * 60);
  if (totalHours <= 0) {
    return httpErr(400, "End time must be after start time");
  }
  const pay = totalHours * hourlyRate;
  const shiftId = v4_default();
  const shift = {
    shiftId,
    staffId,
    email,
...
async function updateShift(shiftId, body, allowedClinics) {
...
async function deleteShift(shiftId, allowedClinics) {
...
async function rejectShift(shiftId, staffId) {
  const { Item } = await ddb.send(new GetCommand({ TableName: SHIFTS_TABLE, Key: { shiftId } }));
  if (!Item)
    return httpErr(404, "Shift not found");
  if (Item.staffId !== staffId)
    return httpErr(403, "Forbidden: You do not own this shift");
  if (Item.status !== "scheduled")
    return httpErr(400, "Shift cannot be rejected");
...
async function getLeave(userPerms, isAdmin) {
  if (isAdmin) {
    const { Items } = await ddb.send(new ScanCommand2({ TableName: LEAVE_TABLE }));
...
async function createLeave(staffId, body) {
  const { startDate, endDate } = body;
  if (!startDate || !endDate) {
    return httpErr(400, "startDate and endDate are required");
  }
  const leaveId = v4_default();
  const leaveRequest = {
    leaveId,
    staffId,
    startDate,
...
async function deleteLeave(leaveId, userPerms, isAdmin) {
...
async function approveLeave(leaveId) {
  console.log("\u{1F504} Starting approveLeave for leaveId:", leaveId);
  console.log("\u{1F504} LEAVE_TABLE:", LEAVE_TABLE);
  console.log("\u{1F504} SHIFTS_TABLE:", SHIFTS_TABLE);
  try {
    if (!leaveId || leaveId === "undefined") {
      console.error("\u274C Invalid leaveId:", leaveId);
      return httpErr(400, "Invalid leave ID");
    }
    console.log("\u{1F504} Getting leave request from DynamoDB...");
    ...
    console.log("\u{1F504} Updating leave status to approved...");
    await ddb.send(new UpdateCommand({
      TableName: LEAVE_TABLE,
      Key: { leaveId },
      UpdateExpression: "set #status = :status",
    ...
    if (leave.startDate && leave.endDate) {
      try {
        ...
        console.log(
          `\u{1F4CA} Found ${overlappingShifts.length} overlapping shifts:`,
    ...
    if (overlappingShifts.length > 0) {
      try {
        const cancelPromises = overlappingShifts.map((shift) => {
          console.log("\u{1F504} Cancelling shift:", shift.shiftId);
    ...
    const response = {
      ...
      message: overlappingShifts.length > 0 ? `Leave approved. ${overlappingShifts.length} overlapping shift(s) have been automatically cancelled.` : "Leave approved successfully. No shifts were affected."
    };
