...
// src/services/chime/start-session.ts
var start_session_exports = {};
__export(start_session_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(start_session_exports);
...
function isSuperAdminFromJwt(payload) {
  return payload.isGlobalSuperAdmin === true || payload.isSuperAdmin === true;
}
...
function calculateSessionExpiry() {
  ...
  return {
    ...
    sessionExpiresAt: new Date(sessionExpiresAtEpoch * 1e3).toISOString(),
...
var chimeClient = new import_client_chime_sdk_meetings.ChimeSDKMeetingsClient({ region: CHIME_MEDIA_REGION });
...
var handler = async (event) => {
  const requestOrigin = event.headers && (event.headers.origin || event.headers.Origin) || void 0;
  ...
  try {
    ...
    const meetingId = (0, import_crypto2.randomUUID)();
    ...
    if (enableMediaCapture) {
      createMeetingParams.NotificationsConfiguration = {
        ...
        // Optional: Lambda for meeting events
        SnsTopicArn: void 0,
        // Optional: SNS for meeting events
        SqsQueueArn: void 0
        // Optional: SQS for meeting events
      };
    ...
    const meetingResponse = await chimeClient.send(new import_client_chime_sdk_meetings.CreateMeetingCommand(createMeetingParams));
    if (!meetingResponse.Meeting?.MeetingId) {
      console.error("[start-session] Meeting created but no MeetingId returned", {
        ...
        hasMediaRegion: !!meetingResponse.Meeting?.MediaRegion
      });
      return {
        statusCode: 500,
        headers: corsHeaders,
    ...
    const attendeeResponse = await chimeClient.send(new import_client_chime_sdk_meetings.CreateAttendeeCommand({
      MeetingId: meetingResponse.Meeting.MeetingId,
    ...
    if (!attendeeResponse.Attendee?.AttendeeId) {
      console.error("[start-session] Attendee created but no AttendeeId returned", {
        ...
        hasJoinToken: !!attendeeResponse.Attendee?.JoinToken
      });
      return {
        statusCode: 500,
        headers: corsHeaders,
    ...
    try {
      ...
      if (!activeClinicIds || activeClinicIds.length === 0) {
        ...
        for (const clinicId of activeClinicIds) {
          const { Items: queuedCalls } = await ddb.send(new import_lib_dynamodb3.QueryCommand({
          ...
          if (queuedCalls && queuedCalls.length > 0) {
            ...
            const lock = new DistributedLock(ddb, {
              ...
              ttlSeconds: 10,
              // Longer TTL for session start which may be slower
              maxRetries: 3,
              retryDelayMs: 100
            });
            ...
            let attendeeCreated = null;
            try {
              const { Items: freshCallRecords } = await ddb.send(new import_lib_dynamodb3.QueryCommand({
                TableName: CALL_QUEUE_TABLE_NAME,
              ...
              const meetingId2 = freshCall.meetingInfo.MeetingId;
              try {
                await chimeClient.send(new import_client_chime_sdk_meetings.GetMeetingCommand({ MeetingId: meetingId2 }));
                console.log(`[start-session] Meeting ${meetingId2} validated - exists`);
              } catch (meetingErr) {
                if (meetingErr.name === "NotFoundException") {
                  console.error(`[start-session] Meeting ${meetingId2} no longer exists`);
              ...
              const attendeeResponse2 = await chimeClient.send(new import_client_chime_sdk_meetings.CreateAttendeeCommand({
                MeetingId: meetingId2,
                ExternalUserId: agentId
              }));
              if (!attendeeResponse2.Attendee) {
                console.error("[start-session] Failed to create attendee for queued call");
                continue;
              }
              attendeeCreated = attendeeResponse2.Attendee;
              try {
                await ddb.send(new import_lib_dynamodb3.UpdateCommand({
                  TableName: CALL_QUEUE_TABLE_NAME,
                  Key: {
                ...
                if (claimErr.name === "ConditionalCheckFailedException") {
                  ...
                  if (attendeeCreated?.AttendeeId) {
                    await chimeClient.send(new import_client_chime_sdk_meetings.DeleteAttendeeCommand({
                      MeetingId: meetingId2,
              ...
              await ddb.send(new import_lib_dynamodb3.UpdateCommand({
                TableName: AGENT_PRESENCE_TABLE_NAME,
                Key: { agentId },
                UpdateExpression: "SET ringingCallId = :callId, callStatus = :status, inboundMeetingInfo = :meeting, inboundAttendeeInfo = :attendee, ringingCallTime = :time",
                ExpressionAttributeValues: {
