...
import { DynamoDBDocumentClient, ScanCommand as ScanCommand2, GetCommand, PutCommand, DeleteCommand, QueryCommand as QueryCommand2, BatchWriteCommand } from "@aws-sdk/lib-dynamodb";
...
// src/services/admin/users.ts
var ddbClient = new DynamoDBClient2({});
...
var handler = async (event) => {
  ...
  try {
    ...
    const isSelfRequest = String(event.path || "").endsWith("/users/self") || String(event.resource || "").endsWith("/users/self") || pathUsername === "self";
...
async function handleGetSelf(email) {
...
async function handleListUsers(isGlobalSuperAdmin, isSuperAdmin, allowedClinics) {
  const result = await ddb.send(new ScanCommand2({
    TableName: STAFF_USER_TABLE
  }));
...
async function handleGetUser(username, isGlobalSuperAdmin, isSuperAdmin, allowedClinics) {
  const result = await ddb.send(new GetCommand({
    TableName: STAFF_USER_TABLE,
    Key: { email: username }
  }));
  ...
  const staffDetails = STAFF_INFO_TABLE ? await getStaffInfoFromDynamoDB(user.email) : [];
  const filteredStaffDetails = hasAllAccess ? staffDetails : staffDetails.filter((d) => hasClinicAccess(allowedClinics, String(d.clinicId)));
  return httpOk({
    ...toPublicUser(user),
    staffDetails: filteredStaffDetails,
    rolesByClinic: buildRolesByClinicFromUser(user, filteredStaffDetails)
...
async function handleUpdateUser(username, body, isGlobalSuperAdmin, isSuperAdmin, allowedClinics) {
  const result = await ddb.send(new GetCommand({
    TableName: STAFF_USER_TABLE,
    Key: { email: username }
  }));
  ...
  const updatedUser = {
    ...existingUser,
    ...body.givenName !== void 0 && { givenName: body.givenName },
    ...body.familyName !== void 0 && { familyName: body.familyName },
    ...body.clinicRoles !== void 0 && { clinicRoles: body.clinicRoles },
    ...body.isActive !== void 0 && { isActive: body.isActive },
    ...body.makeGlobalSuperAdmin !== void 0 && {
...
async function handleDeleteUser(username, isGlobalSuperAdmin, isSuperAdmin, allowedClinics) {
  const result = await ddb.send(new GetCommand({
    TableName: STAFF_USER_TABLE,
    Key: { email: username }
  }));
...
async function getStaffInfoFromDynamoDB(email) {
  if (!STAFF_INFO_TABLE)
    return [];
  try {
    const result = await ddb.send(new QueryCommand2({
...
async function saveStaffInfoToDynamoDB(email, details) {
  if (!STAFF_INFO_TABLE || !details.length)
    return;
...
async function deleteStaffInfoFromDynamoDB(email) {
  if (!STAFF_INFO_TABLE)
    return;
  try {
    const result = await ddb.send(new QueryCommand2({
...
function buildRolesByClinicFromUser(user, staffDetails) {
  const rolesByClinic = {};
