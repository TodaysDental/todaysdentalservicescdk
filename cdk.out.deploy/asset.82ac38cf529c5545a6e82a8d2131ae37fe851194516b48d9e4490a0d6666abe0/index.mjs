...
import { DynamoDBDocumentClient, QueryCommand as QueryCommand2 } from "@aws-sdk/lib-dynamodb";
...
async function getLiveCallStatus(clinicId) {
  try {
    const { Items: calls } = await ddb.send(new QueryCommand2({
...
async function getOnlineAgentsCount(clinicId) {
  ...
  try {
    const { Items: agents } = await ddb.send(new QueryCommand2({
