/**
 * RCS Auto-Reply Configuration API
 *
 * Stores per-clinic configuration for AI auto-replies in the unified RCS table.
 *
 * Endpoints (protected by API Gateway authorizer):
 * - GET  /rcs/{clinicId}/ai/auto-reply   -> returns config
 * - PUT  /rcs/{clinicId}/ai/auto-reply   -> updates config (enabled + agentId)
 *
 * Storage (in RCS_MESSAGES_TABLE):
 * - pk: CLINIC#<clinicId>
...
export interface RcsAutoReplyConfig {
  clinicId: string;
  enabled: boolean;
  agentId?: string;
  agentName?: string;
...
async function getStoredConfig(clinicId: string): Promise<RcsAutoReplyConfig> {
...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  if (!RCS_MESSAGES_TABLE) {
    return { statusCode: 500, headers, body: JSON.stringify({ success: false, error: 'RCS_MESSAGES_TABLE not configured' }) };
  }
  ...
  try {
    ...
    if (event.httpMethod === 'PUT') {
      ...
      // If enabling, require a valid prepared agent id (or allow clearing and relying on tag-based default?)
      ...
      const config: RcsAutoReplyConfig = {
        clinicId,
        enabled: body.enabled,
        agentId: agentId || undefined,
        agentName,
    ...
    console.error('[RcsAutoReplyConfig] Error:', err);
