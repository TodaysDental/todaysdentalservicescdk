...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    try {
        ...
        // Push notification: alert supervisors when an agent goes offline (opt-in)
        if (isPushNotificationsEnabled() && CHIME_CONFIG.PUSH.ENABLE_SESSION_OFFLINE_ALERT && Item) {
            const agentClinicIds = Item.activeClinicIds as string[] | undefined;
            if (agentClinicIds && agentClinicIds.length > 0) {
                // Send to first active clinic (agent's primary clinic)
                sendClinicAlert(
                    agentClinicIds[0],
                    'Agent Offline',
