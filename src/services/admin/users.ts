...
export const handler = async (event: any) => {
  ...
  try {
    // ===== NEW: GET /users/self (Staff can fetch their own details) =====
    if (event.httpMethod === 'GET' && (event.path === '/users/self' || event.resource === '/users/self')) {
      // Get current user's details using the 'sub' from the token (caller's own UUID)
      const currentUsername = caller.staffId; // This is the staff member's UUID
      
      console.log('Staff self-lookup for:', currentUsername);
      ...
      try {
        ...
        console.log('Returning staff self details:', {
