...
//     // ✅ 2. Add this code block to upload your files
//     new s3deploy.BucketDeployment(this, 'DeployHoldMusic', {
//       sources: [
//         // Point this to the local folder you created in Step 2
//         s3deploy.Source.asset(path.join(__dirname, '..', '..', '..', 'assets', 'audio'))
//       ],
//       destinationBucket: holdMusicBucket,
//       // Optional: clears old files from the bucket before uploading new ones
//       prune: false, 
//     });
...
//             // ✅ ADD THIS PROPERTY
//             TerminationHosts: [
//               voiceConnector.getResponseField('VoiceConnector.OutboundHostName')
//             ],
//             // This list is for PSTN calls
//             CidrAllowedList: [
//               '52.0.0.0/27',
//               '54.0.0.0/27',
//               '3.0.0.0/27',
...
//               '3.0.0.0/27',
//               '18.0.0.0/27',
//               '34.0.0.0/27' // Also align this list
//             ]
//           }
//         },
//         physicalResourceId: customResources.PhysicalResourceId.of(`${this.stackName}-voice-connector-termination`),
...
//     // Configure Voice Connector Origination to route all inbound calls
//     // This is the missing piece: it tells the VC to send all inbound
//     // calls to its own hostname, where the SIP rule can catch it.
...
//           Origination: {
//             Routes: [{
//               // Route to the VC's own hostname, which the SIP rule is waiting for
//               Host: voiceConnector.getResponseField('VoiceConnector.OutboundHostName'),
//               Port: 5060,
//               Protocol: 'UDP',
//               Priority: 1,
//               Weight: 1,
...
//          parameters: {
//            VoiceConnectorId: voiceConnector.getResponseField('VoiceConnector.VoiceConnectorId'),
//            // ✅ FIX: Pass an empty Origination object to clear it
//            Origination: {} 
//          },
//          ignoreErrorCodesMatching: '.*NotFound.*|.*ResourceNotFound.*',
//       },
...
//     // ✅ ADD THIS LINE TO FIX THE RACE CONDITION
...
export class ChimeStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: ChimeStackProps) {
    ...
    // ✅ NEW: This Lambda uses 'create-sip-rules.ts' to create one rule per phone number
    const sipRuleManagerFn = new lambdaNode.NodejsFunction(this, 'SipRuleManagerFn', {
      functionName: `${this.stackName}-SipRuleManager`,
      entry: path.join(__dirname, '..', '..', 'services', 'chime', 'create-sip-rules.ts'), //
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      timeout: Duration.minutes(5), // Give it time to create/delete many rules
      bundling: {
        // Ensure clinics.json is bundled with the Lambda
        externalModules: [],
      },
      environment: {},
    });

    // Grant the Lambda permission to manage SIP rules
    sipRuleManagerFn.addToRolePolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: [
        'chime:CreateSipRule',
        'chime:DeleteSipRule',
      ...
      resources: ['*'], // SIP rules don't have resource-specific ARNs for creation
    }));

    // ✅ NEW: This Custom Resource invokes the Lambda to manage the rules
    const sipRuleCustomResource = new customResources.AwsCustomResource(this, 'SipRuleCustomResource', {
      onUpdate: { // Also run on 'onCreate' and 'onDelete'
        service: 'Lambda',
        action: 'invoke',
        parameters: {
          FunctionName: sipRuleManagerFn.functionName,
          InvocationType: 'RequestResponse',
          // Pass useful info to the Lambda as the Payload. Use a JSON string so the Lambda
          // receives an event-like object: { SipMediaApplicationId, StackName }
          Payload: JSON.stringify({
            SipMediaApplicationId: sipMediaApp.getResponseField('SipMediaApplication.SipMediaApplicationId'),
            StackName: this.stackName,
          }),
        },
        physicalResourceId: customResources.PhysicalResourceId.of(`SipRuleManager-${this.stackName}`),
      },
      policy: customResources.AwsCustomResourcePolicy.fromSdkCalls({
        resources: [sipRuleManagerFn.functionArn],
      }),
    });

    // Ensure the Lambda is created before the custom resource
    sipRuleCustomResource.node.addDependency(sipRuleManagerFn);
    // Ensure the SMA is created before the rules are
    sipRuleCustomResource.node.addDependency(sipMediaApp);
