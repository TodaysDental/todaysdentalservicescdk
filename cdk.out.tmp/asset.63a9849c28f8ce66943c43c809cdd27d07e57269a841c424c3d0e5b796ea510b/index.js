...
// src/services/ai-agents/outbound-call-scheduler.ts
var outbound_call_scheduler_exports = {};
__export(outbound_call_scheduler_exports, {
  executeOutboundCall: () => executeOutboundCall,
  handler: () => handler
});
module.exports = __toCommonJS(outbound_call_scheduler_exports);
...
async function isAiOutboundEnabled(clinicId) {
...
// src/services/ai-agents/outbound-call-scheduler.ts
var dynamoClient2 = new import_client_dynamodb3.DynamoDBClient({});
...
var OUTBOUND_CONTACT_FLOW_ID = process.env.OUTBOUND_CONTACT_FLOW_ID || "";
var connectClient = CONNECT_INSTANCE_ID ? new import_client_connect.ConnectClient({}) : null;
var OUTBOUND_CALL_QUEUE_URL = process.env.OUTBOUND_CALL_QUEUE_URL || "";
...
var BULK_SCHEDULE_MAX_CALLS = parseInt(process.env.BULK_SCHEDULE_MAX_CALLS || "500", 10);
var BULK_SCHEDULE_BATCH_SIZE = parseInt(process.env.BULK_SCHEDULE_BATCH_SIZE || "25", 10);
...
var RETRY_CONFIG = {
  BASE_DELAY_MS: 5 * 60 * 1e3,
  // 5 minutes base delay
  MAX_DELAY_MS: 60 * 60 * 1e3,
  // 1 hour maximum delay
...
var BLOCKED_PREFIXES = [
  ...
  // US Premium rate
  "1900",
  // US Premium rate (with country code)
  "1976",
  // US Premium rate (with country code)
  "0900",
...
var VALID_COUNTRY_CODE_PREFIXES = ["1", "44", "61", "52", "33", "49", "39", "34", "81", "86", "91"];
function validatePhoneNumber(phoneNumber) {
  ...
  const hasValidCountryCode = VALID_COUNTRY_CODE_PREFIXES.some(
    (prefix) => digitsOnly.startsWith(prefix)
  );
...
async function listScheduledCalls(event, userPerms) {
  ...
  let expressionAttributeValues = { ":cid": clinicId };
  if (status) {
...
function generateIdempotencyKey(clinicId, phoneNumber, scheduledTime, purpose) {
  const normalizedPhone = phoneNumber.replace(/\D/g, "");
  const normalizedTime = new Date(scheduledTime).toISOString().slice(0, 16);
  return `${clinicId}:${normalizedPhone}:${normalizedTime}:${purpose}`;
}
async function createScheduledCall(event, userPerms) {
  ...
  const agentResponse = await docClient2.send(new import_lib_dynamodb2.GetCommand({
  ...
  const callId = `${idempotencyKey.slice(0, 16)}-${v4_default().slice(0, 8)}`;
  ...
  try {
    const existingCallsResponse = await docClient2.send(new import_lib_dynamodb2.QueryCommand({
      TableName: SCHEDULED_CALLS_TABLE,
      IndexName: "ClinicIndex",
      KeyConditionExpression: "clinicId = :cid",
...
async function getScheduledCall(event, userPerms, callId) {
...
async function cancelScheduledCall(event, userPerms, callId) {
  const response = await docClient2.send(new import_lib_dynamodb2.GetCommand({
    TableName: SCHEDULED_CALLS_TABLE,
    Key: { callId }
  }));
  ...
  for (const scheduleName of schedulesToDelete) {
    if (!scheduleName)
      continue;
    try {
      await schedulerClient.send(new import_client_scheduler.DeleteScheduleCommand({
        Name: scheduleName
      }));
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      callId,
      scheduleCleanup: deleteErrors.length > 0 ? { warning: "Some schedules could not be deleted", errors: deleteErrors } : { success: true }
    })
  };
}
async function sendNow(event, userPerms) {
  const body = JSON.parse(event.body || "{}");
  ...
  const callId = `call-now-${v4_default().slice(0, 8)}`;
...
async function createSingleScheduledCall(request, createdBy, validationContext) {
...
async function bulkScheduleCalls(event, userPerms) {
  const body = JSON.parse(event.body || "{}");
...
var executeOutboundCall = async (event) => {
  ...
  if (isCleanupCheck) {
    ...
    const callResponse = await docClient2.send(new import_lib_dynamodb2.GetCommand({
      TableName: SCHEDULED_CALLS_TABLE,
      Key: { callId }
    }));
    ...
    if (call.status === "in_progress" && call.chimeTransactionId === "UNKNOWN_TXN_ID") {
  ...
  const scheduledCallResponse = await docClient2.send(new import_lib_dynamodb2.GetCommand({
    TableName: SCHEDULED_CALLS_TABLE,
    Key: { callId }
  }));
  const scheduledCall = scheduledCallResponse.Item;
  if (!scheduledCall) {
  ...
  const executionId = `exec-${v4_default().slice(0, 12)}`;
  const executionStartTime = (/* @__PURE__ */ new Date()).toISOString();
  try {
    await docClient2.send(new import_lib_dynamodb2.UpdateCommand({
      TableName: SCHEDULED_CALLS_TABLE,
      Key: { callId },
      ...
      ConditionExpression: isRetry ? "(attribute_not_exists(executionId) OR executionId = :empty OR executionStartedAt < :staleTime) AND attempts = :expectedAttempts" : "attribute_not_exists(executionId) OR executionId = :empty OR executionStartedAt < :staleTime",
      ExpressionAttributeValues: {
        ":execId": executionId,
        ":now": executionStartTime,
        ":empty": "",
        // FIX: Consider execution stale after 5 minutes (Lambda timeout is typically 30s-5min)
        ":staleTime": new Date(Date.now() - 5 * 60 * 1e3).toISOString(),
  ...
  try {
    const clinicResponse = await docClient2.send(new import_lib_dynamodb2.GetCommand({
    ...
    const callResponse = await chimeVoiceClient.send(new import_client_chime_sdk_voice.CreateSipMediaApplicationCallCommand(callCommandInput));
    ...
    if (!transactionId) {
      ...
      const uncertainTimeout = new Date(Date.now() + 5 * 60 * 1e3).toISOString();
      await docClient2.send(new import_lib_dynamodb2.UpdateCommand({
        TableName: SCHEDULED_CALLS_TABLE,
        Key: { callId },
        ...
        ExpressionAttributeValues: {
          ":status": "in_progress",
          // Keep as in_progress - call may be ringing
          ":unknown": "UNKNOWN_TXN_ID",
          ":now": (/* @__PURE__ */ new Date()).toISOString(),
          ":warning": "Chime did not return TransactionId - call status uncertain",
          ":timeout": uncertainTimeout,
          ":empty": ""
          // Clear executionId so cleanup can mark as failed
        }
      }));
      try {
        const cleanupSchedulerName = `outbound-cleanup-${callId}`;
        await schedulerClient.send(new import_client_scheduler.CreateScheduleCommand({
          ...
          Target: {
            ...
            Input: JSON.stringify({
              callId,
              isCleanupCheck: true
              // Signals this is a timeout cleanup, not a new call
            })
          }
        }));
      ...
      return {
        success: true,
        // Tentatively successful - Chime accepted the call
        callId,
...
async function asyncBulkScheduleCalls(event, userPerms) {
  ...
  const bulkJob = {
    ...
    ttl: Math.floor(now.getTime() / 1e3) + 30 * 24 * 60 * 60
    // 30 days
  };
  await docClient2.send(new import_lib_dynamodb2.PutCommand({
  ...
  for (let i = 0; i < body.calls.length; i += BATCH_SIZE) {
    ...
    await sqsClient.send(new import_client_sqs.SendMessageCommand({
      ...
      MessageGroupId: body.clinicId
      // FIFO ordering by clinic (if FIFO queue)
    }));
  }
  await docClient2.send(new import_lib_dynamodb2.UpdateCommand({
    TableName: BULK_OUTBOUND_JOBS_TABLE,
...
async function listBulkJobs(event, userPerms) {
  const clinicId = event.queryStringParameters?.clinicId;
  const status = event.queryStringParameters?.status;
  if (!clinicId) {
    return {
  ...
  const queryParams = {
    ...
    KeyConditionExpression: status ? "clinicId = :clinicId AND #status = :status" : "clinicId = :clinicId",
    ExpressionAttributeValues: {
      ":clinicId": clinicId,
  ...
  const response = await docClient2.send(new import_lib_dynamodb2.QueryCommand(queryParams));
  return {
    statusCode: 200,
    headers: getCorsHeaders(event),
    body: JSON.stringify({
...
async function getBulkJob(event, userPerms, jobId) {
  const response = await docClient2.send(new import_lib_dynamodb2.GetCommand({
    TableName: BULK_OUTBOUND_JOBS_TABLE,
    Key: { jobId }
  }));
  ...
  const progress = job.totalCalls > 0 ? Math.round(job.processedCalls / job.totalCalls * 100) : 0;
  return {
    statusCode: 200,
    headers: getCorsHeaders(event),
    body: JSON.stringify({
      ...job,
      progress: `${progress}%`,
      successRate: job.processedCalls > 0 ? `${Math.round(job.successfulCalls / job.processedCalls * 100)}%` : "N/A"
    })
  };
}
