...
async function startFavorRequest(senderID, payload, senderConnectionId, apiGwManagement) {
  ...
  if (isGroupRequest) {
    ...
    const convos = existingConvos.Items || [];
    existingFavor = convos.find((c) => c.isMainGroupChat === true) || convos.find((c) => c.status !== "completed" && c.status !== "rejected") || convos[0];
  } else {
    const [sentResult, recvResult] = await Promise.all([
      ddb3.send(new QueryCommand3({
        TableName: FAVORS_TABLE3,
    ...
    existingFavor = allConvos.find(
      (c) => c.status !== "completed" && c.status !== "rejected" && (c.senderID === senderID && c.receiverID === receiverID || c.senderID === receiverID && c.receiverID === senderID)
    );
  }
  if (existingFavor) {
    ...
    const updateExprParts = ["updatedAt = :ua"];
    const exprValues = { ":ua": (/* @__PURE__ */ new Date()).toISOString() };
    const exprNames = {};
    if (deadline) {
      updateExprParts.push("deadline = :dl");
      exprValues[":dl"] = String(deadline);
    }
    if (priority && priority !== "Medium") {
      updateExprParts.push("priority = :pri");
      exprValues[":pri"] = priority;
    }
    ...
    await _saveAndBroadcastMessage(messageData2, apiGwManagement, isGroupRequest ? recipients : void 0);
    const allParticipants = [...recipients, senderID];
    await sendToAll(apiGwManagement, allParticipants, {
      type: "favorRequestUpdated",
      favor: updatedFavor
    }, { notifyOffline: false });
    const senderDetails2 = await getUserDetails(senderID);
    await sendTaskPushNotification(
      recipients,
      "task_assigned",
      title || initialMessage.substring(0, 50),
      senderDetails2.fullName,
      existingID,
      { priority, category }
    );
