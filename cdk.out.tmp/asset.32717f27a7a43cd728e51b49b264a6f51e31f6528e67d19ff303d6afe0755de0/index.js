...
// src/services/marketing/messages.ts
...
async function ayrshareGetMessages(apiKey, profileKey, platform, limit = 20) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/messages`, {
      headers: {
...
async function ayrshareSendMessage(apiKey, profileKey, platform, recipientId, message) {
  try {
    const res = await axios_default.post(
      `${AYRSHARE_URL}/messages/send`,
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/messages") && method === "GET") {
      ...
      const messages = (Array.isArray(result) ? result : result.messages || []).map((msg) => ({
        id: msg.id,
