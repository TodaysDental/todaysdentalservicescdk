...
// src/services/rcs/rcs-ai-handler.ts
var rcs_ai_handler_exports = {};
__export(rcs_ai_handler_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(rcs_ai_handler_exports);
...
async function checkRcsConfig(clinicId, corsHeaders) {
  try {
    const secretsResult = await ddb.send(new import_lib_dynamodb.GetCommand({
      TableName: CLINIC_SECRETS_TABLE2,
      Key: { clinicId }
    }));
    const configResult = await ddb.send(new import_lib_dynamodb.GetCommand({
      TableName: CLINIC_CONFIG_TABLE2,
      Key: { clinicId }
    }));
    ...
    const status = {
      clinicId,
      isConfigured,
      rcsSenderId: rcsSenderId || void 0,
      messagingServiceSid: messagingServiceSid || void 0,
      clinicName: clinicName || void 0
    };
    if (!isConfigured) {
...
async function generateTemplate(event, clinicId, corsHeaders) {
...
Generate a compelling ${templateFormat === "carousel" ? "carousel with multiple cards" : "rich card"} for this message type.`;
  try {
    const systemPrompt = templateFormat === "carousel" ? SYSTEM_PROMPTS.CAROUSEL_TEMPLATE : SYSTEM_PROMPTS.RICH_CARD_TEMPLATE;
...
async function generateMessageBody(event, clinicId, corsHeaders) {
  const body = JSON.parse(event.body || "{}");
