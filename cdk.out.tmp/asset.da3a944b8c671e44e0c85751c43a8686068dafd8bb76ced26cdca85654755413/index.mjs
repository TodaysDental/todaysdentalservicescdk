...
async function getCallAnalytics(event, userPerms, corsHeaders) {
  ...
  if (allRecords.length > 1) {
    ...
    for (let i = 1; i < allRecords.length; i++) {
      ...
      for (const [field, defaultVal] of Object.entries(sentinelDefaults)) {
        ...
        const currentIsDefault = currentVal === void 0 || currentVal === null || currentVal === defaultVal || Array.isArray(currentVal) && currentVal.length === 0 || typeof currentVal === "object" && !Array.isArray(currentVal) && currentVal !== null && Object.keys(currentVal).length === 0;
        if (currentIsDefault && olderVal !== void 0 && olderVal !== null && olderVal !== defaultVal) {
  ...
  const hasTranscriptNow = typeof analytics.fullTranscript === "string" && analytics.fullTranscript.trim().length > 0 || Array.isArray(analytics.latestTranscripts) && analytics.latestTranscripts.length > 0;
  ...
  if (RECORDING_METADATA_TABLE && (!hasTranscriptNow || !hasSentimentNow)) {
    try {
      ...
      if (recordings.length > 0) {
        const best = recordings.reduce((a, b) => {
