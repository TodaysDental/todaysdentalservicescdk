...
// src/integrations/communication/voice-call-analytics-api.ts
var voice_call_analytics_api_exports = {};
__export(voice_call_analytics_api_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(voice_call_analytics_api_exports);
...
var getCors = (event) => buildCorsHeaders({}, event.headers?.origin);
...
function isoDaysAgo(days) {
  const d = new Date(Date.now() - days * 24 * 60 * 60 * 1e3);
...
async function handleDashboard(event, allowedClinics) {
  ...
  if (!clinicId)
    return http(400, { error: "clinicId is required" }, event);
  ...
  const result = await ddb.send(
    new import_lib_dynamodb.QueryCommand({
      ...
      Limit: 1e3
      // best-effort snapshot (keeps Lambda fast)
    })
  );
  ...
  const answerRate = totals.total > 0 ? Math.round(totals.answered / totals.total * 1e4) / 100 : 0;
  const completionRate = totals.total > 0 ? Math.round(totals.completed / totals.total * 1e4) / 100 : 0;
  const failureRate = totals.total > 0 ? Math.round(totals.failed / totals.total * 1e4) / 100 : 0;
...
async function handleListCalls(event, allowedClinics) {
  const q = event.queryStringParameters || {};
  const clinicId = String(q.clinicId || "").trim();
  const status = q.status ? String(q.status).trim().toUpperCase() : void 0;
  ...
  const token = res.LastEvaluatedKey ? encodeToken(res.LastEvaluatedKey) : void 0;
  return http(
    200,
    {
      success: true,
...
async function handleGetCall(event, allowedClinics) {
  const callId = event.pathParameters?.callId || (event.path || "").split("/").pop() || "";
  const id = String(callId).trim();
  if (!id)
    return http(400, { error: "callId is required" }, event);
  const res = await ddb.send(
