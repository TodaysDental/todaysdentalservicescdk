...
// src/services/chime/join-active-call.ts
var join_active_call_exports = {};
__export(join_active_call_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(join_active_call_exports);
...
var handler = async (event) => {
  ...
  try {
    ...
    const lock = new DistributedLock(ddb, {
      ...
      lockKey: `supervisor-join-call-${callId}`,
      // FIX: Lock on callId only, not callId+supervisorId
      ttlSeconds: 10,
      maxRetries: 3,
      retryDelayMs: 100
    });
    ...
    try {
      const { Items: freshCallRecords } = await ddb.send(new import_lib_dynamodb3.QueryCommand({
        ...
        // GSI doesn't support consistent read, but lock prevents races
      }));
      ...
      const alreadyJoined = existingSupervisors.some((s) => s.supervisorId === supervisorId);
      if (alreadyJoined) {
        return {
          statusCode: 400,
          headers: corsHeaders,
          body: JSON.stringify({
            ...
            existingMode: existingSupervisors.find((s) => s.supervisorId === supervisorId)?.mode
          })
        };
      }
      ...
      try {
        await chime.send(new import_client_chime_sdk_meetings.GetMeetingCommand({ MeetingId: freshMeetingId }));
      ...
      const supervisorAttendeeResponse = await chime.send(new import_client_chime_sdk_meetings.CreateAttendeeCommand({
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          callDetails: {
            ...
            duration: freshCallRecord.connectedAt ? Math.floor((now - freshCallRecord.connectedAt) / 1e3) : 0
          },
          instructions: {
            silent: mode === "silent" ? "You are in listen-only mode. Mute your microphone." : void 0,
            barge: mode === "barge" ? "You can speak. All parties will hear you." : void 0,
            whisper: mode === "whisper" ? "Coach mode - only agent will hear you (coming soon)" : void 0
          }
        })
      };
    } finally {
