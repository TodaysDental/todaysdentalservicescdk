...
// src/services/clinic/costCrud.ts
var costCrud_exports = {};
__export(costCrud_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(costCrud_exports);
...
var TABLE = process.env.CLINIC_COST_TABLE || "ClinicCostOfOperation";
...
async function getAllCosts(event, userPerms) {
  const resp = await ddb.send(new import_lib_dynamodb.ScanCommand({
    TableName: TABLE,
    Limit: 200
  }));
  return ok({ items: resp.Items || [] }, event);
}
async function getCost(event, userPerms, clinicName) {
  if (!clinicName)
    return err(400, "clinicName required", event);
...
async function updateCost(event, userPerms, clinicName) {
  if (!clinicName)
    return err(400, "clinicName required", event);
  const decodedClinicName = decodeURIComponent(clinicName);
