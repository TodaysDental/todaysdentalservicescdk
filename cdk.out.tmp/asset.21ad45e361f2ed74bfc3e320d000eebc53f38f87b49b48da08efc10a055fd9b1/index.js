...
// src/services/chime/conference-call.ts
var conference_call_exports = {};
__export(conference_call_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(conference_call_exports);
...
async function sendAgentAlert(agentUserIds, title, message2, alertData) {
...
async function mergeCallsIntoConference(agentId, body, corsHeaders, payload) {
  ...
  const [primaryCallResult, secondaryCallResult] = await Promise.all([
    ddb.send(new import_lib_dynamodb3.QueryCommand({
  ...
  const primaryAuthz = checkClinicAuthorization(payload, primaryCall.clinicId);
  const secondaryAuthz = checkClinicAuthorization(payload, secondaryCall.clinicId);
  ...
  const conferenceId = (0, import_crypto.randomUUID)();
  try {
    const [primaryMergeResult, secondaryMergeResult] = await Promise.all([
      chimeVoice.send(new import_client_chime_sdk_voice.UpdateSipMediaApplicationCallCommand({
...
async function addParticipantToConference(agentId, body, corsHeaders, payload) {
  const { Item: agentPresence } = await ddb.send(new import_lib_dynamodb3.GetCommand({
    TableName: AGENT_PRESENCE_TABLE_NAME,
    Key: { agentId }
  }));
...
async function removeParticipantFromConference(agentId, body, corsHeaders, payload) {
  ...
  const currentConferenceCallIds = agentPresence.conferenceCallIds || [];
  const updatedCallIds = currentConferenceCallIds.filter((id) => id !== body.callIdToRemove);
...
async function endConference(agentId, body, corsHeaders, payload) {
  const { Item: agentPresence } = await ddb.send(new import_lib_dynamodb3.GetCommand({
    TableName: AGENT_PRESENCE_TABLE_NAME,
    Key: { agentId }
  }));
  ...
  const endPromises = conferenceCallIds.map(async (callId) => {
    try {
      const { Items: callRecords } = await ddb.send(new import_lib_dynamodb3.QueryCommand({
        TableName: CALL_QUEUE_TABLE_NAME,
        IndexName: "callId-index",
