...
// src/services/chime/agent-active.ts
var agent_active_exports = {};
__export(agent_active_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(agent_active_exports);
...
async function listActiveAgentsForClinic(clinicId) {
  ...
  const agentIds = (Items || []).map((i) => i?.agentId).filter((v) => typeof v === "string" && v.length > 0);
  return Array.from(new Set(agentIds));
}
async function findNextQueuedCallForClinic(clinicId) {
  ...
  const queued = (Items || []).filter((i) => i && i.callId);
  if (queued.length === 0)
    return null;
  queued.sort((a, b) => {
...
async function ringQueuedCallForClinic(clinicId) {
  if (!LOCKS_TABLE_NAME) {
  ...
  try {
    ...
    if (isPushNotificationsEnabled()) {
      try {
        const callerPhoneNumber = typeof call.phoneNumber === "string" ? call.phoneNumber : void 0;
...
var handler = async (event) => {
  ...
  const cleanedClinicIds = clinicIds.map((c) => String(c || "").trim()).filter((c) => c.length > 0);
  ...
  await Promise.allSettled(cleanedClinicIds.map(async (clinicId) => {
  ...
  const ringResults = await Promise.allSettled(
    cleanedClinicIds.map((clinicId) => ringQueuedCallForClinic(clinicId))
  );
  return http(200, {
    ...
    dispatch: ringResults.map((r, idx) => ({
      ...
      ...r.status === "fulfilled" ? r.value : { error: String(r.reason?.message || r.reason || "dispatch_failed") }
    }))
  }, event);
};
