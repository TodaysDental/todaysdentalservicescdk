...
async function storeAnalyticsWithDedup(analytics, eventId) {
  ...
  try {
    let preservedFields = {};
    try {
      const existingQuery = await ddb.send(new import_lib_dynamodb4.QueryCommand({
        TableName: ANALYTICS_TABLE,
      ...
      if (Object.keys(preservedFields).length > 0) {
        console.log(
          `[AnalyticsStream] Preserving ${Object.keys(preservedFields).length} fields from existing records:`,
