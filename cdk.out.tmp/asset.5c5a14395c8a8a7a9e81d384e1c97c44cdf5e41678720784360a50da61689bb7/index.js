...
// src/services/marketing/ads.ts
var ads_exports = {};
__export(ads_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(ads_exports);
...
async function ayrshareBoostPost(apiKey, profileKey, params) {
  try {
    const res = await axios_default.post(
      `${AYRSHARE_URL}/ads/boost`,
      {
        id: params.postId,
...
async function ayrshareCreateAdCampaign(apiKey, profileKey, params) {
  try {
    const res = await axios_default.post(
      `${AYRSHARE_URL}/ads/campaign`,
      {
        name: params.name,
...
async function ayrshareGetAdCampaigns(apiKey, profileKey, params) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/ads/campaigns`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
async function ayrshareGetAdCampaign(apiKey, profileKey, campaignId) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/ads/campaign`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
async function ayrshareUpdateAdCampaign(apiKey, profileKey, campaignId, updates) {
  try {
    const res = await axios_default.put(
      `${AYRSHARE_URL}/ads/campaign`,
      {
        id: campaignId,
        ...updates
...
async function ayrshareDeleteAdCampaign(apiKey, profileKey, campaignId) {
  try {
    const res = await axios_default.delete(`${AYRSHARE_URL}/ads/campaign`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey,
...
async function ayrshareGetAdAnalytics(apiKey, profileKey, params) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/ads/analytics`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      },
...
async function ayrshareGetAdAccount(apiKey, profileKey) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/ads/account`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      }
...
async function ayrshareCreateLeadForm(apiKey, profileKey, params) {
  try {
    const res = await axios_default.post(
      `${AYRSHARE_URL}/ads/leadgen/form`,
      {
        name: params.name,
...
async function ayrshareGetLeadForms(apiKey, profileKey) {
  try {
    const res = await axios_default.get(`${AYRSHARE_URL}/ads/leadgen/forms`, {
      headers: {
        "Authorization": `Bearer ${apiKey}`,
        "Profile-Key": profileKey
      }
...
async function ayrshareCreateFullCampaign(apiKey, profileKey, params) {
  try {
    console.log(`Creating campaign: ${params.campaignName}`);
    const campaignRes = await axios_default.post(
      `${AYRSHARE_URL}/ads/campaign`,
      {
        name: params.campaignName,
        objective: params.objective,
        status: "PAUSED",
        // Start paused for Meta review
        specialAdCategories: [],
        // Required for some objectives
    ...
    const adSetRes = await axios_default.post(
      `${AYRSHARE_URL}/ads/adset`,
      {
        campaignId,
    ...
    if (destinationUrl && params.tracking) {
      ...
      if (params.tracking.utmSource)
        utmParams.set("utm_source", params.tracking.utmSource);
      if (params.tracking.utmMedium)
        utmParams.set("utm_medium", params.tracking.utmMedium);
      if (params.tracking.utmCampaign)
        utmParams.set("utm_campaign", params.tracking.utmCampaign);
      if (params.tracking.utmContent)
        utmParams.set("utm_content", params.tracking.utmContent);
      const separator = destinationUrl.includes("?") ? "&" : "?";
      destinationUrl = `${destinationUrl}${separator}${utmParams.toString()}`;
    }
    const adRes = await axios_default.post(
      `${AYRSHARE_URL}/ads/ad`,
      {
        adSetId,
        name: params.campaignName,
          ...
          objectStorySpec: {
            ...
            linkData: {
              ...
              callToAction: {
                ...
                value: params.destination.type === "LEAD_FORM" ? { leadGenFormId: params.destination.leadFormId } : params.destination.type === "PHONE" ? { phoneNumber: params.destination.phoneNumber } : { link: destinationUrl }
              }
            }
          }
        },
...
var PROFILES_TABLE = process.env.MARKETING_PROFILES_TABLE;
var POSTS_TABLE = process.env.MARKETING_POSTS_TABLE;
var API_KEY = process.env.AYRSHARE_API_KEY;
var META_AD_DRAFTS_TABLE = process.env.META_AD_DRAFTS_TABLE || "MetaAdDrafts";
var META_LEAD_FORMS_TABLE = process.env.META_LEAD_FORMS_TABLE || "MetaLeadForms";
var META_LEADS_TABLE = process.env.META_LEADS_TABLE || "MetaLeads";
var META_BULK_JOBS_TABLE = process.env.META_BULK_JOBS_TABLE || "MetaBulkJobs";
var META_BULK_RESULTS_TABLE = process.env.META_BULK_RESULTS_TABLE || "MetaBulkResults";
var META_SCHEDULED_CAMPAIGNS_TABLE = process.env.META_SCHEDULED_CAMPAIGNS_TABLE || "MetaScheduledCampaigns";
var META_CAMPAIGNS_TABLE = process.env.META_CAMPAIGNS_TABLE || "MetaAdCampaigns";
var BULK_PROCESSOR_QUEUE_URL = process.env.BULK_PROCESSOR_QUEUE_URL || "";
function generateId(prefix) {
  return `${prefix}_${v4_default().replace(/-/g, "").substring(0, 12)}`;
}
async function getProfileKey(clinicId) {
  ...
  try {
    ...
    if (path.match(/\/campaigns\/[^\/]+$/) && method === "PUT") {
      ...
      const result = await ayrshareUpdateAdCampaign(API_KEY, profileKey, campaignId, {
        status,
        budget: budget ? Number(budget) : void 0,
        endDate
      });
      return {
        statusCode: 200,
    ...
    if (path.match(/\/meta\/ads\/drafts\/[^\/]+$/) && method === "PUT") {
      ...
      await ddb.send(new import_lib_dynamodb.UpdateCommand({
        ...
        ExpressionAttributeNames: body.data !== void 0 ? { "#data": "data" } : void 0
      }));
      return {
        statusCode: 200,
        headers: corsHeaders,
    ...
    if (path.match(/\/meta\/ads\/[^\/]+\/lead-forms\/?$/) && method === "GET") {
      ...
      const dbResult = await ddb.send(new import_lib_dynamodb.QueryCommand({
    ...
    if (path.match(/\/meta\/ads\/[^\/]+\/leads\/?$/) && method === "GET") {
      ...
      let expressionValues = { ":clinicId": clinicId };
      if (status) {
      ...
      const result = await ddb.send(new import_lib_dynamodb.QueryCommand({
        ...
        ExpressionAttributeValues: expressionValues,
        ExpressionAttributeNames: status ? { "#status": "status" } : void 0,
        ScanIndexForward: false,
        Limit: limit
      }));
      return {
    ...
    if (path.match(/\/meta\/ads\/[^\/]+\/campaigns\/publish/) && method === "POST") {
      ...
      const campaign = {
        ...
        budgetType,
        dailyBudget: Number(dailyBudget),
        totalBudget: body.endDate ? Math.ceil((new Date(body.endDate).getTime() - new Date(startDate).getTime()) / (1e3 * 60 * 60 * 24)) * Number(dailyBudget) : null,
        startDate,
        endDate: body.endDate,
        status: "PENDING_REVIEW",
        targeting,
    ...
    if (path.match(/\/meta\/ads\/bulk\/jobs\/[^\/]+$/) && method === "GET") {
      ...
      const jobResult = await ddb.send(new import_lib_dynamodb.GetCommand({
        TableName: META_BULK_JOBS_TABLE,
        Key: { bulkJobId }
      }));
      if (!jobResult.Item) {
      ...
      const resultsResult = await ddb.send(new import_lib_dynamodb.QueryCommand({
      ...
      const progressPercentage = job.totalClinics > 0 ? (job.completedClinics + job.failedClinics) / job.totalClinics * 100 : 0;
      return {
        statusCode: 200,
        headers: corsHeaders,
        body: JSON.stringify({
    ...
    if (path.match(/\/meta\/ads\/scheduled\/?$/) && method === "POST") {
      ...
      if (new Date(scheduledStartDate) <= /* @__PURE__ */ new Date()) {
        return {
          statusCode: 400,
          headers: corsHeaders,
          body: JSON.stringify({
    ...
    if (path.match(/\/meta\/ads\/scheduled\/?$/) && method === "GET") {
      ...
      let result;
      if (filterClinicId) {
        result = await ddb.send(new import_lib_dynamodb.QueryCommand({
          TableName: META_SCHEDULED_CAMPAIGNS_TABLE,
          IndexName: "clinicId-index",
          KeyConditionExpression: "clinicId = :clinicId",
    ...
    if (path.match(/\/meta\/ads\/[^\/]+\/overview/) && method === "GET") {
      ...
      const campaignsResult = await ddb.send(new import_lib_dynamodb.QueryCommand({
        TableName: META_CAMPAIGNS_TABLE,
        IndexName: "clinicId-index",
        KeyConditionExpression: "clinicId = :clinicId",
        ExpressionAttributeValues: { ":clinicId": clinicId }
      ...
      const leadsResult = await ddb.send(new import_lib_dynamodb.QueryCommand({
        TableName: META_LEADS_TABLE,
        IndexName: "clinicId-index",
        KeyConditionExpression: "clinicId = :clinicId",
        ExpressionAttributeValues: { ":clinicId": clinicId }
