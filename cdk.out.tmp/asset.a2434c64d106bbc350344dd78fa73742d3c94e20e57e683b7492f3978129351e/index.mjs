...
var CHIME_MEDIA_REGION = process.env.CHIME_MEDIA_REGION || process.env.CHIME_REGION || process.env.AWS_REGION || "us-east-1";
var chimeClient = new ChimeSDKMeetingsClient({ region: CHIME_MEDIA_REGION });
function sleep(ms) {
...
async function createMeetingForScheduledMeeting(meetingID) {
  ...
  try {
    const response2 = await chimeClient.send(new CreateMeetingCommand({
    ...
    if (!response2.Meeting?.MeetingId || !response2.Meeting?.MediaPlacement || !response2.Meeting?.MediaRegion) {
      throw new Error("Failed to create scheduled meeting - no meeting data returned");
    }
    return response2.Meeting;
  } catch (error) {
...
async function createAttendee(meetingId, userID, userName) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const response2 = await chimeClient.send(new CreateAttendeeCommand({
        MeetingId: meetingId,
      ...
      if (!response2.Attendee?.AttendeeId || !response2.Attendee?.ExternalUserId || !response2.Attendee?.JoinToken) {
        throw new Error("Failed to create attendee - no attendee data returned");
      }
      return response2.Attendee;
    } catch (error) {
      lastError = error;
...
var PUBLIC_APP_BASE_URL = process.env.PUBLIC_APP_BASE_URL || "https://todaysdentalinsights.com";
...
function computeGuestJoinExpiresAtSeconds(startTime, endTime) {
  const end = parseDateMaybe(endTime);
  const start = parseDateMaybe(startTime);
  const baseMs = (end || start || /* @__PURE__ */ new Date()).getTime();
  const expiresMs = baseMs + 24 * 60 * 60 * 1e3;
  const fallbackMs = Date.now() + 7 * 24 * 60 * 60 * 1e3;
  const ms = Number.isFinite(expiresMs) ? expiresMs : fallbackMs;
  return Math.floor(ms / 1e3);
}
function makeGuestJoinToken() {
  return randomBytes(32).toString("base64url");
}
function hashGuestJoinToken(token) {
...
async function getTeamById(teamID, fnCtx) {
...
function isUserDirectConversationParticipant(favor, userID) {
...
var handler = async (event) => {
  ...
  const logCtx = { requestId, userID: userID || "public", httpMethod, path, isPublic: isPublicMeetingJoin };
  ...
  try {
    ...
    if (isPublicMeetingJoin) {
      ...
      routeMatched = true;
      result = await joinMeeting2(authedUserID, meetingID, parsedBody, logCtx);
...
async function createMeeting(userID, body, logCtx) {
  ...
  const meetingParticipants = Array.isArray(participants) && participants.length > 0 ? uniqStrings([...participants.map(String), userID]) : uniqStrings([...conversationParticipants, userID]);
...
function sanitizeMeeting(meetingRecord) {
  return {
    meetingID: meetingRecord.meetingID,
    conversationID: meetingRecord.conversationID,
    title: meetingRecord.title,
...
async function joinMeeting2(userID, meetingID, body, logCtx) {
  const fnStart = Date.now();
  ...
  const meetingRecord = meetingResult.Item;
  if (!meetingRecord) {
  ...
  const inMeetingParticipants = meetingRecord.organizerID === userID || Array.isArray(meetingRecord.participants) && meetingRecord.participants.includes(userID);
  ...
  const displayName = typeof body?.displayName === "string" ? body.displayName.trim().slice(0, 80) : void 0;
  const nowIso = (/* @__PURE__ */ new Date()).toISOString();
  let joinInfo;
  let createdNewMeeting = false;
  if (meetingRecord.chimeMeetingId) {
    try {
      joinInfo = await joinMeeting(meetingRecord.chimeMeetingId, userID, displayName);
    } catch (err) {
...
async function publicJoinMeeting(meetingID, body, logCtx) {
  const fnStart = Date.now();
  ...
  const tokenRaw = typeof body?.token === "string" && body.token || typeof body?.t === "string" && body.t || typeof body?.guestJoinToken === "string" && body.guestJoinToken || "";
  const token = tokenRaw.trim();
  const nameRaw = typeof body?.name === "string" && body.name || typeof body?.displayName === "string" && body.displayName || "Guest";
  ...
  if (meetingRecord.chimeMeetingId) {
    try {
      joinInfo = await joinMeeting(meetingRecord.chimeMeetingId, guestUserIDBase, displayName);
    } catch (err) {
      if (err?.name === "NotFoundException") {
        log.warn("Stored Chime meeting not found; recreating", { ...fnCtx, chimeMeetingId: meetingRecord.chimeMeetingId });
      } else {
