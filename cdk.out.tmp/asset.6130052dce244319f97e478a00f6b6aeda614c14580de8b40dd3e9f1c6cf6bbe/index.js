...
// src/integrations/communication/email-analytics-api.ts
var email_analytics_api_exports = {};
__export(email_analytics_api_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(email_analytics_api_exports);
...
async function handleGetStats(event, allowedClinics) {
  ...
  const clinicsToQuery = clinicId ? [clinicId] : allowedClinics.has("*") ? await getAllClinicIds() : Array.from(allowedClinics);
...
async function handleGetDashboard(event, allowedClinics) {
  const query = event.queryStringParameters || {};
  const clinicId = query.clinicId;
  ...
  return http(200, {
    ...
    totalComplaints: currentStats.totalComplained,
    // map field name for frontend
    deliveryRate: currentStats.deliveryRate,
    // decimal 0–1
    openRate: currentStats.openRate,
    // decimal 0–1
    clickRate: currentStats.clickRate,
    // decimal 0–1
    bounceRate: currentStats.bounceRate,
    // decimal 0–1
    complaintRate: currentStats.complaintRate,
    // decimal 0–1
    trends
  }, event);
}
async function handleListEmails(event, allowedClinics) {
  const query = event.queryStringParameters || {};
  const clinicId = query.clinicId;
  ...
  const clinicIds = clinicId ? [clinicId] : !allowedClinics.has("*") ? Array.from(allowedClinics) : [];
  let emails = [];
  ...
  if (clinicIds.length > 0) {
    ...
    for (const cid of clinicIds) {
      ...
      try {
        const result = await ddb.send(new import_lib_dynamodb.QueryCommand(queryParams));
    ...
    const result = await ddb.send(new import_lib_dynamodb.ScanCommand(scanParams));
    emails = result.Items || [];
...
async function handleGetEmailDetail(event, allowedClinics) {
...
function getPeriodsForRange(period) {
  const now = /* @__PURE__ */ new Date();
  switch (period) {
    case "7d":
    case "30d": {
...
async function getAllClinicIds() {
  try {
    const uniqueClinicIds = /* @__PURE__ */ new Set();
    let lastKey;
    do {
      ...
      for (const item of result.Items || []) {
        if (item.clinicId)
          uniqueClinicIds.add(item.clinicId);
      }
...
function aggregateStats(stats, period) {
  const aggregated = {
    clinicId: "all",
    period,
    totalSent: 0,
...
function computeRatesAsDecimals(stats) {
  if (stats.totalSent > 0) {
    stats.deliveryRate = Math.round(stats.totalDelivered / stats.totalSent * 1e4) / 1e4;
    stats.bounceRate = Math.round(stats.totalBounced / stats.totalSent * 1e4) / 1e4;
  }
  if (stats.totalDelivered > 0) {
    stats.openRate = Math.round(stats.totalOpened / stats.totalDelivered * 1e4) / 1e4;
    stats.complaintRate = Math.round(stats.totalComplained / stats.totalDelivered * 1e4) / 1e4;
  }
  if (stats.totalOpened > 0) {
    stats.clickRate = Math.round(stats.totalClicked / stats.totalOpened * 1e4) / 1e4;
  }
}
async function getAggregatedStatsForClinics(clinicIds, period) {
...
async function getAggregatedStatsForClinicsMultiPeriod(clinicIds, periods) {
...
function formatEmailForList(email) {
  return {
...
function formatEmailForDetail(email) {
  return {
    ...email,
    openCount: email.openCount || 0,
...
function buildEventTimeline(email) {
  const events = [];
