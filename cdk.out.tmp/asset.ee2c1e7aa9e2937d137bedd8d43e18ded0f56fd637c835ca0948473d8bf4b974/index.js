...
// src/services/chime/supervisor-whisper.ts
var supervisor_whisper_exports = {};
__export(supervisor_whisper_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(supervisor_whisper_exports);
...
async function sendWhisperMessage(ddb2, supervisorId, agentId, callId, message2, agentPresenceTableName) {
  try {
...
async function getWhisperMessages(ddb2, agentId, callId, agentPresenceTableName) {
  try {
    const { Item: agent } = await ddb2.send(new import_lib_dynamodb2.GetCommand({
      TableName: agentPresenceTableName,
      Key: { agentId },
    ...
    return agent.whisperMessages.filter((msg) => msg.callId === callId);
  } catch (error) {
    console.error("[getWhisperMessages] Error:", error.message);
    return [];
  }
}
async function markWhisperMessagesRead(ddb2, agentId, messageIds, agentPresenceTableName) {
  try {
    const { Item: agent } = await ddb2.send(new import_lib_dynamodb2.GetCommand({
      TableName: agentPresenceTableName,
      Key: { agentId },
    ...
    const updatedMessages = agent.whisperMessages.map((msg) => ({
      ...msg,
...
var handler = async (event) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      const { agentId, callId, message: message2 } = body;
      if (!agentId || !callId || !message2) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "agentId, callId, and message are required" }) };
      }
      const result = await sendWhisperMessage(ddb, userId, agentId, callId, message2, AGENT_PRESENCE_TABLE_NAME);
      return {
