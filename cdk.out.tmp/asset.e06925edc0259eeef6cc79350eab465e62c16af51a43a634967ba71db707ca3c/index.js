...
// src/services/push-notifications/register-device.ts
var register_device_exports = {};
__export(register_device_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(register_device_exports);
...
function createDeviceId(deviceToken) {
  const hash = (0, import_crypto.createHash)("sha256").update(deviceToken).digest("hex");
  return `device_${hash.substring(0, 16)}`;
}
function calculateTtl() {
  return Math.floor(Date.now() / 1e3) + TOKEN_TTL_DAYS * 24 * 60 * 60;
}
async function findExistingDeviceByToken(deviceToken) {
  try {
    ...
    if (result.Items && result.Items.length > 0) {
      ...
      if (!existingDevice.clinicId) {
        const fullItem = await ddb.send(new import_lib_dynamodb.GetCommand({
          TableName: DEVICE_TOKENS_TABLE,
          Key: {
...
async function removeTokenFromPreviousUser(previousUserId, deviceId) {
  try {
...
async function handleRegister(event, userPerms) {
  ...
  const explicitClinicIds = new Set(
    (userPerms.clinicRoles || []).map((cr) => String(cr?.clinicId || "").trim()).filter(Boolean)
  );
  let clinicIdsToRegister = [];
  if (bodyClinicIds && Array.isArray(bodyClinicIds) && bodyClinicIds.length > 0) {
    const normalizedRequested = bodyClinicIds.map((cid) => String(cid || "").trim()).filter(Boolean);
    const filtered = explicitClinicIds.size > 0 ? normalizedRequested.filter((cid) => explicitClinicIds.has(cid)) : normalizedRequested.filter((cid) => hasClinicAccess(allowedClinics, cid));
    ...
    clinicIdsToRegister = explicitClinicIds.size > 0 ? Array.from(explicitClinicIds) : [requestedClinicId];
  ...
  clinicIdsToRegister = Array.from(
    new Set(
      clinicIdsToRegister.map((cid) => String(cid || "").trim()).filter(Boolean)
    )
  );
  const userId = userPerms.email || "unknown";
  const deviceId = clientDeviceId || createDeviceId(trimmedToken);
  const env = platform === "android" || platform === "web" ? "production" : environment;
  try {
    ...
    const primaryClinicId = requestedPrimaryClinicId && clinicIdsToRegister.includes(requestedPrimaryClinicId) ? requestedPrimaryClinicId : clinicIdsToRegister[0];
    const updateResult = await ddb.send(new import_lib_dynamodb.UpdateCommand({
      TableName: DEVICE_TOKENS_TABLE,
      Key: { userId, deviceId },
      UpdateExpression: `
...
async function handleUnregister(event, userPerms) {
  const body = parseBody(event.body);
  const userId = userPerms.email || "unknown";
  try {
    if (body.allDevices) {
      const devices = await ddb.send(new import_lib_dynamodb.QueryCommand({
        TableName: DEVICE_TOKENS_TABLE,
...
async function handleGetDevices(event, userPerms) {
  const userId = userPerms.email || "unknown";
  try {
    const result = await ddb.send(new import_lib_dynamodb.QueryCommand({
      TableName: DEVICE_TOKENS_TABLE,
...
async function handleHeartbeat(event, userPerms) {
  const body = parseBody(event.body);
  const userId = userPerms.email || "unknown";
  ...
  try {
    ...
    return http(200, {
      ...
      deviceId,
      expiresAt: new Date(ttl * 1e3).toISOString()
    }, event);
  } catch (error) {
