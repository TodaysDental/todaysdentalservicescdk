...
// src/services/clinic/pricingCrud.ts
var pricingCrud_exports = {};
__export(pricingCrud_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(pricingCrud_exports);
...
var TABLE = process.env.CLINIC_PRICING_TABLE || "ClinicPricing";
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/pricing") && method === "GET")
      return getPricing(event, userPerms);
    if (path.endsWith("/pricing") && method === "POST")
      return createPricing(event, userPerms);
    if (path.endsWith("/pricing") && method === "PUT")
      return updatePricing(event, userPerms);
    if (path.endsWith("/pricing") && method === "DELETE")
      return deletePricing(event, userPerms);
    return err(404, "not found", event);
  } catch (e) {
    return err(500, e?.message || "error", event);
  }
};
async function getPricing(event, userPerms) {
  const clinicId = event.pathParameters?.clinicId || "";
  if (!clinicId)
    return err(400, "clinicId required", event);
  const resp = await ddb.send(new import_lib_dynamodb.QueryCommand({
    TableName: TABLE,
...
async function createPricing(event, userPerms) {
  const clinicId = event.pathParameters?.clinicId || "";
  const body = parse(event.body);
  if (!clinicId)
    return err(400, "clinicId required", event);
  if (!body.category)
    return err(400, "category required", event);
  const item = {
    clinicId,
...
async function updatePricing(event, userPerms) {
  const clinicId = event.pathParameters?.clinicId || "";
  const body = parse(event.body);
  if (!clinicId)
    return err(400, "clinicId required", event);
  if (!body.category)
    return err(400, "category required", event);
  const existingResp = await ddb.send(new import_lib_dynamodb.GetCommand({
    TableName: TABLE,
    Key: { clinicId, category: body.category }
  }));
  if (!existingResp.Item)
    return err(404, "pricing item not found", event);
  const item = {
    ...existingResp.Item,
    ...body,
    clinicId,
    // ensure clinicId is not overwritten
    updatedAt: (/* @__PURE__ */ new Date()).toISOString()
  };
...
async function deletePricing(event, userPerms) {
  const clinicId = event.pathParameters?.clinicId || "";
  const body = parse(event.body);
  if (!clinicId)
    return err(400, "clinicId required", event);
