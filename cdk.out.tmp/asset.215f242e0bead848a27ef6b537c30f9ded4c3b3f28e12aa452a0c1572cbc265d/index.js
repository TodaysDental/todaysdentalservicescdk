...
// src/services/marketing/profiles.ts
...
async function ayrshareGetProfile(apiKey, profileKey) {
  try {
...
async function ayrshareGenerateJWT(apiKey, profileKey, domain, privateKey, expiresIn = 300) {
  try {
    ...
    const res = await ayrshareHttp.post(
      `${AYRSHARE_URL}/profiles/generateJWT`,
      {
        domain,
        profileKey,
        privateKey,
...
async function getApiKey() {
  ...
  if (!apiKey)
    throw new Error("Ayrshare API key not found in GlobalSecrets");
  cachedApiKey = apiKey;
  return apiKey;
}
async function getPrivateKey() {
  ...
  if (!privateKey)
    throw new Error("Ayrshare private key not found in GlobalSecrets");
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/sync") && method === "POST") {
      ...
      const clinicsToSync = clinicIds && Array.isArray(clinicIds) && clinicIds.length > 0 ? allEnabledClinics.filter((c) => clinicIds.includes(c.config.clinicId)) : allEnabledClinics;
      const results = [];
      const failed = [];
      for (const clinic of clinicsToSync) {
        ...
        try {
          ...
          const existing = await ddb.send(new import_lib_dynamodb.GetCommand({
    ...
    if (path.endsWith("/profiles") && method === "GET") {
      ...
      if (platform) {
        allProfiles = allProfiles.filter(
          (p) => p.connectedPlatforms?.includes(platform)
        );
      }
    ...
    if (pathParts.includes("profiles") && pathParts.length >= 2 && !path.includes("generate-jwt") && !path.includes("social") && method === "GET") {
      ...
      if (!clinicId)
        throw new Error("clinicId required");
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          hint: configOnly ? "Profile loaded from clinics.json. Call POST /profiles/sync to persist to DynamoDB." : void 0
        })
      };
    }
