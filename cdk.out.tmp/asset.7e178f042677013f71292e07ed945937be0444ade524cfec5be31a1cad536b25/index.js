...
// src/services/chime/outbound-call.ts
var outbound_call_exports = {};
__export(outbound_call_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(outbound_call_exports);
...
var parseNumberOr = (value, fallback) => {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : fallback;
};
var OUTBOUND_CALL_MAX_ATTEMPTS = Math.max(1, Math.floor(parseNumberOr(process.env.OUTBOUND_CALL_RETRY_ATTEMPTS, 3)));
var OUTBOUND_CALL_RETRY_BASE_DELAY_MS = Math.max(250, Math.floor(parseNumberOr(process.env.OUTBOUND_CALL_RETRY_DELAY_MS, 1e3)));
var isConcurrentCallLimitError = (error) => {
  const message2 = (error?.message || "").toString();
  return error?.name === "BadRequestException" && message2.includes("Concurrent call limits");
};
var handler = async (event) => {
  ...
  let meetingId;
  let meetingInfo;
  let agentAttendeeInfo;
  let callReference;
  let callId;
  let markedBusyClinicIds = [];
  let tempBusyClinicId;
  let dialLock = null;
  try {
    ...
    agentId = getUserIdFromJwt(verifyResult.payload);
    if (!agentId) {
    ...
    try {
      ...
      console.error("[outbound-call] Invalid JSON in request body", { error: parseErr.message });
      return {
        statusCode: 400,
        headers: corsHeaders,
    ...
    const { Item: clinic } = await ddb.send(new import_lib_dynamodb4.GetCommand({
    ...
    dialLock = new DistributedLock(ddb, {
      ...
      ttlSeconds: 20,
      // FIX: Extended TTL to cover full SMA call + DB updates
      maxRetries: 1,
      // Don't retry - if locked, user already has a dial in progress
      retryDelayMs: 0
    });
    ...
    try {
      const { Items: agentActiveRows } = await ddb.send(new import_lib_dynamodb4.QueryCommand({
      ...
      const activeClinicIds = rows.filter((r) => String(r?.state || "").toLowerCase() === "active" && typeof r?.clinicId === "string" && r.clinicId.length > 0).map((r) => String(r.clinicId));
      callReference = `outbound-${Date.now()}-${(0, import_crypto3.randomUUID)().slice(0, 8)}`;
      const nowIso = (/* @__PURE__ */ new Date()).toISOString();
      const markerNowTs = Math.floor(Date.now() / 1e3);
      ...
      if (activeClinicIds.length > 0) {
        const busyResults = await Promise.allSettled(
          activeClinicIds.map(async (clinicId) => {
      ...
      console.log("[outbound-call] \u{1F4DE} Initiating SIP media application call", {
        fromPhoneNumber,
      ...
      const callCommandInput = {
        ...
        // Prevents duplicate calls if Lambda is retried
        ArgumentsMap: {
          ...
          callReference,
          idempotencyKey,
          // Also pass in arguments for logging
          toPhoneNumber: body.toPhoneNumber,
          fromPhoneNumber,
      ...
      const updateNowIso = (/* @__PURE__ */ new Date()).toISOString();
      const busyClinicIdsToUpdate = [
        ...markedBusyClinicIds,
        ...tempBusyClinicId ? [tempBusyClinicId] : []
      ];
      await Promise.allSettled(busyClinicIdsToUpdate.map(async (clinicId) => {
        await ddb.send(new import_lib_dynamodb4.UpdateCommand({
          TableName: AGENT_ACTIVE_TABLE_NAME,
          Key: { clinicId, agentId },
    ...
    if (AGENT_ACTIVE_TABLE_NAME && agentId && markedBusyClinicIds.length > 0) {
      ...
      await Promise.allSettled(markedBusyClinicIds.map(async (clinicId) => {
        try {
          await ddb.send(new import_lib_dynamodb4.UpdateCommand({
            TableName: AGENT_ACTIVE_TABLE_NAME,
            Key: { clinicId, agentId },
    ...
    if (meetingId) {
      await chime.send(new import_client_chime_sdk_meetings.DeleteMeetingCommand({ MeetingId: meetingId })).catch(() => {
      });
    }
    if (dialLock) {
    ...
    return {
      ...
      headers: corsHeaders,
      body: JSON.stringify({
        message: concurrentLimitHit ? "All outbound lines are currently in use. Please wait a moment and try again." : "Failed to make outbound call",
        error: err?.message,
        code: err?.name || err?.code
      })
    };
