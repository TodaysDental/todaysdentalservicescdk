...
var handler = async (event) => {
  ...
  try {
    ...
    const { Items: callLookupRecords } = await ddb.send(new import_lib_dynamodb6.QueryCommand({
    ...
    if (agentId) {
      if (LOCKS_TABLE_NAME2) {
        ...
        if (lockAcquired) {
          try {
            const { Item: refreshedAgentInfo } = await ddb.send(new import_lib_dynamodb6.GetCommand({
              TableName: AGENT_PRESENCE_TABLE_NAME,
              Key: { agentId }
            }));
            if (refreshedAgentInfo && refreshedAgentInfo.status === "Online" && !refreshedAgentInfo.currentCallId) {
              ...
              if (wrapUpSeconds > 0) {
                ...
                await new Promise((resolve) => setTimeout(resolve, wrapUpSeconds * 1e3));
                const { Item: postWrapAgent } = await ddb.send(new import_lib_dynamodb6.GetCommand({
                  TableName: AGENT_PRESENCE_TABLE_NAME,
                  Key: { agentId }
                }));
