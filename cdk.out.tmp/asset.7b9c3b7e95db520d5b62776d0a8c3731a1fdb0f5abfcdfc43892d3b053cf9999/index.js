...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/templates") && method === "POST") {
      ...
      const newTemplateId = (0, import_crypto.randomUUID)();
    ...
    if (templateId && method === "PUT") {
      ...
      await ddb.send(new import_lib_dynamodb.UpdateCommand({
        ...
        ExpressionAttributeNames: body.name !== void 0 ? { "#name": "name" } : void 0
      }));
      const updated = await ddb.send(new import_lib_dynamodb.GetCommand({
        TableName: RCS_TEMPLATES_TABLE,
        Key: {
          pk: `CLINIC#${clinicId}`,
          sk: `TEMPLATE#${templateId}`
