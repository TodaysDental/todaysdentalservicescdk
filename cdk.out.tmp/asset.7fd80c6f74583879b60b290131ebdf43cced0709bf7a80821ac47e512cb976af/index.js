...
var voice_agent_config_exports = {};
__export(voice_agent_config_exports, {
  AVAILABLE_VOICES: () => AVAILABLE_VOICES,
  DEFAULT_AFTER_HOURS_GREETING: () => DEFAULT_AFTER_HOURS_GREETING,
  DEFAULT_FILLER_PHRASES: () => DEFAULT_FILLER_PHRASES,
  DEFAULT_OUTBOUND_GREETINGS: () => DEFAULT_OUTBOUND_GREETINGS,
  DEFAULT_VOICE_SETTINGS: () => DEFAULT_VOICE_SETTINGS,
  getAfterHoursCallingMode: () => getAfterHoursCallingMode,
  getConfiguredOutboundAgent: () => getConfiguredOutboundAgent,
  getConfiguredVoiceAgent: () => getConfiguredVoiceAgent,
  getFullVoiceConfig: () => getFullVoiceConfig,
  handler: () => handler,
  isAiInboundEnabled: () => isAiInboundEnabled,
  isAiOutboundEnabled: () => isAiOutboundEnabled
});
module.exports = __toCommonJS(voice_agent_config_exports);
...
function normalizePollyEngine(value) {
  const raw = String(value ?? "").trim().toLowerCase();
  if (raw === "standard" || raw === "neural" || raw === "generative")
    return raw;
  if (raw === "long-form" || raw === "longform" || raw === "long_form")
    return "long-form";
  return null;
}
var VOICES_CACHE_TTL_MS = 60 * 60 * 1e3;
var voicesCache = /* @__PURE__ */ new Map();
async function listPollyVoices(event) {
  ...
  const languageCode = languageCodeRaw || void 0;
  ...
  do {
    const resp = await pollyClient.send(new import_client_polly.DescribeVoicesCommand({
      Engine: engine,
...
async function getConfig(event, userPerms, clinicId) {
  ...
  const derivedAfterHoursMode = normalizeAfterHoursCallingMode(config.afterHoursCallingMode) ?? (config.aiInboundEnabled === false ? "OFF" : config.aiInboundEnabled === true || !!config.inboundAgentId ? "FORWARD_TO_AI" : "OFF");
  const configForResponse = {
    ...config,
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      status
      // Computed active states (enabled AND agent configured)
    })
  };
}
async function updateConfig(event, userPerms, clinicId) {
  ...
  if (requestedAfterHoursMode !== void 0 && normalizedAfterHoursMode === void 0) {
    return {
      statusCode: 400,
      headers: getCorsHeaders(event),
      body: JSON.stringify({
  ...
  const isToggleOnly = (typeof body.aiInboundEnabled === "boolean" || typeof body.aiOutboundEnabled === "boolean" || typeof body.afterHoursCallingMode === "string") && !body.inboundAgentId && !body.outboundAgentId;
  ...
  let inboundAgentName;
  let outboundAgentName;
  ...
  const existingResponse = await docClient.send(new import_lib_dynamodb.GetCommand({
    TableName: VOICE_CONFIG_TABLE,
    Key: { clinicId }
  }));
  const existing = existingResponse.Item;
  const aiInboundEnabled = typeof body.aiInboundEnabled === "boolean" ? body.aiInboundEnabled : normalizedAfterHoursMode ? normalizedAfterHoursMode === "FORWARD_TO_AI" : existing?.aiInboundEnabled ?? true;
  const aiOutboundEnabled = typeof body.aiOutboundEnabled === "boolean" ? body.aiOutboundEnabled : existing?.aiOutboundEnabled ?? true;
  const afterHoursCallingMode = normalizedAfterHoursMode ?? normalizeAfterHoursCallingMode(existing?.afterHoursCallingMode) ?? (aiInboundEnabled ? "FORWARD_TO_AI" : "OFF");
  const config = {
    clinicId,
    // AI calling toggles
    aiInboundEnabled,
...
async function getClinicHours(event, userPerms, clinicId) {
  const userClinicIds = userPerms.clinicRoles.map((cr) => cr.clinicId);
  const isAdmin = userPerms.isSuperAdmin || userPerms.isGlobalSuperAdmin;
  if (!isAdmin && !userClinicIds.includes(clinicId)) {
    return { statusCode: 403, headers: getCorsHeaders(event), body: JSON.stringify({ error: "Access denied" }) };
...
async function updateClinicHours(event, userPerms, clinicId) {
  const canUpdate = hasModulePermission(
    userPerms.clinicRoles,
    AI_AGENTS_MODULE,
    "put",
...
async function isAiInboundEnabled(clinicId) {
  const response = await docClient.send(new import_lib_dynamodb.GetCommand({
    TableName: VOICE_CONFIG_TABLE,
    Key: { clinicId }
  }));
  ...
  if (config.aiInboundEnabled === void 0) {
    return false;
  }
...
async function getConfiguredVoiceAgent(clinicId) {
  const response = await docClient.send(new import_lib_dynamodb.GetCommand({
    TableName: VOICE_CONFIG_TABLE,
    Key: { clinicId }
  }));
...
async function getConfiguredOutboundAgent(clinicId) {
  const response = await docClient.send(new import_lib_dynamodb.GetCommand({
    TableName: VOICE_CONFIG_TABLE,
    Key: { clinicId }
  }));
...
0 && (module.exports = {
  ...
  getAfterHoursCallingMode,
  getConfiguredOutboundAgent,
