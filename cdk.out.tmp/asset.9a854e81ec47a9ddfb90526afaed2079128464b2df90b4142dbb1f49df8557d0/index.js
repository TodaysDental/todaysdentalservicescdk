...
// src/services/clinic/templates.ts
...
async function listTemplates(event, userPerms) {
  ...
  const templatesByModule = {};
  for (const template of filteredTemplates) {
    const module2 = template.module || "Uncategorized";
    if (!templatesByModule[module2]) {
      templatesByModule[module2] = [];
    }
    templatesByModule[module2].push(template);
  }
  return {
    statusCode: 200,
    headers: getCorsHeaders(event),
...
async function createTemplate(event, userPerms) {
  ...
  const hasAnyContent = typeof body.email_body === "string" && body.email_body.trim().length > 0 || typeof body.text_message === "string" && body.text_message.trim().length > 0 || typeof body.rcs_message === "string" && body.rcs_message.trim().length > 0 || !!body.rcs_rich_card || !!body.rcs_carousel || typeof body.voice_message === "string" && body.voice_message.trim().length > 0 || typeof body.ai_voice_prompt === "string" && body.ai_voice_prompt.trim().length > 0;
  if (!hasAnyContent) {
    return {
      statusCode: 400,
      headers: getCorsHeaders(event),
      body: JSON.stringify({
        error: "At least one template content field is required (email_body, text_message, rcs_message, rcs_rich_card, rcs_carousel, voice_message, ai_voice_prompt)"
      })
    };
  }
  if (!body.module) {
  ...
  if (!SYSTEM_MODULES.includes(body.module)) {
    return {
      statusCode: 400,
      headers: getCorsHeaders(event),
      body: JSON.stringify({
  ...
  const item = {
    ...
    // Optional: clinic-specific template
  };
...
async function updateTemplate(event, userPerms, templateId) {
  const body = JSON.parse(event.body || "{}");
  if (!body.template_name || !String(body.template_name).trim()) {
    return {
      statusCode: 400,
  ...
  if (!body.module || !SYSTEM_MODULES.includes(body.module)) {
    return {
      statusCode: 400,
      headers: getCorsHeaders(event),
      body: JSON.stringify({
...
async function deleteTemplate(event, userPerms, templateId) {
  const getCommand = new import_lib_dynamodb.QueryCommand({
    TableName: TABLE_NAME,
