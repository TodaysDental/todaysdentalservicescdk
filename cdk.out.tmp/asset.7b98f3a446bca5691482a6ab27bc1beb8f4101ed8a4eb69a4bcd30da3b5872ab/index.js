...
var import_client_polly3 = require("@aws-sdk/client-polly");
...
var pollyClient2 = new import_client_polly2.PollyClient({ region: REGION });
...
var VOICE_ENGINE_PREFERENCE = [import_client_polly2.Engine.NEURAL, import_client_polly2.Engine.STANDARD];
...
async function generateTTSToS3(callId, text, sequenceNumber, options = {}) {
  ...
  try {
    ...
    const response = await pollyClient2.send(new import_client_polly2.SynthesizeSpeechCommand({
      Text: speechText,
      TextType: textType,
      OutputFormat: import_client_polly2.OutputFormat.PCM,
...
var CONFIG = {
  ...
  DEFAULT_VOICE_ID: import_client_polly3.VoiceId.Joanna,
  DEFAULT_VOICE_ENGINE: import_client_polly3.Engine.NEURAL,
  SAMPLE_RATE: "8000",
  // Telephony standard
  OUTPUT_FORMAT: import_client_polly3.OutputFormat.PCM,
  // Goodbye message
...
var pollyClient3 = new import_client_polly3.PollyClient({
  region: process.env.AWS_REGION || "us-east-1"
});
...
async function textToSpeech(text, clinicId) {
  ...
  const command = new import_client_polly3.SynthesizeSpeechCommand({
    Text: text,
  ...
  const response = await pollyClient3.send(command);
  if (response.AudioStream) {
    const chunks = [];
