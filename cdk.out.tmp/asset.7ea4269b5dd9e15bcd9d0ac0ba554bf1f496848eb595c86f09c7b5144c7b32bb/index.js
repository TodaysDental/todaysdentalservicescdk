...
// src/services/clinic/consent-form-data.ts
var consent_form_data_exports = {};
__export(consent_form_data_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(consent_form_data_exports);
...
var TABLE_NAME = process.env.TABLE_NAME || "ConsentFormData";
var getCorsHeaders = (event) => buildCorsHeaders({}, event.headers?.origin);
var getJsonHeaders = (event) => ({
  ...getCorsHeaders(event),
  "Content-Type": "application/json"
});
function normalizeConsentFormLanguage(raw) {
  const v = String(raw ?? "").trim().toLowerCase();
  if (!v)
    return void 0;
  if (v === "en" || v === "english")
    return "en";
  if (v === "es" || v === "spanish" || v === "espa\xF1ol" || v === "espanol")
    return "es";
  return void 0;
}
var handler = async (event) => {
  ...
  try {
    ...
    if (consentFormId) {
      if (httpMethod === "GET") {
        return await getConsentForm(event, userPerms, consentFormId);
      }
      if (httpMethod === "PUT") {
...
async function listConsentForms(event, userPerms) {
  ...
  const includeElements = !wantsSummary && !["0", "false", "no"].includes(String(includeElementsRaw ?? "").trim().toLowerCase());
  ...
  const consentForms = languageFilter ? normalizedItems.filter((i) => i.language === languageFilter) : normalizedItems;
  const responseItems = includeElements ? consentForms : consentForms.map((i) => {
...
async function getConsentForm(event, userPerms, consentFormId) {
  ...
  return {
    ...
    body: JSON.stringify({
      ...response.Item,
      language: normalizeConsentFormLanguage(response.Item?.language) || "en"
    })
  };
}
async function createConsentForm(event, userPerms) {
  ...
  const consentFormId = v4_default();
...
async function updateConsentForm(event, userPerms, consentFormId) {
  const body = JSON.parse(event.body || "{}");
  if (!body.templateName || !body.elements || !Array.isArray(body.elements)) {
    return {
      statusCode: 400,
  ...
  const command = new import_lib_dynamodb.PutCommand({
    TableName: TABLE_NAME,
    Item: item
    // PutCommand overwrites the item
  });
  await docClient.send(command);
  return {
    statusCode: 200,
...
async function deleteConsentForm(event, userPerms, consentFormId) {
