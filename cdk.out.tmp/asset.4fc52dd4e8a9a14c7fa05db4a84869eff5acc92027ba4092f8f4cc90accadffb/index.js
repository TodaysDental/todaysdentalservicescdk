...
// src/services/marketing/posts.ts
var posts_exports = {};
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/posts") && method === "POST") {
      ...
      if (clinicIds[0] === "*") {
        const scanRes = await ddb.send(new import_lib_dynamodb.QueryCommand({
      ...
      const results = [];
      const failed = [];
      const ayrsharePostIds = {};
      const platformPostIds = {};
      for (const clinicId of targetClinicIds) {
        try {
          const profileRes = await ddb.send(new import_lib_dynamodb.GetCommand({
            TableName: PROFILES_TABLE,
            Key: { clinicId }
          }));
    ...
    if (path.endsWith("/posts") && method === "GET") {
      ...
      const queryRes = await ddb.send(new import_lib_dynamodb.QueryCommand(queryParams));
      ...
      if (deleteFromPlatforms && post.ayrsharePostIds) {
        for (const [clinicId, ayrsharePostId] of Object.entries(post.ayrsharePostIds)) {
          try {
            ...
            if (profileRes.Item?.ayrshareProfileKey) {
              await ayrshareDeletePost(API_KEY, profileRes.Item.ayrshareProfileKey, ayrsharePostId);
