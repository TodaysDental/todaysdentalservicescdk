...
// src/services/marketing/templates.ts
...
var TEMPLATES_TABLE = process.env.DESIGN_TEMPLATES_TABLE;
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/templates") && method === "POST") {
      ...
      const templateId = existingId || v4_default();
      ...
      await ddb.send(
        new import_lib_dynamodb.PutCommand({ TableName: TEMPLATES_TABLE, Item: item })
      );
      return {
        statusCode: 200,
        headers: corsHeaders,
    ...
    if ((path.endsWith("/templates") || path.endsWith("/templates/")) && method === "GET" && !event.pathParameters?.templateId) {
    ...
    if (event.pathParameters?.templateId && method === "DELETE") {
      ...
      if (record.Item.thumbnailUrl) {
        ...
        if (urlParts[1])
          keysToDelete.push(urlParts[1]);
      }
      for (const key of keysToDelete) {
        try {
          await s3.send(
            new import_client_s3.DeleteObjectCommand({ Bucket: MEDIA_BUCKET, Key: key })
          );
        } catch (err) {
