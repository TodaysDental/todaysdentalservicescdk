...
var DynamoDBManager = class _DynamoDBManager {
  constructor(config = {}) {
    ...
    this.documentClient = import_lib_dynamodb2.DynamoDBDocumentClient.from(this.client, {
      marshallOptions: {
  ...
  async warmConnections() {
    if (this.warmed)
      return;
    try {
      await this.documentClient.send(new import_lib_dynamodb2.GetCommand({
...
var handler = async (event) => {
  ...
  try {
    ...
    try {
      const { Items: callRecords } = await ddb2.send(new import_lib_dynamodb5.QueryCommand({
      ...
      const { Item: agentRecord } = await ddb2.send(new import_lib_dynamodb5.GetCommand({
      ...
      try {
        await ddb2.send(new import_lib_dynamodb5.TransactWriteCommand({
          TransactItems: [
            {
              Update: {
                TableName: CALL_QUEUE_TABLE_NAME2,
