...
// src/services/chime/join-queued-call.ts
var join_queued_call_exports = {};
__export(join_queued_call_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(join_queued_call_exports);
...
var handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      const { Item: agentPresence } = await ddb.send(new import_lib_dynamodb4.GetCommand({
      ...
      if (meetingId) {
        try {
          await chime.send(new import_client_chime_sdk_meetings.GetMeetingCommand({ MeetingId: meetingId }));
      ...
      if (isPushNotificationsEnabled() && CHIME_CONFIG.PUSH.ENABLE_QUEUE_PICKUP_CANCEL_PUSH) {
        const ringingAgentIds = callRecord.ringingAgentIds;
