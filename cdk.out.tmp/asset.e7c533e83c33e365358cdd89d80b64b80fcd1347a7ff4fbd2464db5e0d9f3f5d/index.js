...
// src/services/marketing/media.ts
var media_exports = {};
...
var MEDIA_TABLE = process.env.MARKETING_MEDIA_TABLE;
var MEDIA_BUCKET = process.env.MEDIA_BUCKET;
var API_KEY = process.env.AYRSHARE_API_KEY;
var allowedHostsCache = null;
function getAllowedHosts() {
  if (allowedHostsCache)
    return allowedHostsCache;
  const hosts = /* @__PURE__ */ new Set();
  for (const origin2 of ALLOWED_ORIGINS_LIST) {
    try {
      const u = new URL(origin2);
      if (u.hostname)
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.includes("/upload") && method === "POST") {
      ...
      const presignedUrl = await (0, import_s3_request_presigner.getSignedUrl)(s3, putCommand, { expiresIn: 3600 });
    ...
    if (path.includes("/import") && method === "POST") {
      const body = JSON.parse(event.body || "{}");
      const { url: url2, clinicId, purpose, fileName: requestedFileName, tags } = body;
      if (!url2) {
        return {
          statusCode: 400,
          headers: corsHeaders,
      ...
      const mediaId = isLogo ? `logo-${clinicId}` : v4_default();
      ...
      const s3Key = isLogo ? `clinic-logos/${safeClinicId}/${finalFileName}` : `imports/${mediaId}/${finalFileName}`;
      const publicUrl = `https://${MEDIA_BUCKET}.s3.amazonaws.com/${s3Key}`;
    ...
    if (path.endsWith("/media") && method === "GET") {
      ...
      const scanRes = await ddb.send(new import_lib_dynamodb.ScanCommand(scanParams));
    ...
    if (event.pathParameters?.mediaId && method === "DELETE") {
      ...
      const mediaRes = await ddb.send(new import_lib_dynamodb.GetCommand({
        TableName: MEDIA_TABLE,
        Key: { mediaId }
      }));
      if (!mediaRes.Item) {
    ...
    if (path.includes("/upload-url") && method === "GET") {
      ...
      const uploadUrl = await (0, import_s3_request_presigner.getSignedUrl)(s3, putCommand, { expiresIn: 3600 });
    ...
    if (path.includes("/verify-url") && method === "POST") {
      ...
      try {
        const result = await ayrshareVerifyMediaUrl(API_KEY, url2);
        return {
          statusCode: 200,
          headers: corsHeaders,
          body: JSON.stringify({
