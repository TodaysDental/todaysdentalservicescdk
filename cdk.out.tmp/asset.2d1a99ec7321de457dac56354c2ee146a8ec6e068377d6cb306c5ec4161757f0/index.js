...
// src/services/chime/call-accepted.ts
var call_accepted_exports = {};
__export(call_accepted_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(call_accepted_exports);
...
var handler = async (event) => {
  ...
  try {
    ...
    const lock = new DistributedLock(ddb, {
      ...
      ttlSeconds: 30,
      // Increased to cover transaction + attendee creation + SMA bridging
      maxRetries: 10,
    ...
    try {
      ...
      const transactionItems = [
        ...
        // Item 2: Update the accepting agent's status to 'OnCall'
        {
          Update: {
            TableName: AGENT_PRESENCE_TABLE_NAME,
            Key: { agentId },
            UpdateExpression: "SET #status = :onCall, currentCallId = :callId, callStatus = :connected, lastActivityAt = :timestamp REMOVE ringingCallId, ringingCallTime, ringingCallFrom, ringingCallClinicId, ringingCallNotes, ringingCallTransferAgentId, ringingCallTransferMode",
            ConditionExpression: "ringingCallId = :callId",
            // Ensure agent was ringing for this call
            ExpressionAttributeNames: { "#status": "status" },
            ExpressionAttributeValues: {
              ":onCall": "OnCall",
              ":connected": "connected",
      ...
      try {
        await ddb.send(new import_lib_dynamodb3.TransactWriteCommand({ TransactItems: transactionItems }));
        ...
        if (err.name === "TransactionCanceledException") {
          ...
          if (callQueueFailed && agentPresenceFailed) {
            ...
            try {
              const { Items: currentCallState } = await ddb.send(new import_lib_dynamodb3.QueryCommand({
                TableName: CALL_QUEUE_TABLE_NAME,
                IndexName: "callId-index",
                KeyConditionExpression: "callId = :callId",
                ExpressionAttributeValues: { ":callId": callId }
      ...
      try {
        const ringList = Array.isArray(freshCallRecord.agentIds) ? freshCallRecord.agentIds : [];
        const otherAgentIds = ringList.filter((id) => id !== agentId);
        if (otherAgentIds.length > 0) {
          ...
          Promise.allSettled(otherAgentIds.map(async (otherId) => {
            await ddb.send(new import_lib_dynamodb3.UpdateCommand({
              TableName: AGENT_PRESENCE_TABLE_NAME,
              Key: { agentId: otherId },
      ...
      if (smaId) {
        try {
          ...
          if (otherAgentIds.length > 0) {
            ...
            const cleanupWithRetry = async (otherId, retries = 2) => {
            ...
            Promise.all(otherAgentIds.map((otherId) => cleanupWithRetry(otherId))).then(() => console.log("[call-accepted] Finished cleaning up other agents")).catch(() => {
            });
          }
        } catch (smaErr) {
