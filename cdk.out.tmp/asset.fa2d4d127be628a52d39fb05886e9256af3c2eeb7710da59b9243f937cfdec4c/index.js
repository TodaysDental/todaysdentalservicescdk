...
// src/services/chime/process-voicemail.ts
var process_voicemail_exports = {};
__export(process_voicemail_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(process_voicemail_exports);
...
async function sendVoicemailNotification(notification) {
  if (!PUSH_NOTIFICATIONS_ENABLED)
    return;
  const callerDisplay = getCallerDisplay(notification);
  const durationStr = notification.durationSeconds > 60 ? `${Math.floor(notification.durationSeconds / 60)}:${String(notification.durationSeconds % 60).padStart(2, "0")}` : `${notification.durationSeconds}s`;
...
var handler = async (event) => {
  for (const record of event.Records) {
    ...
    const estimatedDurationSeconds = Math.round(size / 16e3);
    try {
