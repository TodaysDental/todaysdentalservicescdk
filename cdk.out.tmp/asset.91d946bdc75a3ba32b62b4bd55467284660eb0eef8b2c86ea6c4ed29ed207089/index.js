...
// src/services/chime/supervisor-monitor.ts
var supervisor_monitor_exports = {};
__export(supervisor_monitor_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(supervisor_monitor_exports);
...
var SupervisionMode = /* @__PURE__ */ ((SupervisionMode2) => {
  SupervisionMode2["SILENT"] = "silent";
  SupervisionMode2["WHISPER"] = "whisper";
  SupervisionMode2["BARGE"] = "barge";
  return SupervisionMode2;
})(SupervisionMode || {});
async function startSupervision(ddb2, supervisorId, callId, clinicId, queuePosition, mode, callQueueTableName, supervisorSessionsTableName) {
  ...
  try {
    const { Item: call } = await ddb2.send(new import_lib_dynamodb2.GetCommand({
    ...
    const { Attendee } = await chimeClient.send(new import_client_chime_sdk_meetings.CreateAttendeeCommand({
      MeetingId: call.meetingId,
      ExternalUserId: `supervisor-${supervisorId}`,
      Capabilities: {
        Audio: mode === "silent" /* SILENT */ ? "ReceiveOnly" : "SendReceive",
        Video: "None",
        Content: "None"
      }
    }));
    ...
    await publishMetric("CallVolume" /* CALL_VOLUME */, 1, {
      clinicId,
...
async function changeSupervisionMode(ddb2, sessionId, supervisorId, newMode, clinicId, queuePosition, callQueueTableName) {
  ...
  try {
    ...
    return {
      ...
      session: {
        ...
        clinicId,
        mode: newMode,
        supervisorAttendeeId: "",
        // Would need to fetch from session
...
async function endSupervision(ddb2, sessionId, supervisorId, clinicId, queuePosition, callQueueTableName, supervisorSessionsTableName, notes) {
...
var SUPERVISOR_SESSIONS_TABLE_NAME = process.env.SUPERVISOR_SESSIONS_TABLE_NAME;
var handler = async (event) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (!callId || !clinicId || queuePosition === void 0 || !mode) {
      ...
      if (mode === "whisper" /* WHISPER */ && !CHIME_CONFIG.SUPERVISOR.ENABLE_WHISPER) {
        return { statusCode: 403, headers: corsHeaders, body: JSON.stringify({ message: "Whisper mode is disabled" }) };
      }
      if (mode === "barge" /* BARGE */ && !CHIME_CONFIG.SUPERVISOR.ENABLE_BARGE) {
      ...
      const result = await startSupervision(
        ...
        SUPERVISOR_SESSIONS_TABLE_NAME
      );
      return {
    ...
    if (method === "PUT") {
      ...
      if (!sessionId || !mode || !clinicId || queuePosition === void 0) {
    ...
    if (method === "DELETE") {
      ...
      if (!sessionId || !clinicId || queuePosition === void 0) {
