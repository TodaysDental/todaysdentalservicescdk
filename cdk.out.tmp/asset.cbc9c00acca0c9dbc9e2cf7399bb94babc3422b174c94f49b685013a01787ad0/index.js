...
// src/services/marketing/webhooks.ts
...
var COMMENTS_TABLE = process.env.MARKETING_COMMENTS_TABLE;
var AYRSHARE_API_KEY = process.env.AYRSHARE_API_KEY;
var AYRSHARE_WEBHOOK_SECRET = process.env.AYRSHARE_WEBHOOK_SECRET || "";
function verifyAyrshareWebhookSignature(body, signature, timestamp, secret) {
  ...
  const expectedSignature = (0, import_crypto4.createHmac)("sha256", secret).update(message).digest("hex");
  try {
    ...
    return (0, import_crypto4.timingSafeEqual)(sigBuffer, expectedBuffer);
  } catch (err) {
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/webhooks") && method === "POST") {
      ...
      const { url: url2, actions, profileKey } = body;
      if (!url2 || !actions || !Array.isArray(actions)) {
        return {
          statusCode: 400,
          headers: corsHeaders,
          body: JSON.stringify({
      ...
      const validActions = ["social", "message", "analytics", "comment", "feed"];
    ...
    if (path.includes("/ayrshare") && method === "POST") {
      ...
      let clinicId = null;
      let clinicName = null;
      const profilesRes = await ddb.send(new import_lib_dynamodb.ScanCommand({
      ...
      switch (action) {
        case "comment":
          if (type === "new" && comment) {
            ...
            const postsRes = await ddb.send(new import_lib_dynamodb.ScanCommand({
            ...
            if (internalPostId) {
              await ddb.send(new import_lib_dynamodb.PutCommand({
                ...
                Item: {
                  ...
                  commentId: comment.id || v4_default(),
                  clinicId,
                  clinicName,
                  platform,
                  ...
                  createdAt: comment.created || (/* @__PURE__ */ new Date()).toISOString(),
