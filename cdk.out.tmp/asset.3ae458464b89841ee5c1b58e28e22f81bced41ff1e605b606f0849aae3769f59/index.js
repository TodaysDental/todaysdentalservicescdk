...
// src/services/chime/stop-session.ts
var stop_session_exports = {};
__export(stop_session_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(stop_session_exports);
...
async function cleanupActiveCall(callId, agentId) {
  if (!callId || !CALL_QUEUE_TABLE_NAME)
    return;
  ...
  if (LOCKS_TABLE_NAME) {
    lock = new DistributedLock(ddb, {
      ...
      // Unified lock key matching hold-call.ts and resume-call.ts
      ttlSeconds: 15,
      maxRetries: 3,
      retryDelayMs: 100
    });
  ...
  try {
    ...
    try {
      await ddb.send(new import_lib_dynamodb4.UpdateCommand({
        ...
        UpdateExpression: "SET #status = :status, callStatus = :status, completedAt = :timestamp, abandonedReason = :reason, abandonedByAgentId = :agentId",
...
var handler = async (event) => {
  ...
  try {
    ...
    const { Item } = await ddb.send(new import_lib_dynamodb4.GetCommand({
    ...
    if (Item?.currentCallId)
      callsToCleanup.push(Item.currentCallId);
    if (Item?.heldCallId && Item.heldCallId !== Item?.currentCallId)
      callsToCleanup.push(Item.heldCallId);
    if (Item?.ringingCallId && !callsToCleanup.includes(Item.ringingCallId))
      callsToCleanup.push(Item.ringingCallId);
    ...
    if (Item && Item.meetingInfo?.MeetingId) {
      ...
      try {
        await chimeClient.send(new import_client_chime_sdk_meetings.DeleteMeetingCommand({
          MeetingId: meetingId
        }));
    ...
    await ddb.send(new import_lib_dynamodb4.UpdateCommand({
      TableName: AGENT_PRESENCE_TABLE_NAME,
      Key: { agentId },
      UpdateExpression: "SET #status = :status, #updatedAt = :updatedAt REMOVE activeClinicIds, meetingInfo, attendeeInfo, #ttl, currentCallId, heldCallId, ringingCallId, callStatus, heldCallMeetingId, heldCallAttendeeId, inboundMeetingInfo, inboundAttendeeInfo, ringingCallTime, ringingCallFrom, ringingCallClinicId, ringingCallNotes, ringingCallTransferAgentId, ringingCallTransferMode",
    ...
    if (isPushNotificationsEnabled() && CHIME_CONFIG.PUSH.ENABLE_SESSION_OFFLINE_ALERT && Item) {
      const agentClinicIds = Item.activeClinicIds;
      if (agentClinicIds && agentClinicIds.length > 0) {
        sendClinicAlert(
          agentClinicIds[0],
          "Agent Offline",
          `Agent ${Item.agentName || agentId} has gone offline`,
          {
        ...
        ).catch((err) => console.warn("[stop-session] Agent offline push failed (non-fatal):", err.message));
      }
    }
    const responseBody = {
      success: true,
