...
// src/services/chime/call-rejected-v2.ts
var call_rejected_v2_exports = {};
__export(call_rejected_v2_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(call_rejected_v2_exports);
...
var MAX_RING_AGENTS = Math.max(1, Number(process.env.MAX_RING_AGENTS || 25) || 25);
...
async function listActiveAgentsForClinic(clinicId) {
  if (!AGENT_ACTIVE_TABLE_NAME)
    return [];
  const { Items } = await ddb.send(new import_lib_dynamodb4.QueryCommand({
  ...
  return uniqStrings((Items || []).map((i) => i?.agentId)).slice(0, MAX_RING_AGENTS);
}
var handler = async (event) => {
  ...
  if (!acquired)
    return http(409, { message: "Call is being handled by another agent. Please try again.", callId }, event);
  try {
    ...
    const ringList = uniqStrings(Array.isArray(freshCall.agentIds) ? freshCall.agentIds : []);
    const existingRejected = uniqStrings(Array.isArray(freshCall.rejectedAgentIds) ? freshCall.rejectedAgentIds : []);
    ...
    if (nextRingAgents.length === 0) {
      await ddb.send(new import_lib_dynamodb4.UpdateCommand({
        TableName: CALL_QUEUE_TABLE_NAME,
        Key: { clinicId, queuePosition },
        UpdateExpression: "SET #status = :queued, rejectedAgentIds = :rejected, updatedAt = :ts, lastStateChange = :ts REMOVE agentIds, ringStartTimeIso, ringStartTime",
        ConditionExpression: "#status = :ringing AND contains(agentIds, :agentId)",
        ExpressionAttributeNames: { "#status": "status" },
        ExpressionAttributeValues: {
          ":ringing": "ringing",
    ...
    const callerPhoneNumber = typeof freshCall.phoneNumber === "string" ? freshCall.phoneNumber : void 0;
    await ddb.send(new import_lib_dynamodb4.UpdateCommand({
      TableName: CALL_QUEUE_TABLE_NAME,
      Key: { clinicId, queuePosition },
      UpdateExpression: "SET #status = :ringing, agentIds = :agentIds, rejectedAgentIds = :rejected, ringStartTimeIso = :ts, ringStartTime = :now, lastStateChange = :ts, updatedAt = :ts",
      ConditionExpression: "#status = :ringing AND contains(agentIds, :agentId)",
      ExpressionAttributeNames: { "#status": "status" },
      ExpressionAttributeValues: {
        ":ringing": "ringing",
