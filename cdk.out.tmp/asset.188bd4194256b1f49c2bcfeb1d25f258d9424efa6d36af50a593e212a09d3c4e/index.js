...
// src/services/chime/supervisor-live-calls.ts
var supervisor_live_calls_exports = {};
__export(supervisor_live_calls_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(supervisor_live_calls_exports);
...
async function getMonitorableCalls(ddb2, supervisorId, allowedClinicIds, callQueueTableName, agentPresenceTableName) {
  const liveCalls = [];
  for (const clinicId of allowedClinicIds) {
    try {
      const { Items: calls } = await ddb2.send(new import_lib_dynamodb2.QueryCommand({
      ...
      if (calls) {
        for (const call of calls) {
          ...
          const connectedAt = call.connectedAt ? new Date(call.connectedAt).getTime() : call.queueEntryTime ? call.queueEntryTime * 1e3 : Date.now();
          const duration = Math.floor((Date.now() - connectedAt) / 1e3);
          liveCalls.push({
            callId: call.callId,
...
var handler = async (event) => {
  ...
  try {
    ...
    const allowedClinicIds = payload.clinicIds || payload.authorizedClinics || [];
