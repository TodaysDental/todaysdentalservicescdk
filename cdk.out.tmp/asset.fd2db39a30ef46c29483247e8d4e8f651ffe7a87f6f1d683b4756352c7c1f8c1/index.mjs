...
var CALL_ANALYTICS_TABLE = process.env.CALL_ANALYTICS_TABLE_NAME;
    ...
    if (CALL_ANALYTICS_TABLE) {
      ...
      if (analytics?.transcript) {
        if (Array.isArray(analytics.transcript)) {
          return { transcript: analytics.transcript };
...
function parseTranscriptSegments(segments) {
  const transcript = segments.map((seg, idx) => {
    ...
    const timestamp = seg.timestamp ? formatTimestamp(typeof seg.timestamp === "number" ? seg.timestamp : parseFloat(seg.timestamp) || idx * 5) : formatTimestamp(idx * 5);
    return {
      timestamp,
      speaker,
