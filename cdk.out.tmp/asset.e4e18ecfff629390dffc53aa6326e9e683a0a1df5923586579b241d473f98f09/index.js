...
// src/services/chime/get-joinable-calls.ts
var get_joinable_calls_exports = {};
__export(get_joinable_calls_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_joinable_calls_exports);
...
// src/services/chime/utils/priority-queue.ts
var import_lib_dynamodb = require("@aws-sdk/lib-dynamodb");
var PriorityQueueManager = class {
  constructor(ddb2, callQueueTable) {
    this.ddb = ddb2;
   ...
   */
  async getNextCallForAgent(agentInfo, clinicId) {
    const { Items: queuedCalls } = await this.ddb.send(new import_lib_dynamodb.QueryCommand({
   ...
   */
  calculatePriorityScore(call) {
    let score = 0;
    const now = Date.now();
    const queueTime = (now - new Date(call.queueEntryTime).getTime()) / 1e3;
    ...
    const waitBonus = Math.min(queueTime, 600) * 2;
    score += waitBonus;
    if (call.isVip) {
      score += 500;
    }
    ...
    if (call.maxWaitDeadline && now > call.maxWaitDeadline - 6e4) {
      score += 2e3;
    }
    return score;
  }
   ...
   */
  canAgentHandleCall(agent, call) {
    if (call.requiredSkills && call.requiredSkills.length > 0) {
      const agentSkills = agent.skills || [];
      const hasAllSkills = call.requiredSkills.every(
   ...
   */
  async getQueueMetrics(clinicId) {
    const { Items: queuedCalls } = await this.ddb.send(new import_lib_dynamodb.QueryCommand({
      TableName: this.callQueueTable,
      KeyConditionExpression: "clinicId = :clinicId",
      FilterExpression: "#status = :queued",
    ...
    for (const call of queuedCalls) {
      ...
      const waitTime = (now - new Date(call.queueEntryTime).getTime()) / 1e3;
      totalWait += waitTime;
...
var priorityManager = new PriorityQueueManager(ddb, process.env.CALL_QUEUE_TABLE_NAME);
...
var handler = async (event) => {
  ...
  try {
    ...
    const includeQueued = queryParams.includeQueued !== "false";
    const isSupervisor = isSuperAdmin || roles.includes("supervisor") || roles.includes("admin");
    const includeActive = queryParams.includeActive === "true" || isSupervisor;
      ...
      try {
        ...
        for (const call of calls) {
          if (includeQueued && call.status === "queued") {
            const waitTime = call.queuedAt ? Math.floor((now - call.queuedAt) / 1e3) : 0;
            const PRIORITY_WEIGHTS = { high: 1e3, normal: 100, low: 10 };
          ...
          if (includeActive && ["connected", "on-hold", "ringing"].includes(call.status)) {
            const duration = call.connectedAt ? Math.floor((now - call.connectedAt) / 1e3) : 0;
            let agentName;
