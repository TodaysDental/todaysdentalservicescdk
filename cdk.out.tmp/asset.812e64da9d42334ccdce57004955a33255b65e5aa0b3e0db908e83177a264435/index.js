...
// src/services/chime/call-accepted-v2.ts
var call_accepted_v2_exports = {};
__export(call_accepted_v2_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(call_accepted_v2_exports);
...
async function sendCallCancelledToAgents(agentUserIds, excludeAgentId, notification) {
  const targetIds = agentUserIds.filter((id) => id !== excludeAgentId);
  if (!PUSH_NOTIFICATIONS_ENABLED || targetIds.length === 0)
    return;
  const idempotencyKey = `call_cancelled:${notification.callId}:${notification.timestamp}`;
  console.log("[ChimePush] \u{1F515} Sending call_cancelled push to stop ringing", {
...
function http(statusCode, body, event) {
  const origin = event.headers && (event.headers.origin || event.headers.Origin) || void 0;
...
var handler = async (event) => {
  ...
  if (!CALL_QUEUE_TABLE_NAME)
    return http(500, { message: "Server misconfiguration: CALL_QUEUE_TABLE_NAME not set" }, event);
  if (!LOCKS_TABLE_NAME)
    return http(500, { message: "Server misconfiguration: LOCKS_TABLE_NAME not set" }, event);
  ...
  if (!callId)
    return http(400, { message: "callId is required" }, event);
  ...
  try {
    ...
    const { Item: freshCall } = await ddb.send(new import_lib_dynamodb4.GetCommand({
      TableName: CALL_QUEUE_TABLE_NAME,
    ...
    if (!freshCall)
      return http(404, { message: "Call not found", callId }, event);
    const status = String(freshCall.status || "");
    ...
    try {
        ...
        TransactItems: [
          {
            Update: {
              TableName: CALL_QUEUE_TABLE_NAME,
              Key: { clinicId, queuePosition },
              UpdateExpression: "SET #status = :accepting, assignedAgentId = :agentId, acceptedAt = :timestamp, #ttl = :ttl, #version = if_not_exists(#version, :zero) + :one",
              ConditionExpression: "#status = :ringing AND (attribute_not_exists(assignedAgentId) OR assignedAgentId = :agentId) AND contains(agentIds, :agentId)",
      ...
      if (err?.name === "TransactionCanceledException") {
        ...
        const agentActiveConditionFailed = reasons.length >= 2 && reasons[1]?.Code === "ConditionalCheckFailed";
        const callConditionFailed = reasons.length >= 1 && reasons[0]?.Code === "ConditionalCheckFailed";
        if (agentActiveConditionFailed && !callConditionFailed) {
          ...
          try {
            ...
            return http(409, {
              ...
              callId,
              agentId,
              reasons: retryErr?.CancellationReasons || reasons
            }, event);
          }
        } else {
          return http(409, {
    ...
    let meetingInfo = freshCall.meetingInfo;
    let meetingId = freshCall.meetingId || freshCall.meetingInfo?.MeetingId;
    let customerAttendee = freshCall.customerAttendeeInfo;
    ...
    let agentAttendee;
    try {
      const agentAttendeeResponse = await chime.send(new import_client_chime_sdk_meetings.CreateAttendeeCommand({
        MeetingId: meetingId,
      ...
      return http(500, { message: "Failed to create agent meeting credentials", error: attendeeErr?.message || "attendee_create_failed" }, event);
    }
    await ddb.send(new import_lib_dynamodb4.UpdateCommand({
      TableName: CALL_QUEUE_TABLE_NAME,
      Key: { clinicId, queuePosition },
    ...
    if (isPushNotificationsEnabled() && ringList.length > 1) {
      try {
        await sendCallCancelledToAgents(ringList, agentId, {
          callId,
          clinicId,
          clinicName: freshCall.clinicName || clinicId,
          callerPhoneNumber: freshCall.phoneNumber || "",
    ...
    console.error("[call-accepted-v2] Error:", {
      ...
      callId: event?.body ? (() => {
        try {
          return JSON.parse(event.body).callId;
        } catch {
          return void 0;
        }
      })() : void 0
