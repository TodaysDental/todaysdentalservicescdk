...
var RCS_TEMPLATES_TABLE = process.env.RCS_TEMPLATES_TABLE;
var RCS_ANALYTICS_TABLE = process.env.RCS_ANALYTICS_TABLE;
function parseDateRange(params) {
  const now = Date.now();
  const defaultRange = 7 * 24 * 60 * 60 * 1e3;
...
function aggregateStatusCounts(messages) {
  const counts = {
    queued: 0,
    sending: 0,
    sent: 0,
...
function aggregateMessageTypes(messages) {
  const types = {
    text: 0,
    richCard: 0,
    carousel: 0,
...
async function getTemplatePerformance(clinicId, messages) {
  const templateStats = /* @__PURE__ */ new Map();
  for (const msg of messages) {
    const templateId = msg.templateId || msg.contentSid;
    if (!templateId || msg.direction !== "outbound")
      continue;
  ...
  for (const [templateId, stats] of templateStats) {
    let name = templateId;
    try {
      const templateResult = await ddb.send(new import_lib_dynamodb.GetCommand({
...
async function pushCloudWatchMetrics(clinicId, metrics) {
...
function generateTimeSeries(messages, startDate, endDate, granularity) {
  let bucketMs;
  switch (granularity) {
    case "hour":
      bucketMs = 60 * 60 * 1e3;
    ...
    case "day":
      ...
      break;
    case "week":
      bucketMs = 7 * 24 * 60 * 60 * 1e3;
      break;
    case "month":
      bucketMs = 30 * 24 * 60 * 60 * 1e3;
      break;
  }
...
var handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/analytics/summary") && method === "GET") {
      ...
      const totalSent = statusCounts.queued + statusCounts.sending + statusCounts.sent + statusCounts.delivered + statusCounts.read + statusCounts.failed + statusCounts.undelivered;
      ...
      const summary = {
        ...
        engagement: {
          buttonClicks: 0,
          // Requires tracking via postback data
        ...
        smsFallback: {
          ...
          successRate: smsFallbackMsgs.length > 0 ? Math.round(smsFallbackMsgs.filter((m) => m.status === "sent").length / smsFallbackMsgs.length * 100) / 100 : 0
        }
      };
    ...
    if (path.endsWith("/analytics/export") && method === "POST") {
      ...
      return {
        ...
        headers: {
          ...corsHeaders,
          "Content-Disposition": `attachment; filename="rcs-analytics-${clinicId}-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.json"`
        },
        body: JSON.stringify({
...
async function fetchMessagesInRange(clinicId, startDate, endDate) {
  const messages = [];
  let lastKey = void 0;
  do {
...
async function buildFullSummary(clinicId, messages, startDate, endDate, granularity) {
  const statusCounts = aggregateStatusCounts(messages);
  const messageTypes = aggregateMessageTypes(messages);
  const hourlyDist = calculateHourlyDistribution(messages);
  const avgResponseTime = calculateAvgResponseTime(messages);
