...
import { DynamoDBDocumentClient, GetCommand, PutCommand, DeleteCommand, ScanCommand as ScanCommand2 } from "@aws-sdk/lib-dynamodb";
...
var apiHandler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/hours")) {
      if (method === "GET" && !path.includes("/clinics/"))
        return listHours(event, userPerms);
      if (method === "GET" && path.includes("/clinics/"))
        return getHours(event, userPerms, clinicId);
      if (method === "POST")
...
async function listHours(event, userPerms) {
  let items = [];
  if (isAdminUser(userPerms.clinicRoles, userPerms.isSuperAdmin, userPerms.isGlobalSuperAdmin)) {
    const resp = await ddb.send(new ScanCommand2({ TableName: TABLE_NAME, Limit: 200 }));
    items = resp.Items || [];
  } else {
...
async function getHours(event, userPerms, clinicId) {
