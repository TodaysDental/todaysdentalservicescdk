// src/services/admin/directory-lookup.ts
...
var handler = async (event) => {
  ...
  try {
    ...
    const directory = (result.Items || []).map((user) => ({
      userID: user.email,
    ...
    const responseNextToken = result.LastEvaluatedKey ? Buffer.from(JSON.stringify(result.LastEvaluatedKey)).toString("base64") : void 0;
    return httpOk({
      items: directory,
