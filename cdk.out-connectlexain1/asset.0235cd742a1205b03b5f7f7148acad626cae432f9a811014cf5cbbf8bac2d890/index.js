...
function buildMessage(notification, platform) {
...
async function sendToEndpoint(endpointArn, notification, platform) {
  try {
    const message = buildMessage(notification, platform);
    const result = await sns.send(new import_client_sns.PublishCommand({
...
async function handleSendToUser(userId, notification, dryRun) {
...
async function handleSendToClinic(clinicId, notification, dryRun) {
