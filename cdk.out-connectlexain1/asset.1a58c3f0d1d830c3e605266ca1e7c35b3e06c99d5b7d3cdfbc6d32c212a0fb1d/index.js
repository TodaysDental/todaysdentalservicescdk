...
// src/services/clinic/reports.ts
var reports_exports = {};
...
var TABLE_NAME = process.env.TABLE_NAME || "SQL_Reports";
var MODULE_NAME = "IT";
...
var handler = async (event) => {
  ...
  const requiredPermission = METHOD_PERMISSIONS[httpMethod] || "read";
  ...
  try {
    ...
    if ((path.includes("/reports/") || path.endsWith("/reports/{reportName}")) && httpMethod === "GET") {
      const reportName = event.pathParameters?.reportName || path.split("/").pop();
...
async function listReports(event, _userPerms) {
...
async function getReport(event, _userPerms, reportName) {
  if (!reportName)
    return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ error: "reportName required" }) };
  const res = await docClient.send(new import_lib_dynamodb.GetCommand({ TableName: TABLE_NAME, Key: { ReportName: reportName } }));
  if (!res.Item)
    return { statusCode: 404, headers: getCorsHeaders(event), body: JSON.stringify({ error: "Not Found" }) };
  return { statusCode: 200, headers: getCorsHeaders(event), body: JSON.stringify(res.Item) };
}
async function createReport(event, _userPerms) {
...
async function updateReport(event, _userPerms, reportName) {
  if (!reportName)
    return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ error: "reportName required" }) };
  const body = parseBody(event.body);
...
async function deleteReport(event, _userPerms, reportName) {
  if (!reportName)
    return { statusCode: 400, headers: getCorsHeaders(event), body: JSON.stringify({ error: "reportName required" }) };
  await docClient.send(new import_lib_dynamodb.DeleteCommand({ TableName: TABLE_NAME, Key: { ReportName: reportName } }));
