...
async function processRecordingIdempotent(ddb2, metadataTableName, callQueueTableName, bucket, key) {
  ...
  const callRecord = await findCallByCallId(ddb2, callQueueTableName, callId);
  if (!callRecord) {
  ...
  const timestamp = callRecord?.queueEntryTime ? new Date(callRecord.queueEntryTime).getTime() : Date.now();
