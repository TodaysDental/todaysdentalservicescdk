  ...
  try {
    ...
    try {
      ...
      await ddb.send(new import_lib_dynamodb3.UpdateCommand({
        ...
        ConditionExpression: "currentCallId = :callRef",
        // Ensure we are updating the correct call
        ExpressionAttributeValues: {
          ":realCallId": callId,
          ":callRef": callReference
        }
    ...
    if (typeof agentId !== "undefined") {
      try {
        await ddb.send(new import_lib_dynamodb3.UpdateCommand({
          ...
          ConditionExpression: "#status = :dialingStatus",
          // Only rollback if they are still "dialing"
          ExpressionAttributeNames: { "#status": "status" },
          ExpressionAttributeValues: {
            ":onlineStatus": "Online",
            ":dialingStatus": "dialing",
