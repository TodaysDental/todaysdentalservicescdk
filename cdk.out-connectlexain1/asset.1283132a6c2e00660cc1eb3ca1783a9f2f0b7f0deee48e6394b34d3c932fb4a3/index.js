...
// src/services/chime/call-hungup.ts
var call_hungup_exports = {};
__export(call_hungup_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(call_hungup_exports);
...
async function sendMissedCallNotification(notification) {
  if (!PUSH_NOTIFICATIONS_ENABLED)
    return;
...
var handler = async (event) => {
  ...
  try {
    ...
    const { Items: callLookupRecords } = await ddb.send(new import_lib_dynamodb5.QueryCommand({
      TableName: CALL_QUEUE_TABLE_NAME,
      IndexName: "callId-index",
      KeyConditionExpression: "callId = :callId",
      ExpressionAttributeValues: { ":callId": callId }
    }));
    const callMetadata = callLookupRecords && callLookupRecords.length > 0 ? callLookupRecords[0] : void 0;
    if (!callMetadata) {
    ...
    if (callMetadata.clinicId) {
      const authzCheck = checkClinicAuthorization(verifyResult.payload, callMetadata.clinicId);
    ...
    if (currentCallStatus === "on_hold" && callMetadata.heldByAgentId && agentId && callMetadata.heldByAgentId !== agentId) {
      ...
      const holdDuration = holdStartTime > 0 ? (Date.now() - holdStartTime) / 1e3 : 0;
      const MAX_HOLD_DURATION = CHIME_CONFIG.HOLD.MAX_HOLD_DURATION_MINUTES * 60;
      const payload = verifyResult.payload;
      const isSupervisor = requestingAgentId && payload && (payload.isSuperAdmin || payload.isGlobalSuperAdmin || payload.roles?.includes("supervisor") || payload.roles?.includes("admin"));
    ...
    const clinicId = typeof callMetadata.clinicId === "string" ? callMetadata.clinicId : void 0;
    ...
    try {
      await chimeVoiceClient.send(new import_client_chime_sdk_voice2.UpdateSipMediaApplicationCallCommand({
        SipMediaApplicationId: smaId,
    ...
    if (startTime && !isNaN(startTime)) {
      ...
      calculatedDuration = Math.max(0, Math.floor((endTime - startTime) / 1e3));
    ...
    if (agentId && callMetadata.clinicId && typeof callMetadata.queuePosition !== "undefined") {
      try {
        await ddb.send(new import_lib_dynamodb5.TransactWriteCommand({
          TransactItems: [
            {
              Update: {
      ...
      try {
        const { Items: callRecords } = await ddb.send(new import_lib_dynamodb5.QueryCommand({
          TableName: CALL_QUEUE_TABLE_NAME,
          IndexName: "callId-index",
          KeyConditionExpression: "callId = :callId",
          ExpressionAttributeValues: { ":callId": callId }
    ...
    if (agentId) {
      if (LOCKS_TABLE_NAME) {
        ...
        if (lockAcquired) {
          try {
            const { Item: refreshedAgentInfo } = await ddb.send(new import_lib_dynamodb5.GetCommand({
              TableName: AGENT_PRESENCE_TABLE_NAME,
              Key: { agentId }
            }));
