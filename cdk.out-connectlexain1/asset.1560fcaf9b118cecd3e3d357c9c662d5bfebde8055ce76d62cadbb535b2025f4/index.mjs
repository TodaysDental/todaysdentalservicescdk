// src/services/auth/initiate-otp.ts
...
var ses = new SESv2Client({ region: process.env.SES_REGION || "us-east-1" });
...
var handler = async (event) => {
  ...
  try {
    ...
    const now = Date.now();
    if (user.otpLastSent && now - user.otpLastSent < RATE_LIMIT_SECONDS * 1e3) {
      const waitTime = Math.ceil((RATE_LIMIT_SECONDS * 1e3 - (now - user.otpLastSent)) / 1e3);
      return {
        statusCode: 429,
        headers,
        body: JSON.stringify({
    ...
    const otpExpiry = now + OTP_EXPIRY_MINUTES * 60 * 1e3;
