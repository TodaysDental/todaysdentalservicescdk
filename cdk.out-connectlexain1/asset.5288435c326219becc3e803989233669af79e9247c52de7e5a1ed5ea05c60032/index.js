...
async function getOrCreateSession(callId, clinicId, agentId, callerNumber) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const directRead = await docClient2.send(new import_lib_dynamodb2.GetCommand({
    ...
    const existingResponse = await docClient2.send(new import_lib_dynamodb2.QueryCommand({
      TableName: VOICE_SESSIONS_TABLE,
    ...
    try {
      ...
      if (error.name === "ConditionalCheckFailedException") {
        ...
        const existingSession = await docClient2.send(new import_lib_dynamodb2.GetCommand({
          TableName: VOICE_SESSIONS_TABLE,
          Key: { sessionId },
          ConsistentRead: true
        }));
  ...
  const finalCheck = await docClient2.send(new import_lib_dynamodb2.GetCommand({
    TableName: VOICE_SESSIONS_TABLE,
    Key: { sessionId },
    ConsistentRead: true
  }));
...
async function getVoiceAgent(clinicId) {
  ...
  const defaultResponse = await docClient2.send(new import_lib_dynamodb2.QueryCommand({
    TableName: AGENTS_TABLE2,
  ...
  const voiceResponse = await docClient2.send(new import_lib_dynamodb2.QueryCommand({
    TableName: AGENTS_TABLE2,
    IndexName: "ClinicIndex",
    KeyConditionExpression: "clinicId = :cid",
  ...
  if (!voiceConfig) {
    const fallbackResponse = await docClient2.send(new import_lib_dynamodb2.QueryCommand({
      TableName: AGENTS_TABLE2,
      IndexName: "ClinicIndex",
      KeyConditionExpression: "clinicId = :cid",
  ...
  try {
    switch (event.eventType) {
      ...
      case "TRANSCRIPT": {
        ...
        let session;
        if (sessionId) {
          const sessionResponse = await docClient2.send(new import_lib_dynamodb2.GetCommand({
            TableName: VOICE_SESSIONS_TABLE,
            Key: { sessionId }
          }));
        ...
        if (!session && callId) {
          const callIdQuery = await docClient2.send(new import_lib_dynamodb2.QueryCommand({
            TableName: VOICE_SESSIONS_TABLE,
            IndexName: "CallIdIndex",
            KeyConditionExpression: "callId = :cid",
            ExpressionAttributeValues: { ":cid": callId },
