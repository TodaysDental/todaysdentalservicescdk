...
// src/services/chime/send-dtmf.ts
var send_dtmf_exports = {};
__export(send_dtmf_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(send_dtmf_exports);
...
var VALID_DTMF_REGEX = /^[0-9*#]+$/;
var MAX_DTMF_LENGTH = 32;
var handler = async (event) => {
  ...
  try {
    ...
    const durationMs = Math.min(Math.max(body.durationMs || 250, 50), 1e3);
    ...
    const isOnCall = agentPresence.currentCallId === body.callId || agentPresence.secondaryCallId === body.callId;
    if (!isOnCall) {
    ...
    try {
      await ddb.send(new import_lib_dynamodb2.UpdateCommand({
        ...
        ExpressionAttributeValues: {
          ...
          ":empty": [],
          ":dtmfEntry": [{
            digits: body.digits.replace(/./g, "*"),
            // Mask digits for security
