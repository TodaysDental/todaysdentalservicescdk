...
var APNS_PLATFORM_ARN = process.env.APNS_PLATFORM_ARN || "";
var APNS_SANDBOX_PLATFORM_ARN = process.env.APNS_SANDBOX_PLATFORM_ARN || "";
var FCM_PLATFORM_ARN = process.env.FCM_PLATFORM_ARN || "";
...
function getPlatformApplicationArn(platform, environment) {
  if (platform === "android") {
...
async function createOrUpdateEndpoint(platformArn, deviceToken, userId, clinicId) {
  const customUserData = JSON.stringify({ userId, clinicId });
  try {
    const createResult = await sns.send(new import_client_sns.CreatePlatformEndpointCommand({
    ...
    try {
      const attributes = await sns.send(new import_client_sns.GetEndpointAttributesCommand({
      ...
      if (currentToken !== deviceToken || enabled !== "true") {
        await sns.send(new import_client_sns.SetEndpointAttributesCommand({
          EndpointArn: endpointArn,
          Attributes: {
            Token: deviceToken,
            Enabled: "true",
...
async function handleRegister(event, userPerms) {
  ...
  const { deviceToken, platform, environment = "production", deviceName, appVersion, osVersion } = body;
  if (!deviceToken) {
  ...
  if (!clinicId) {
    const allowedClinics2 = getAllowedClinicIds(userPerms.clinicRoles, userPerms.isSuperAdmin, userPerms.isGlobalSuperAdmin);
    if (allowedClinics2.size === 1) {
      clinicId = Array.from(allowedClinics2)[0];
    } else {
  ...
  const env = platform === "android" ? "production" : environment;
  try {
